...
function IsHoliday($today, $i){
  $test_date = date('m/d/Y', mktime (0,0,0, $today['mon'], ($today['mday'] + $i), $today['year']));
  ...
  //format_date($y, 3, 17);
  // Easter
  $holidays[4] = calculate_easter($y);
  // Memorial Day
  $holidays[5] = get_holiday($y, 5, 1);
  // Independance Day
  $holidays[6] = observed_day($y, 7, 4);
  // Labor Day
  $holidays[7] = get_holiday($y, 9, 1, 1);
  // Columbus Day
  //$holidays[8] = get_holiday($y, 10, 1, 2);
  // Columbus Day
  //format_date($y, 10, 31);
  // Thanksgiving
  $holidays[8] = get_holiday($y, 11, 4, 4);
  // X-Mas
  $holidays[9] = format_date($y, 12, 25);
  // test- make April 1 a holiday
  $holidays[10] = format_date($y, 4, 1);

  for($x = 0; $x <= 10; $x++){
    // Now Compare the dates to see if we have a holiday or not...
    $holiday = compareDate($test_date, $holidays[$x]);
    if($holiday == 0){
      return 1;
    }
  }
