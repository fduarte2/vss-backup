...
//	echo $sql."<br>";
   if (!ora_fetch_into($Short_Term_Cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		...
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
		header("Location: cargo_release_login.php?input=1");
   } else {
...
Password!  Please <a href="cargo_release_login.php">go back and try again.</a>
               </td>
            </tr>
         </table>
         </p>
...
<?
       ...
       $sql = "insert into eport_user_logins
				...
				values 
					('".$username."', SYSDATE, '".$ip_address."', '".$success."', 'cargo release')";
//		echo $sql."<br>";
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
