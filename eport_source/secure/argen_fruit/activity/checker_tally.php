...
// note:  AT LEAST 1 of these will be set, or this page will not be accessed
if($order_num != ""){
	$more_sql .= " and order_num like '%$order_num%' ";
}
if($pallet_num != ""){
	$more_sql .= " and pallet_id = '$pallet_num' ";
}

/*if($eport_customer_id != 0){
  $stmt = "select to_char(A.DATE_OF_ACTIVITY, 'MM/DD/YYYY HH24:MI:SS') DATE_OF_ACTIVITY, QTY_CHANGE, QTY_LEFT, SERVICE_CODE, PALLET_ID, ACTIVITY_ID, ARRIVAL_NUM, C.CUSTOMER_NAME from cargo_activity A, customer_profile C where A.customer_id = C.customer_id and order_num like '%$order_num%' and A.customer_id = '$eport_customer_id' and A.SERVICE_CODE = '6' and A.ACTIVITY_DESCRIPTION is null order by order_num, date_of_activity";
}
else{
  $stmt = "select to_char(A.DATE_OF_ACTIVITY, 'MM/DD/YYYY HH24:MI:SS') DATE_OF_ACTIVITY, QTY_CHANGE, QTY_LEFT, SERVICE_CODE, PALLET_ID, ACTIVITY_ID, ARRIVAL_NUM, C.CUSTOMER_NAME from cargo_activity A, customer_profile C where A.customer_id = C.customer_id and order_num like '%$order_num%' and A.SERVICE_CODE = '6' and A.ACTIVITY_DESCRIPTION is null order by order_num, date_of_activity";
}*/
if(true /*$eport_customer_id != 0*/){
  $stmt = "select to_char(A.DATE_OF_ACTIVITY, 'MM/DD/YYYY HH24:MI:SS') DATE_OF_ACTIVITY, QTY_CHANGE, QTY_LEFT, SERVICE_CODE, PALLET_ID, ACTIVITY_ID, ARRIVAL_NUM, ORDER_NUM, C.CUSTOMER_NAME from cargo_activity A, customer_profile C where A.customer_id = C.customer_id ".$more_sql." and A.customer_id = '1626' and A.SERVICE_CODE = '6' and A.ACTIVITY_DESCRIPTION is null order by order_num, date_of_activity";
}
else{
  $stmt = "select to_char(A.DATE_OF_ACTIVITY, 'MM/DD/YYYY HH24:MI:SS') DATE_OF_ACTIVITY, QTY_CHANGE, QTY_LEFT, SERVICE_CODE, PALLET_ID, ACTIVITY_ID, ARRIVAL_NUM, ORDER_NUM, C.CUSTOMER_NAME from cargo_activity A, customer_profile C where A.customer_id = C.customer_id ".$more_sql." and A.SERVICE_CODE = '6' and A.ACTIVITY_DESCRIPTION is null order by order_num, date_of_activity";
}
//echo $stmt;
...
// general information
array_push($general_info, array('first'=>"Shipment: $shipment",
...
do {
  ...
  if(true /*$eport_customer_id != 0*/){
    $stmt2 = "select NVL(VARIETY, 'N/A') || ' - ' || NVL(REMARK, 'N/A') || ' - ' || NVL(CARGO_SIZE, 'N/A') DESCR, C.COMMODITY_NAME, NVL(V.VESSEL_NAME, 'TRUCKED-IN') THE_VES from cargo_tracking T, commodity_profile C, vessel_profile V where T.arrival_num = V.arrival_num(+) and T.commodity_code = C.commodity_code and pallet_id = '$pallet_id' and receiver_id = '1626'";
  }
  else{
    $stmt2 = "select VARIETY || ' - ' || REMARK || ' - ' || CARGO_SIZE DESCR, C.COMMODITY_NAME from cargo_tracking T, commodity_profile C where T.commodity_code = C.commodity_code and pallet_id = '$pallet_id'";
  }
  ...
  // add the line item
  array_push($lot_info, array('desc'=>$row2['DESCR'], 'comm'=>$row2['COMMODITY_NAME'], 
			      'pallet'=>$row1['PALLET_ID'], 'qty'=>$row1['QTY_CHANGE'], 'qty_left'=>$row2['THE_VES'],
...
$pdf->ezText("Start Time (First Scan): $start_time - End Time (Last Scan): $end_time");
$pdf->ezSetDy(-10);
...
// write the footer
$today = date('m/j/y'); 
$format = "Port of Wilmington, Printed: " . $today . "    Pallet Total: $pallets         Source Eportwilmington/argen_fruit/activity/checker_tally.php";
$all = $pdf->openObject();
$pdf->saveState();
