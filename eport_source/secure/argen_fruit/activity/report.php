...
if (true /*$eport_customer_id != 0*/ )
{
  $stmt = "select T.pallet_id, CM.commodity_name commodity, C.customer_name customer, 
	          T.qty_received, T.qty_in_house, T.warehouse_location, T.cargo_description, T.mark,
           ...
           where A.customer_id = 1626 and A.date_of_activity > to_date('$date 00:00:00', 'MM/DD/YYYY HH24:MI:SS') and A.date_of_activity < to_date('$date 23:59:59', 'MM/DD/YYYY HH24:MI:SS') and A.customer_id = C.customer_id 
  ...
  $stmt = "select T.pallet_id, CM.commodity_name commodity, C.customer_name customer, 
	          T.qty_received, T.qty_in_house, T.warehouse_location, T.cargo_description, T.mark,
                  ...
                  A.qty_left, A.qty_change, SC.service_name
           from CARGO_TRACKING T, CUSTOMER_PROFILE C, COMMODITY_PROFILE CM, CARGO_ACTIVITY A, SERVICE_CATEGORY SC
           where A.date_of_activity > to_date('$date 00:00:00', 'MM/DD/YYYY HH24:MI:SS') and A.date_of_activity < to_date('$date 23:59:59', 'MM/DD/YYYY HH24:MI:SS') and A.customer_id = C.customer_id 
                  and A.pallet_id = T.pallet_id
                  and A.customer_id = T.receiver_id
                  and A.service_code = SC.service_code
                  and T.commodity_code = CM.commodity_code
...
<title>Eport - Daily Activity Report</title>
</head>
   ...
   <tr>
	 ...
	 <table border="0" width="100%" cellpadding="4" cellspacing="0"> 
	    <tr>
		  ...
		  <?
		  if ($rows1 == 0) {
		    if (false /* $eport_customer_id == 0 */) {
		  ?>		      
		  <font size = "2" face="Verdana">No customers have Activity for <?= $date ?>!  
		  ...
		  ?>
		  <font size = "2" face="Verdana"><?= $user ?> does not have any Activity for <?= $date ?>!  
		  Please go back to select another date to view Activity Report.</font>
		  <br /><br />
		  <hr>
		  <font size = "2" face="Verdana">Port of Wilmington, <?= $today ?>, Printed by <?= $user ?></font>
