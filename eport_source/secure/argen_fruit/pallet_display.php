...
<?
		$sql = "SELECT CARGO_SIZE THE_SIZE, EXPORTER_CODE, TO_NUMBER(BATCH_ID) THE_BATCH, COUNT(*) THE_PALLETS, SUM(QTY_RECEIVED) THE_RECEIVED 
				FROM CARGO_TRACKING
				WHERE ARRIVAL_NUM = '".$vessel."' AND COMMODITY_CODE = '".$comm."' AND RECEIVER_ID = '".$cust."'".$sub_sql."";
		if($size != "ALL"){
			$sql .= " AND CARGO_DESC = '".$size."'";
		}
		$sql .= " AND DATE_RECEIVED IS NOT NULL 
				GROUP BY CARGO_SIZE, EXPORTER_CODE, TO_NUMBER(BATCH_ID) 
				ORDER BY CARGO_SIZE, EXPORTER_CODE, TO_NUMBER(BATCH_ID)";
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
