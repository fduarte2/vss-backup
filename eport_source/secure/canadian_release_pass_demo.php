...
//   $row_num = pg_num_rows($result);
   ...
   if (!ora_fetch_into($Short_Term_Cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		...
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
		header("Location: canadian_release_login_demo.php?input=1");
   } else {
...
Password!  Please <a href="canadian_release_login_demo.php">go back and try again.</a>
               </td>
            </tr>
         </table>
         </p>
...
<?
       ...
       $sql = "insert into eport_user_logins
		...
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
       header("Location: canadian_release_demo/index_canadian.php");
       exit;
      }
	$sql = "insert into eport_user_logins
				(USERNAME, LOGIN_DATE, IP_ADDRESS, RESULT)
