...
//   $row_num = pg_num_rows($result);
   ...
   // if records present
	$sql = "SELECT * FROM eport_login where username = '" . $web_user . "' and user_type = 'CANSW'";
   ...
   if (!ora_fetch_into($Short_Term_Cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		...
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
		header("Location: CanSW_login.php?input=1");
   } else {
...
<html>
<head>
<title>Canadian Swingload Login Error</title>
...
Password!  Please <a href="CanSW_login.php">go back and try again.</a>
               </td>
            </tr>
         </table>
         </p>
...
<?
       ...
       $sql = "insert into eport_user_logins
				...
				values 
					('".$username."', SYSDATE, '".$ip_address."', '".$success."', 'CanSW')";
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
       header("Location: CanadianSwing/index_CanSW.php");
       exit;
      }
	$sql = "insert into eport_user_logins
				(USERNAME, LOGIN_DATE, IP_ADDRESS, RESULT, LOGIN_SYSTEM)
