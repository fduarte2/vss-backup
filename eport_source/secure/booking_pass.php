...
//   $row_num = pg_num_rows($result);
   ...
   // if records present
	$sql = "SELECT * FROM eport_login where username = '" . $web_user . "' and user_type = 'BOOK'";
...
Password!  Please <a href="booking_login.php">go back and try again.</a>
               </td>
            </tr>
         </table>
         </p>
...
<?
       ...
       setcookie("eport_book_flag", "T", time() + 2592000, "/");
       $test_pass = "";
       $web_pass = "";
       $success = "t";
       $sql = "insert into eport_user_logins 
				...
				values 
					('".$username."', SYSDATE, '".$ip_address."', '".$success."', 'booking')";
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
       header("Location: booking/Booking_orders.php");
       exit;
      }
    }
   $sql = "insert into eport_user_logins 
	...
	ora_parse($Short_Term_Cursor, $sql);
	ora_exec($Short_Term_Cursor);
  header("Location: booking_login.php?input=1");
?>
