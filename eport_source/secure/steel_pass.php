...
//   $row_num = pg_num_rows($result);
   ...
   // if records present
	$sql = "SELECT * FROM eport_login where username = '" . $web_user . "' and user_type = 'STEEL'";
   ...
   if (!ora_fetch_into($Short_Term_Cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		...
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
		header("Location: steel_login.php?input=1");
   } else {
...
Password!  Please <a href="steel_login.php">go back and try again.</a>
               </td>
            </tr>
         </table>
         </p>
...
<?
       ...
       $sql = "insert into eport_user_logins
				...
				values 
					('".$username."', SYSDATE, '".$ip_address."', '".$success."', 'steel')";
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
       header("Location: steel/index.php");
       exit;
      }
	$sql = "insert into eport_user_logins
				(USERNAME, LOGIN_DATE, IP_ADDRESS, RESULT, LOGIN_SYSTEM)
