...
<table border="0" width="100%" cellpadding="4" cellspacing="0">
   <tr>
      ...
      <td>
	 <p align="left">
	    <font size="5" face="Verdana" color="#0066CC">Clementine Fruit Reconciliation Report
</font>
	    <hr>
	 </p>
      </td>
...
<table border="0" width="100%" cellpadding="4" cellspacing="0">
<form name="get_data" action="clem_recon_eport_index.php" method="post">
	<tr>
		<td align="left">
...
<?
		$sql = "SELECT DISTINCT ARRIVAL_NUM FROM CARGO_TRACKING 
				WHERE COMMODITY_CODE IN
					(SELECT COMMODITY_CODE FROM COMMODITY_PROFILE WHERE COMMODITY_TYPE = 'CLEMENTINES')
...
<?
		...
		$sql = "SELECT CT.PALLET_ID, COMMODITY_NAME, NVL(TO_CHAR(DATE_RECEIVED, 'MM/DD/YYYY HH24:MI:SS'), 'NR') THE_REC, QTY_RECEIVED, SUBSTR(CT.PALLET_ID, 18, 6) THE_SUBPALLET, WAREHOUSE_LOCATION, ACTUAL_LOCATION
				...
				WHERE CT.ARRIVAL_NUM = '".$vessel."' 
					...
					AND CT.COMMODITY_CODE = CP.COMMODITY_CODE ".$comm_sql.$rec_sql."
...
?>
	<tr <? if ($first_row['THE_REC'] == "NR") {?> bgcolor="#33CC00" <?} else {?> bgcolor="#FFFFFF" <?}?>>
		<td><font size="2" face="Verdana"><? echo $first_row['THE_SUBPALLET']; ?></font></td>
...
<?
				$csv_output .= $first_row['THE_SUBPALLET'].",".$first_row['COMMODITY_NAME'].",".$first_row['THE_REC'].",".$first_row['QTY_RECEIVED'];
...
?>
	<tr>
		<td rowspan="<? echo $second_row['THE_COUNT']; ?>"><font size="2" face="Verdana"><? echo $first_row['THE_SUBPALLET']; ?></font></td>
...
<?
				...
				$sql = "SELECT ACTIVITY_NUM, QTY_CHANGE, CA.SERVICE_CODE, TO_CHAR(DATE_OF_ACTIVITY, 'MM/DD/YYYY HH24:MI:SS') THE_ACT, UPPER(SERVICE_NAME) THE_SERV, DECODE(CA.SERVICE_CODE, 17, SUBSTR(ACTIVITY_DESCRIPTION, 21), NVL(ORDER_NUM, 'N/A')) THE_ORD FROM CARGO_ACTIVITY CA, SERVICE_CATEGORY SC WHERE ARRIVAL_NUM = '".$vessel."' AND CUSTOMER_ID = '".$cust."' AND PALLET_ID = '".$first_row['PALLET_ID']."' AND CA.SERVICE_CODE NOT IN ('1', '2', '8', '9', '12', '16', '24') AND (ACTIVITY_DESCRIPTION IS NULL OR ACTIVITY_DESCRIPTION != 'VOID') AND NOT (CA.SERVICE_CODE = '17' AND QTY_CHANGE = QTY_LEFT) AND CA.SERVICE_CODE = SC.SERVICE_CODE AND CA.ACTIVITY_NUM != '1' ORDER BY ACTIVITY_NUM";
