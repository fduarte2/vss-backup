<?
  ...
  if(!$connection){
    printf("Could not open connection to database server.  Please go back to <a href=\"papinet_login.php\">Papinet Login Page</a> and try again later!<br />"); 
    exit;
   }
   ...
   $query = "SELECT * FROM eport_login where username ~* '" . $web_user . "'
and user_type like 'PAPINET%'";
...
<html>
<head>
<title>Papinet Login Error</title>
...
Password!  Please <a href="papinet_login.php">go back and try again.</a>
               </td>
            </tr>
         </table>
         </p>
...
<?
       ...
       setcookie("eport_papinet_user", "$username", time() + 86400);
       setcookie("eport_papinet_email", "$mail", time() + 86400);
       ...
       header("Location: papinet/index.php");
       exit;
      }
    }
  $sql = "insert into eport_user_logins values ('$username', '$timestamp', '$ip_address', '$success')";
  $result = pg_query($connection, $sql) or 
             die("Error in query: $sql. " .  pg_last_error($connection));
  pg_close($connection);
  header("Location: papinet_login.php?input=1");
?>
