<?
  ...
  if(!$connection){
    printf("Could not open connection to database server.  Please go back to <a href=\"../ccds_login.php\">CCDS Login Page</a> and try again later!<br />"); 
    exit;
   }
   ...
   $query = "SELECT * FROM eport_login where username ~* '" . $web_user . "' and user_type like 'CCDS%'";
...
Password!  Please <a href="ccds_login.php">go back and try again.</a>
               </td>
            </tr>
         </table>
         </p>
...
<?
       ...
       header("Location: ccds/index.php");
       exit;
      }
    }
  $sql = "insert into eport_user_logins values ('$username', '$timestamp', '$ip_address', '$success')";
  $result = pg_query($connection, $sql) or 
             die("Error in query: $sql. " .  pg_last_error($connection));
  pg_close($connection);
  header("Location: ccds_login.php?input=1");
?>
