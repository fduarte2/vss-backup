...
//   $row_num = pg_num_rows($result);
   ...
   // if records present
	$sql = "SELECT * FROM eport_login where username = '" . $web_user . "' and user_type = 'MOROCCAN'";
   ...
   if (!ora_fetch_into($Short_Term_Cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		...
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
		header("Location: moroccan_login.php?input=1");
   } else {
...
//      $username = $row['USERNAME'];
	  ...
	  $scanned = $row['SCANNED_OR_UNSCANNED_LINKS'];
...
Password!  Please <a href="moroccan_login.php">go back and try again.</a>
               </td>
            </tr>
         </table>
         </p>
...
<?
       ...
       setcookie("eport_mor_customer_id", "$customer_id", time() + 86400, "/");
       setcookie("eport_user_type", "$user_type", time() + 86400, "/");
	   setcookie("eport_user_subtype", "$user_subtype", time() + 86400, "/");
	   setcookie("eport_user_scanned", "$scanned", time() + 86400, "/");
       $test_pass = "";
       $web_pass = "";
       $success = "T";
       $sql = "insert into eport_user_logins
				...
				values 
					('".$username."', SYSDATE, '".$ip_address."', '".$success."', 'moroccan')";
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
       header("Location: moroccan/index_moroccan.php");
       exit;
      }
	$sql = "insert into eport_user_logins
				(USERNAME, LOGIN_DATE, IP_ADDRESS, RESULT, LOGIN_SYSTEM)
