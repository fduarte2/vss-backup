...
*****************************************************************/
	...
	if($submit == "Create Order"){
		...
		$sql = "INSERT INTO DOLEPAPER_ORDER (ORDER_NUM, ENTERED_BY, STATUS, ENTERED_DATE, LOAD_DATE, SAIL_DATE, DESTINATION_NB, ARRIVAL_NUM, ORDER_FOR) VALUES ('".$next_order."', '".$user."', '1', SYSDATE, TO_DATE('".$load_date."', 'MM/DD/YYYY'), TO_DATE('".$sail_date."', 'MM/DD/YYYY'), '".$destination."', '".$vessel."', 'PORT')";
		ora_parse($short_term_data_cursor, $sql);
		ora_exec($short_term_data_cursor);
...
<table border="0" width="100%" cellpadding="4" cellspacing="0">
   <tr>
      <td>
         <p align="left">
            <font size="5" face="Verdana" color="#0066CC">Dole PORT ACCOUNT Order Details Page
            </font>
         </p>
		 <hr>
      </td>
...
<table border="0" width="100%" cellpadding="4" cellspacing="0">
<form name="order_select_1" action="order_mod_portact.php" method="post">
	<tr>
		<td align="left" width="10%"><font size="3" face="Verdana">Pending Orders:</td>
...
<?
	...
	$sql = "SELECT ORDER_NUM FROM DOLEPAPER_ORDER WHERE ORDER_FOR = 'PORT' AND STATUS NOT IN ('7', '8') ORDER BY ORDER_NUM DESC";
	ora_parse($short_term_data_cursor, $sql);
	ora_exec($short_term_data_cursor);
...
<form name="order_select_2" action="order_mod_portact.php" method="post">
	<tr>
		<td align="left" width="10%"><font size="3" face="Verdana">Finalized Orders:</td>
		<td align="left"><select name="order_num" onchange="document.order_select_2.submit(this.form)"><option value="">Select an Order:</option>
<?

	$sql = "SELECT ORDER_NUM FROM DOLEPAPER_ORDER WHERE ORDER_FOR = 'PORT' AND STATUS IN ('7', '8') ORDER BY ORDER_NUM DESC";
	ora_parse($short_term_data_cursor, $sql);
	ora_exec($short_term_data_cursor);
	while(ora_fetch_into($short_term_data_cursor, $short_term_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
?>
...
<?
	if($order_num != ""){
		...
		$sql = "SELECT ARRIVAL_NUM, STATUS, TO_CHAR(LOAD_DATE, 'MM/DD/YYYY') THE_LOAD, TO_CHAR(SAIL_DATE, 'MM/DD/YYYY') THE_SAIL, DESTINATION_NB, USER_COMMENTS, STATUS FROM DOLEPAPER_ORDER WHERE ORDER_NUM = '".$order_num."'";
		ora_parse($short_term_data_cursor, $sql);
		ora_exec($short_term_data_cursor);
		ora_fetch_into($short_term_data_cursor, $short_term_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC);
...
?>
<table border="0" width="100%" cellpadding="4" cellspacing="0">
<form name="order_info" action="order_entry_portact.php" method="post" onsubmit="return validate_form()">
...
<?
				$sql = "SELECT THE_TICKET, THE_DESC, THE_CODE, IN_HOUSE, THE_DMG, THE_WEIGHT, NVL(THE_SHIP_ORDERED, 0) THE_SHIP_ORDERED, NVL(THE_DMG_ORDERED, 0)	THE_DMG_ORDERED FROM
...
//				echo $sql;
				...
				while(ora_fetch_into($short_term_data_cursor, $short_term_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
?>
							<option value="<? echo $short_term_row['THE_TICKET']." -- ".$short_term_row['THE_DESC']." -- ".$short_term_row['THE_CODE']." -- ".($short_term_row['IN_HOUSE'] - $short_term_row['THE_SHIP_ORDERED'])." IH -- ".($short_term_row['THE_DMG'] - $short_term_row['THE_DMG_ORDERED'])." DMG -- ".round($short_term_row['THE_WEIGHT'] / $short_term_row['IN_HOUSE'])." lbs avg"; ?>"<? if($short_term_row['THE_TICKET'] == $row['DOCK_TICKET']){$init_IH = $short_term_row['IN_HOUSE'] - $short_term_row['THE_SHIP_ORDERED']; $init_DMG = $short_term_row['THE_DMG'] - $short_term_row['THE_DMG_ORDERED']; $disp_wt = round($short_term_row['THE_WEIGHT'] * $row['QTY_SHIP'] / $short_term_row['IN_HOUSE']); ?> selected <?}?>><? echo $short_term_row['THE_TICKET']." -- ".$short_term_row['THE_DESC']." -- ".$short_term_row['THE_CODE']." -- ".($short_term_row['IN_HOUSE'] - $short_term_row['THE_SHIP_ORDERED'])." IH -- ".($short_term_row['THE_DMG'] - $short_term_row['THE_DMG_ORDERED'])." DMG -- ".round($short_term_row['THE_WEIGHT'] / $short_term_row['IN_HOUSE'])." lbs avg"; ?></option>
<?
				}
?>
					</select></td>
...
<?
				...
				while(ora_fetch_into($short_term_data_cursor, $short_term_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
?>
							<option value="<? echo $short_term_row['THE_TICKET']." -- ".$short_term_row['THE_DESC']." -- ".$short_term_row['THE_CODE']." -- ".($short_term_row['IN_HOUSE'] - $short_term_row['THE_SHIP_ORDERED'])." IH -- ".($short_term_row['THE_DMG'] - $short_term_row['THE_DMG_ORDERED'])." DMG -- ".round($short_term_row['THE_WEIGHT'] / $short_term_row['IN_HOUSE'])." lbs avg"; ?>"><? echo $short_term_row['THE_TICKET']." -- ".$short_term_row['THE_DESC']." -- ".$short_term_row['THE_CODE']." -- ".($short_term_row['IN_HOUSE'] - $short_term_row['THE_SHIP_ORDERED'])." IH -- ".($short_term_row['THE_DMG'] - $short_term_row['THE_DMG_ORDERED'])." DMG -- ".round($short_term_row['THE_WEIGHT'] / $short_term_row['IN_HOUSE'])." lbs avg"; ?></option>
<?
					$temp++;
				}
?>
