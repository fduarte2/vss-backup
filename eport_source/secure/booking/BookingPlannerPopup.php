...
<table border="1" width="100%" cellpadding="4" cellspacing="0">
	<tr>
		...
		<td><font size="2" face="Verdana"><b>Most Recent Shipout Date</b></font></td>
		<td><font size="2" face="Verdana"><b>Arrival #</b></font></td>
	</tr>
<?
	$sql = "SELECT BAD.ORDER_NUM, NVL(BAD.BOOKING_NUM, 'NONE') THE_BOOK, COUNT(DISTINCT CT.PALLET_ID) THE_ROLLS, BPGC.SSCC_GRADE_CODE,
				...
				NVL(TO_CHAR(MIN(DATE_OF_ACTIVITY), 'MM/DD/YYYY'), 'NONE') THE_FIRST, NVL(TO_CHAR(MAX(DATE_OF_ACTIVITY), 'MM/DD/YYYY'), 'NONE') THE_LAST
			FROM CARGO_TRACKING CT, BOOKING_ADDITIONAL_DATA BAD, UNIT_CONVERSION_FROM_BNI UC1, UNIT_CONVERSION_FROM_BNI UC2, CARGO_ACTIVITY CA, BOOKING_PAPER_GRADE_CODE BPGC
	...
	$sql .= " GROUP BY CT.ARRIVAL_NUM, BAD.ORDER_NUM, NVL(BAD.BOOKING_NUM, 'NONE'), ROUND(BAD.DIAMETER * UC2.CONVERSION_FACTOR), BPGC.SSCC_GRADE_CODE";
...
?>
	<tr>
		...
		<td><font size="2" face="Verdana"><? echo $row['THE_FIRST']; ?></font></td>
		<td><font size="2" face="Verdana"><? echo $row['THE_LAST']; ?></font></td>
...
<table border="1" width="100%" cellpadding="4" cellspacing="0">
<?
	$sql = "SELECT DISTINCT ROUND(BAD.WIDTH * UC1.CONVERSION_FACTOR) THE_WIDTHS, BPGC.SSCC_GRADE_CODE
			FROM CARGO_TRACKING CT, BOOKING_ADDITIONAL_DATA BAD, UNIT_CONVERSION_FROM_BNI UC1, BOOKING_PAPER_GRADE_CODE BPGC
			WHERE CT.ARRIVAL_NUM = BAD.ARRIVAL_NUM
				AND CT.PALLET_ID = BAD.PALLET_ID
				AND CT.RECEIVER_ID = BAD.RECEIVER_ID
...
<?
	$sql = "SELECT NVL(BOOKING_NUM, 'NONE') THE_BOOK, ORDER_NUM, BPGC.SSCC_GRADE_CODE, 
	...
	$sql .= " GROUP BY NVL(BOOKING_NUM, 'NONE'), ORDER_NUM, CT.ARRIVAL_NUM, ROUND(BAD.WIDTH * UC1.CONVERSION_FACTOR), BPGC.SSCC_GRADE_CODE";
	$sql .= " ORDER BY NVL(BOOKING_NUM, 'NONE'), ORDER_NUM, CT.ARRIVAL_NUM, ROUND(BAD.WIDTH * UC1.CONVERSION_FACTOR), BPGC.SSCC_GRADE_CODE";
	$ora_success = ora_parse($cursor, $sql);
	$ora_success = ora_exec($cursor, $sql);
	while(ora_fetch_into($cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		if($row['THE_BOOK'] != $print_record['booking']
			...
			|| $row['SSCC_GRADE_CODE'] != $print_record['sscc']
			|| $print_record['arv'] == ""){
			if($print_record['arv'] != ""){
?>
	<tr>
		...
		<td><font size="2" face="Verdana"><? echo $print_record['sscc']; ?></font></td>
<?
...
<?
			...
			$print_record['sscc'] = $row['SSCC_GRADE_CODE'];
		}
