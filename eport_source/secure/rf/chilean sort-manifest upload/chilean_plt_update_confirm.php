...
*/
		...
		$sql = "SELECT COUNT(*) THE_COUNT
				...
				WHERE COMMODITY_CODE = '".$useable_data[($i - 1)]["comm"]."'";
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
		ora_fetch_into($Short_Term_Cursor, $Short_Term_Row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC);
		if($Short_Term_Row['THE_COUNT'] < 1){
			// this code is not recognized.  swap to GRAPES as default
			$useable_data[($i - 1)]["comm"] = "8060";
		}
/*
...
// step 3:  have data, delete existing (if someone backpaged), validate + insert to DB
	...
	$sql = "SELECT COUNT(*) THE_COUNT FROM CHILEAN_CUSTOMER_PLT_CHANGES WHERE TRANSACTION_ID = '".$trans_id."' AND PALLET_TO_DB_COMPARE = 'ALREADYREC'";
	ora_parse($Short_Term_Cursor, $sql);
	ora_exec($Short_Term_Cursor);
	ora_fetch_into($Short_Term_Cursor, $Short_Term_Row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC);
	$display_count_received = $Short_Term_Row['THE_COUNT'];
	$sql = "SELECT COUNT(*) THE_COUNT FROM CHILEAN_CUSTOMER_PLT_CHANGES WHERE TRANSACTION_ID = '".$trans_id."' AND PALLET_TO_DB_COMPARE = 'TOBEEXCHG'";
	ora_parse($Short_Term_Cursor, $sql);
	ora_exec($Short_Term_Cursor);
	ora_fetch_into($Short_Term_Cursor, $Short_Term_Row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC);
	$display_count_tobeexchg = $Short_Term_Row['THE_COUNT'];
...
<table border="0" width="100%" cellpadding="4" cellspacing="0">
	...
	<tr>
		<td colspan="3"><font size="3" face="Verdana"><? echo ($display_count_good + $display_count_noDB + $display_count_tobetrans + $display_count_tobeexchg); ?> Pallets in File:</font></td>
	</tr>
...
<input type="hidden" name="trans_id" value="<? echo $trans_id; ?>">
	...
	<tr>
		<td><font size="3" face="Verdana"><? echo $display_count_tobeexchg; ?>  Pallet(s) matched, but per Shipping Line, these belong to a different receiver.  PoW will resolve during discharge.</font></td>
		<td colspan="2"><font size="3" face="Verdana"><a href="match_pallets.php?trans_id=<? echo $trans_id; ?>&type=TOBEEXCHG&cust=<? echo $user_cust_num; ?>" target="match_pallets.php?trans_id=<? echo $trans_id; ?>&type=TOBEEXCHG&cust=<? echo $user_cust_num; ?>">Review</a></font></td>
	</tr>
	<tr>
		<td><font size="3" face="Verdana"><? echo $display_count_received; ?>  Pallet(s) matched, but were already received under another customer.  Please contact PoW to resolve.</font></td>
		<td colspan="2"><font size="3" face="Verdana"><a href="match_pallets.php?trans_id=<? echo $trans_id; ?>&type=ALREADYREC&cust=<? echo $user_cust_num; ?>" target="match_pallets.php?trans_id=<? echo $trans_id; ?>&type=ALREADYREC&cust=<? echo $user_cust_num; ?>">Review</a></font></td>
	</tr>
</form>
	<tr>
		<td><font size="3" face="Verdana"><? echo $display_count_noDB; ?>  Pallet(s) did not match any available in our system.</font><? if($display_count_noDB > 0){?><font size="3" face="Verdana" color="#FF0000">  They will be added to our System.</font><? } ?></td>
...
// 3A)  does it match?
		...
		$count = $Short_Term_Row['THE_COUNT'];
		if($count == 0){
			// no matches?  check again with no zeroes
			$match_attempt_pallet_no0 = strip_zero($pallet_to_match);

			$sql = "SELECT PALLET_ID 
					FROM CARGO_TRACKING_ADDITIONAL_DATA 
					WHERE PLT_ID_NO_LEAD_ZERO = '".$match_attempt_pallet_no0."' 
			...
			if(!ora_fetch_into($Short_Term_Cursor, $Short_Term_Row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
				// still nothing.  just let the "momatch" pass.
			} else {
				// there is a match with a different # of zeroes.  use the "matching to CARGO_TRACKING" pallet ID instead for the rest of this.
				$pallet_to_match = $Short_Term_Row['PALLET_ID'];
				$count = 1;
			}
		}
		...
		if($count > 0){
			...
			$sql = "SELECT CHILEAN_SORT_FILE_UPLOAD, MANIFESTED_RECEIVER_ID, CT.RECEIVER_ID, NVL(TO_CHAR(DATE_RECEIVED), 'UNREC') THE_RECEIVED
...
//					$comparison = "TOBETRANS";
					if($Short_Term_Row['THE_RECEIVED'] == "UNREC"){
						$comparison = "TOBEEXCHG"; // not yet received, just hand it to the new guy
					} else {
						$comparison = "ALREADYREC"; // already received.  do nothing but notify
					}
//				} else {
...
// 3B)  The actual write
		$sql = "INSERT INTO CHILEAN_CUSTOMER_PLT_CHANGES
				...
				'".$comparison."',
				'".$pallet_to_match."')";
//		echo "Pallet: ".$useable_data[$i]["pallet"]."  type:  ".$comparison."<br>";
//		echo $sql."<br>";
		ora_parse($cursor, $sql);
		ora_exec($cursor);
...
function strip_zero($string){
	if(substr($string, 0, 1) !== "0"){
		return $string;
	} else {
		return strip_zero(substr($string, 1));
	}
}