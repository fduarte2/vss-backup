...
if($eport_customer_id != 0){
  $stmt = "select to_char(A.DATE_OF_ACTIVITY, 'MM/DD/YYYY HH24:MI:SS') DATE_OF_ACTIVITY, QTY_CHANGE, QTY_LEFT, SERVICE_CODE, PALLET_ID, ACTIVITY_ID, ARRIVAL_NUM, ORDER_NUM, C.CUSTOMER_NAME from cargo_activity A, customer_profile C where A.customer_id = C.customer_id ".$more_sql." and A.customer_id = '$eport_customer_id' and A.SERVICE_CODE = '6' and A.ACTIVITY_DESCRIPTION is null order by order_num, date_of_activity";
}
else{
...
do {
  ...
  if($eport_customer_id != 0){
    $stmt2 = "select NVL(VARIETY, 'N/A') || ' - ' || NVL(REMARK, 'N/A') || ' - ' || NVL(CARGO_SIZE, 'N/A') DESCR, C.COMMODITY_NAME, NVL(V.VESSEL_NAME, 'TRUCKED-IN') THE_VES from cargo_tracking T, commodity_profile C, vessel_profile V where T.arrival_num = V.arrival_num(+) and T.commodity_code = C.commodity_code and pallet_id = '$pallet_id' and receiver_id = '$eport_customer_id'";
  }
  else{
