...
//		echo $sql."<br>";
		...
		$sql = "UPDATE CARGO_TRACKING_ADDITIONAL_DATA CTAD
				SET SUPPLIER_PACKDATE = (SELECT MIN(SUPPLIER_PACKDATE) FROM WM_UPLOAD_HISTORY WCUH, WALMART_CARGO_RAW_DUMP WCRD
										WHERE WCUH.UPLOAD_NUMBER = WCRD.UPLOAD_NUMBER
											AND WCUH.UPLOAD_NUMBER = '".$upload_num."'
											AND CTAD.PALLET_ID = WCRD.PALLET_ID
											AND CTAD.ARRIVAL_NUM = WCUH.ARRIVAL_NUM)
				WHERE ARRIVAL_NUM = '".$cur_ves."'
					AND RECEIVER_ID = '".$view_cust."'
					AND PALLET_ID IN
					(SELECT PALLET_ID FROM WALMART_CARGO_RAW_DUMP WHERE UPLOAD_NUMBER = '".$upload_num."')";
