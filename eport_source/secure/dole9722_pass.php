...
//   $row_num = pg_num_rows($result);
   ...
   // if records present
	$sql = "SELECT * FROM eport_login_v2 where username = '" . $web_user . "'";// and user_type = 'CARGO_REL'";
...
//	echo $sql."<br>";
   if (!ora_fetch_into($Short_Term_Cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		...
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
		header("Location: dole9722_login.php?input=1");
   } else {
...
Password!  Please <a href="dole9722_login.php">go back and try again.</a>
               </td>
            </tr>
         </table>
         </p>
...
<?
       ...
       setcookie("eport_user_v2", "$username", time() + 86400, "/");
...
/*	   setcookie("eport_user_ams_auth", "$ams_auth", time() + 86400, "/");
       ...
       $sql = "insert into eport_user_logins
				...
				values 
					('".$username."', SYSDATE, '".$ip_address."', '".$success."', 'dole9722')";
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
       header("Location: dole9722/index.php");
       exit;
      }
	$sql = "insert into eport_user_logins
				(USERNAME, LOGIN_DATE, IP_ADDRESS, RESULT, LOGIN_SYSTEM)
