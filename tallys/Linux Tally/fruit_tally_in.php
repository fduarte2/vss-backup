...
$desc_cursor = ora_open($rf_conn);
	...
	$sql = "SELECT PALLET_ID, QTY_CHANGE, ACTIVITY_NUM, SERVICE_CODE
			FROM CARGO_ACTIVITY
	...
	if(!ora_fetch_into($pallet_cursor, $pallet_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		...
		$filename = "Fruitin_".$order."_".$timedatestamp;
		...
		fwrite($handle, "PORT OF WILMINGTON CHILEAN FRUIT INBOUND TALLY\n\n");
		fwrite($handle, "Customer: ".$custname."\n");
		fwrite($handle, "Order#: ".$order."\n");
		if($pallet_row['SERVICE_CODE'] != "11"){
			fwrite($handle, "SHIPPING TYPE:  INBOUND\n\n");
		} else {
			fwrite($handle, "SHIPPING TYPE:  INBOUND - TRANSFER\n\n");
		}
		...
		$the_desc = str_pad("DESCRIPTION", 100);
		$the_qty = str_pad("QTY", 5);
		$the_checker = str_pad("CHECKER", 8);
		fwrite($handle, $the_BC.$the_desc.$the_qty.$the_checker."\n");
		fwrite($handle, "================================================================================================================================================\n");
		do {
			$emp_name = get_employee_for_print($pallet_row['PALLET_ID'], $order, $cust, $pallet_row['ACTIVITY_NUM'], $rf_conn);
			$total_ctn += $pallet_row['QTY_CHANGE'];
			$total_plt++;

			$the_BC = str_pad(trim($pallet_row['PALLET_ID']), 33);
			$the_desc = str_pad(GetPalletDesc($pallet_row['PALLET_ID'], $order, $cust, $swingflag, $rf_conn), 100);
			$the_qty = str_pad($pallet_row['QTY_CHANGE'], 5);
		...
		$disp_total_words = str_pad(trim("Total Cases:"), 40);
		$disp_total_qty = str_pad(trim($total_ctn), 5);
		fwrite($handle, $disp_total_spaces.$disp_total_words.$disp_total_qty."\n");
		$disp_total_spaces = str_pad("", 93);
		$disp_total_words = str_pad(trim("Total Pallets:"), 40);
		$disp_total_qty = str_pad(trim($total_plt), 5);
		fwrite($handle, $disp_total_spaces.$disp_total_words.$disp_total_qty."\n");
		fwrite($handle, "\n\n");
