...
Begin VB.Form frmSFLaborTicket 
   ...
   Begin VB.OptionButton optVessel 
      Caption         =   "List All Vessels"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   End
   Begin VB.OptionButton optServ 
      Caption         =   "List Relevant Services"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
...
'              " AND S.STATUS = 'N' and S.SERVICE_GROUP_ID = G.SERVICE_GROUP_ID ORDER BY G.SERVICE_GROUP_ID"
    ...
    'fogbugz 362 / HD 11237 -- AW 4/8/2016.  Adding a radio button for "all" ust like the cust and comm boxes
    If optServ(1).Value = True Then
        sqlStmt = "SELECT DISTINCT G.SERVICE_GROUP_ID, G.SERVICE_GROUP_NAME FROM HOURLY_DETAIL D, SERVICE S, SERVICE_GROUP G " & _
                 " WHERE S.SERVICE_CODE = D.SERVICE_CODE AND S.SERVICE_GROUP_ID = G.SERVICE_GROUP_ID ORDER BY G.SERVICE_GROUP_ID"
    Else
        'HD9281
...
Private Sub cmdShipList_Click()
    ...
    If optVessel(1).Value = True Then
        sqlStmt = "SELECT LR_NUM, VESSEL_NAME FROM VESSEL_PROFILE WHERE LR_NUM < 100000 ORDER BY LR_NUM DESC"
        Set rsVessel = OraDatabaseBNI.DBCreateDynaset(sqlStmt, 0&)
        If OraDatabaseBNI.LastServerErr = 0 And rsVessel.RecordCount > 0 Then
            While Not rsVessel.EOF
                frmPV.lstPV.AddItem rsVessel.Fields("lr_num").Value & " : " & rsVessel.Fields("vessel_name").Value
                rsVessel.MoveNext
            Wend
        End If
    Else
...
'        End If
        If rs.Fields("BILL_STATUS").Value = "V" Then
            MsgBox "Ticket Number " & rs.Fields("TICKET_NUM").Value & " has been Voided by the Supervisor.  Please contact them for further information."
        ElseIf rs.Fields("BILL_STATUS").Value = "U" Then
            MsgBox "Ticket Number " & rs.Fields("TICKET_NUM").Value & " has been Rejected by Finance.  Please correct and resubmit the ticket."
            Call retrieveTicket(ticket_date, cust_id, service_group, Trim(Me.txtCommId.Text))
        ElseIf rs.Fields("BILL_STATUS").Value = "Y" Then
            MsgBox "Ticket Number " & rs.Fields("TICKET_NUM").Value & " is currently being handled by Finance.  You can view the ticket, but cannot save changes."
            Call retrieveTicket(ticket_date, cust_id, service_group, Trim(Me.txtCommId.Text))
        Else
            Call retrieveTicket(ticket_date, cust_id, service_group, Trim(Me.txtCommId.Text))
        End If
...
Private Sub retrieveTicket(ticket_date As String, cust_id As Long, service_group As Integer, com_code)
    ...
    sqlStmt = "SELECT * FROM LABOR_TICKET_HEADER WHERE USER_ID = '" & UserID & "' AND LOCATION_ID = '" & txtArea.Text & "' AND " & _
            ...
            " AND COMMODITY_CODE=" & com_code & " AND (BILL_STATUS IS NULL OR BILL_STATUS != 'V')"
    'Else
    '    sqlStmt = " SELECT * FROM LABOR_TICKET_HEADER WHERE USER_ID = '" & UserID & "' AND " & _
    '          " CUSTOMER_ID = " & cust_id & " AND SERVICE_GROUP = " & service_group & " AND " & _
    '          " SERVICE_DATE = TO_DATE('" & ticket_date & "', 'MM/DD/YYYY')"
