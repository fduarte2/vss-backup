...
Public Function ValidateDescription(UpdatedValue As Variant) As Boolean
    ValidateDescription = True
    If InStr(1, UpdatedValue, "'") > 0 Then
        MsgBox "no apostrophes, please.  Use the tick mark next to the number 1 on your keyboard if absolutely necessary."
        ValidateDescription = False
    End If
End Function

Public Function ValidateStartDate(UpdatedValue As Variant) As Boolean
    ValidateStartDate = True
    If Not IsDate(Format(UpdatedValue, "MM/DD/YYYY")) Then
        MsgBox "Starting Date must be in MM/DD/YYYY Format."
        ValidateStartDate = False
    End If
End Function

Public Function ValidateEndDate(UpdatedValue As Variant) As Boolean
    ValidateEndDate = True
    If Not IsDate(Format(UpdatedValue, "MM/DD/YYYY")) Then
        MsgBox "Ending Date must be in MM/DD/YYYY Format."
...
'    If UpdatedValue <> "A" And UpdatedValue <> "V" And UpdatedValue <> "T" And UpdatedValue <> "R" And UpdatedValue <> "X" Then
'        MsgBox "Arrival Type must be either A, V, T, R, or X."
...
'    If UpdatedValue <> "A" And UpdatedValue <> "B" And UpdatedValue <> "C" And UpdatedValue <> "D" And UpdatedValue <> "E" And UpdatedValue <> "F" And UpdatedValue <> "G" And UpdatedValue <> "H" And UpdatedValue <> "" Then
'        MsgBox "Warehouse must be either A, B, C, D, E, F, G, or H."
