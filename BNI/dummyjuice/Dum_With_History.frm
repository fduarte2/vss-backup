...
Begin VB.Form frmD_W_His 
   ...
   Begin VB.CommandButton exit 
      ...
      Width           =   1695
   End
   Begin VB.CommandButton PrintUnsaved2 
...
Private Sub exit_Click()
  Unload Me     'Juice Proj 4/19/2007 Rudy:
  End
End Sub
...
Private Sub printunsaved_Click()
          'Rudy.  Note as of 5/11/2007 this is not used anymore, use PrintUnsaved2_click sub routine instead
    'Dim ds CARGO_DELIVERY As Object
    ...
    'Juice Proj 4/19/2007 Rudy: begin block
    On Error GoTo ErrHandler      'have this EH if Word fails, it'll bubble up to here
    Dim strOrigDummyNum As String
    strOrigDummyNum = txtDeliveryNum.Text
    txtDeliveryNum.Text = "*" & txtDeliveryNum.Text & "*"
    
    txtDeliveryNum.SelStart = 0
    txtDeliveryNum.SelLength = Len(txtDeliveryNum.Text)
    'txtDeliveryNum.SelText
    Clipboard.SetText txtDeliveryNum.SelText
    'Juice Proj 4/19/2007 Rudy: end block
...
'
'    'BC C39 2 to 1 HD Wide
'    Num_Of_Lines = Num_Of_Lines - 35
'
'    Printer.Print Tab(118); "Barcode:"
'
'    Printer.FontBold = False
'
''    Printer.FontName = "BC C39 2 to 1 HD Wide"
''    Printer.FontSize = 18
'
'    'from their help file:
'    Printer.Font.Name = "BC C128 HD Wide"
'    Printer.Font.size = 36
'    'Printer.Print BC_Code39(Data1.Recordset.Fields("Code128"))
'
'    'Do Events
'
'    Printer.Print ""
'    'Printer.Print Tab(72); "*"; txtDeliveryNum.Text; "*";
'    Printer.Print Tab(35); txtDeliveryNum.Text;
'    Printer.Print ""
'
'    Printer.FontName = "TIMES NEW ROMAN"      'set it back
'    Printer.FontBold = True
'    Printer.FontSize = 12
'    Printer.Print Tab(82); " Dummy Number :            " & txtDeliveryNum.Text;
'
    'Juice Proj 4/19/2007 Rudy: end block
    ...
    'Juice Proj 4/19/2007 Rudy: here to end of sub incl new eh
    PrintBarcode
    
    txtDeliveryNum.Text = strOrigDummyNum

  Exit Sub
ErrHandler:
    MsgBox "Error " & Err & " - " & Error$ & " occurred while processing.", vbExclamation, "Print Dummy Withdrawal"
...
Private Sub save_dummy_Click()
 ...
 'Lock all the required tables in exclusive mode, try 10 times
    ...
    '    Msg Box "Table could not be locked. Please try again. Server Message: " & OraDatabase.LastServerErrText, vbExclamation, "Save Delivery"
    '    Exit Sub
    'End If
    On Error GoTo 0
    iError = False
    ...
    If Trim$(cboSuper.Text) = "" Then
       MsgBox "Supervisor Field Cannot Be Null"
       Exit Sub
    End If
...
'        Set dsDummy_max = Nothing
    ...
    Else
        ...
        Dim dsDummy_Del As Object          'only in save_dummy
            ...
            If Trim$(txtDelDetQty1(i).Text) <> "" And Val(Trim$(txtDelDetQty1(i).Text)) > 0 Then
                dsDummy.AddNew
                        ...
                        If chkCOA.Value = 1 Then
                            dsDummy.Fields("COA").Value = "Y"
                        End If
                        
                        dsDummy.Fields("COMMENTS").Value = comment.Text & vbCrLf & comment1.Text & vbCrLf & comment2.Text
...
Private Sub cmdCommodityList_Click()
    ...
    gsSqlStmt = "SELECT * FROM COMMODITY_PROFILE WHERE COMMODITY_TYPE = 'JUICE' ORDER BY COMMODITY_CODE"
...
'    Set OraDatabase = OraSession.OpenDatabase("BNITEST", "SAG_OWNER/BNITEST238", 0&)     'Juice Proj 4/19/2007 Rudy: orig above
...
Private Sub txtCommodityCode_LostFocus()
    If Trim$(txtCommodityCode) <> "" Then
        gsSqlStmt = "SELECT * FROM COMMODITY_PROFILE WHERE COMMODITY_TYPE = 'JUICE' AND COMMODITY_CODE = " & txtCommodityCode.Text
        Set dsCOMMODITY_PROFILE = OraDatabase.DbCreateDynaset(gsSqlStmt, 0&)
        If OraDatabase.LastServerErr = 0 And dsCOMMODITY_PROFILE.RecordCount > 0 Then
...
Private Sub txtDelDetQty1_LostFocus(Index As Integer)
    ...
    If OraDatabase.LastServerErr = 0 And dsCARGO_MANIFEST.RecordCount > 0 Then
        ...
        Dim dsCARGO_TRACKING As Object         '5/14/2007 only in lost_focus
        ...
        If OraDatabase.LastServerErr = 0 And dsCARGO_TRACKING.RecordCount > 0 Then
            ...
            Dim dsDummy_verify As Object       'only in lost focus
            ...
            If OraDatabase.LastServerErr = 0 And dsDummy_verify.RecordCount > 0 Then
                ...
                End If
            End If
            Set dsDummy_verify = Nothing

            '..................11_07_2001
        End If
        Set dsCARGO_TRACKING = Nothing
...
Public Sub dummy_fill()
    ...
    If OraDatabase.LastServerErr = 0 And dsDummy.RecordCount > 0 Then
        ...
        If Not IsNull(dsDummy.Fields("COMMENTS").Value) And dsDummy.Fields("COMMENTS").Value <> "" Then
            Dim teststring As String
            teststring = dsDummy.Fields("COMMENTS").Value
            Dim temp() As String
            temp() = Split(teststring, vbCrLf)
            comment.Text = temp(0)
            If UBound(temp) > 0 Then
                comment1.Text = temp(1)
            End If
            If UBound(temp) > 1 Then
                comment2.Text = temp(2)
            End If
        Else
            comment.Text = ""
            comment1.Text = ""
...
Private Sub txtDeliveryNum_LostFocus()
    ...
    Dim dsCARGO_DELIVERY As Object       '5/14/2007 only in lost_focus
    ...
    If OraDatabase.LastServerErr = 0 And dsCARGO_DELIVERY.RecordCount > 0 Then
        ...
        Do While Not dsCARGO_DELIVERY.EOF
            ...
            Dim dsCARGO_ACTIVITY As Object       '5/14/2007 only in lost_focus
           ...
           Set dsCARGO_ACTIVITY = Nothing
            ...
            Dim dsCARGO_ACTIVITY_EXT As Object   '5/14/2007 only in lost_focus
...
Sub PrintBarcode()    'Juice Proj 4/19/2007 Rudy: new sub, pull these three subs into one place
            'Rudy.  Note as of 5/11/2007 this is not used anymore, it's in the old PrintUnsaved_click
  CreateWordObject
...
'5/14/2007 Juice Rudy: New sub, to print info into the header of the word doc
Private Sub UseHeaderObject()
Dim msm As String
...
Dim CRPosition2 As Long
  ...
  'looks like I req Do Events here!!  Number 2
  DoEvents
  
  lblStatus.Caption = "UseHeaderObject Bookmarks begin"
  wrdApp.ActiveDocument.Bookmarks("txtVesselName").Range.Text = txtVesselName.Text
  wrdApp.ActiveDocument.Bookmarks("txtCustomerName").Range.Text = txtCustomerName.Text
  wrdApp.ActiveDocument.Bookmarks("txtCommodityName").Range.Text = txtCommodityName.Text
  wrdApp.ActiveDocument.Bookmarks("txtDeliveryNum").Range.Text = Replace(txtDeliveryNum.Text, "*", "")
  wrdApp.ActiveDocument.Bookmarks("txtIntial").Range.Text = txtIntial.Text
    
  wrdApp.ActiveDocument.Bookmarks("txtDelDetOrderDate").Range.Text = txtDelDetOrderDate.Text
  wrdApp.ActiveDocument.Bookmarks("txtDelDetOrderNum").Range.Text = txtDelDetOrderNum.Text
  wrdApp.ActiveDocument.Bookmarks("cboDelDetTransMode").Range.Text = cboDelDetTransMode.Text
  wrdApp.ActiveDocument.Bookmarks("txtDelDetTransNum").Range.Text = txtDelDetTransNum.Text
  wrdApp.ActiveDocument.Bookmarks("txtDelDetCustomerName").Range.Text = txtDelDetCustomerName.Text
  wrdApp.ActiveDocument.Bookmarks("cboSuper").Range.Text = cboSuper.Text


  wrdApp.ActiveDocument.Bookmarks("DeliveryAddr1").Range.Text = DeliveryAddr1
  wrdApp.ActiveDocument.Bookmarks("DeliveryAddr2").Range.Text = DeliveryAddr2
  wrdApp.ActiveDocument.Bookmarks("DeliveryAddr3").Range.Text = DeliveryAddr3
  wrdApp.ActiveDocument.Bookmarks("DeliveryAddr4").Range.Text = DeliveryAddr4
  wrdApp.ActiveDocument.Bookmarks("txtDelDetRemarks").Range.Text = txtDelDetRemarks.Text
  wrdApp.ActiveDocument.Bookmarks("rem1").Range.Text = rem1
  wrdApp.ActiveDocument.Bookmarks("rem2").Range.Text = rem2
  lblStatus.Caption = "UseHeaderObject Bookmarks end"
  
End Sub

'5/14/2007 Juice Rudy: New sub, to print info into the footer of the word doc
Private Sub UseFooterObject()
  'looks like I reg Do Events here!!  Number 3
  DoEvents
    
  lblStatus.Caption = "UseFooterObject Bookmarks end"
  wrdApp.ActiveDocument.Bookmarks("txtDelDetQty1Total").Range.Text = txtDelDetQty1Total.Text
  wrdApp.ActiveDocument.Bookmarks("txtDelDetQty2Total").Range.Text = txtDelDetQty2Total.Text
  wrdApp.ActiveDocument.Bookmarks("txtDelDetWeightTotal").Range.Text = Format(Val(txtDelDetWeightTotal.Text), "##,###,###,##0.00")
  
  wrdApp.ActiveDocument.Bookmarks("comment").Range.Text = Space(50) & comment.Text
  wrdApp.ActiveDocument.Bookmarks("comment1").Range.Text = Space(50) & comment1.Text
  wrdApp.ActiveDocument.Bookmarks("comment2").Range.Text = Space(50) & comment2.Text
  lblStatus.Caption = "UseFooterObject Bookmarks end"

End Sub

'5/14/2007 Juice Rudy: New sub, to print info into the body of the word doc
Private Sub UseBodyObject()
...
Dim msm As String
Dim DumOrder As String
Dim strBarcodeUseable As String
Dim dsCARGO_MANIFEST As Object
Dim wt As Double
Dim check As String
Dim mark As String
Dim bol As String
Dim sSqlStmt As String
Dim strBlankLines As String 'HD 2967 Rudy 6/12/2007
Dim j As Integer            'HD 2967 Rudy 6/12/2007
Dim i As Integer            'added as integer 5/11/2007
Dim Num_Of_Lines As Integer 'added as integer 5/11/2007
Dim strBookmark As String
  
'  lblStatus.Caption = "UseBodyObject wrd App.WindowState = wdWindowStateMinimize"
'  wrdApp.WindowState = wdWindowStateMinimize
  
'  lblStatus.Caption = "wrd App.Visible = True"
''  wrdApp.Visible = True     'useful for de bugging, ie can SEE it happening.  Will not run without it??!!
''  wrdApp.Visible = False
'
'  lblStatus.Caption = "UseBodyObject wrd App.Activate"
'  wrdApp.Activate
  
'  lblStatus.Caption = "UseBodyObject wrd App.WindowState = wdWindowStateMinimize"
'  wrdApp.WindowState = wdWindowStateMinimize
  
  lblStatus.Caption = "UseBodyObject App.Path & \DummyNumBarcode.doc "
  msm = App.Path & "\DummyNumBarcode.doc"
  
  Me.Refresh
  
  lblStatus.Caption = "UseBodyObject Open FileName " & msm
  msm = wrdApp.Documents.Open(FileName:=msm, ReadOnly:=True)
  
  DumOrder = txtDeliveryNum.Text
  
  strBarcodeUseable = "*" & DumOrder & "*"
  ...
  For i = 0 To 9
    ...
    mark = txtDelDetMark(i).Text
    If (bol <> "") Then
      sSqlStmt = "SELECT CARGO_LOCATION FROM CARGO_MANIFEST WHERE CARGO_BOL='"
      sSqlStmt = sSqlStmt & bol & "' AND LR_NUM = '" & txtLRNum & "' AND CARGO_MARK = '" & mark & "'"
      Set dsCARGO_MANIFEST = OraDatabase.DbCreateDynaset(sSqlStmt, 0&)
      If OraDatabase.LastServerErr = 0 And dsCARGO_MANIFEST.RecordCount > 0 Then
        loc.Text = dsCARGO_MANIFEST.Fields("CARGO_LOCATION").Value
      Else
    ...
    ' edit Adam walter, May 31, 2007.  Removing TR*'s from mark before printing, to save space.
    If InStr(1, mark, "TR*") > 0 Then
        mark = Mid(mark, 1, InStr(1, mark, "TR*") - 1)
    Else
        ' do nothing
    End If
    ...
    If (check <> "") Then
      wt = Val(txtDelDetWeight(i).Text)
      strBookmark = "Detail" & (i + 1)
      'wrdApp.ActiveDocument.Bookmarks(strBookmark).Range.Text = txtDelDetBOL(i).Text & vbTab & mark & vbTab & loc.Text & vbTab & txtDelDetQty1(i).Text & vbTab & txtDelDetUnit1(i).Text & vbTab & txtDelDetQty2(i).Text & vbTab & txtDelDetUnit2(i).Text & vbTab & Format(wt, "##,###,###,##0.00") & vbTab & whse(i).Text
      wrdApp.ActiveDocument.Bookmarks(strBookmark).Range.Text = txtDelDetBOL(i).Text & vbTab & mark & vbTab & loc.Text & vbTab & txtDelDetQty1(i).Text & vbTab & txtDelDetUnit1(i).Text & vbTab & txtDelDetQty2(i).Text & vbTab & txtDelDetUnit2(i).Text & vbTab & Format(wt, "##,###,###,##0.00") & vbTab & whse(i).Text & vbCrLf     'HD 2967 Rudy 6/12/2007
    Else
  ...
  'HD 2967 Rudy 6/12/2007 begin
  If Num_Of_Lines > 9 Then
    Num_Of_Lines = 9
  End If
  
  For j = 1 To Num_Of_Lines - 5
    strBlankLines = strBlankLines & vbCrLf
  Next
  'HD 2967 Rudy 6/12/2007 end
  
  wrdApp.ActiveDocument.Bookmarks("BlankLines").Range.Text = strBlankLines
  
  lblStatus.Caption = "UseBodyObject Bookmarks (Barcode)"

  sSqlStmt = "SELECT * FROM CARGO_TRACKING WHERE ARRIVAL_NUM = '" & txtLRNum & "' AND DATE_RECEIVED IS NOT NULL"
  Set dsCARGO_TRACKING_RF = OraDatabaseRF.DbCreateDynaset(sSqlStmt, 0&)
  If dsCARGO_TRACKING_RF.RecordCount = 0 Then
    wrdApp.ActiveDocument.Bookmarks("NoScanWarning").Range.Text = "DO NOT SCAN THIS ORDER"
  End If
  wrdApp.ActiveDocument.Bookmarks("Barcode").Range.Text = strBarcodeUseable
  wrdApp.ActiveDocument.Bookmarks("DummyNumber").Range.Text = DumOrder

  'try a Do Events here!!    Number 1
  DoEvents

End Sub


Private Sub PrintUnsaved2_click()
      'Note as of 5/11/2007 this is a new and preferred routine (so don't use PrintUnsaved2_click sub routine anymore)

  On Error GoTo ErrHandler      'have this EH if Word fails, it'll bubble up to here
  Dim strStatus As String
  strStatus = lblStatus.Caption
  ...
  'this one first, it opens the doc
  UseBodyObject
  ...
  lblStatus.Caption = "PrintUnsaved2_click wrd App.PrintOut"
  wrdApp.PrintOut

  lblStatus.Caption = "PrintUnsaved2_click wrd App.Visible = False"
  wrdApp.Visible = False
  ...
  Exit Sub
ErrHandler:
    MsgBox "Error " & Err & " - " & Err.Description & " occurred while processing.", vbExclamation, "Print Dummy Withdrawal"
    lblStatus.Caption = "Error Occurred."
End Sub
