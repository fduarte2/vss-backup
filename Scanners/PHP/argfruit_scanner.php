<?
/*
*	Argen Fruit Scanner, originally written Sep2012.
*
*	is a "copy" of ARGEN FRUIT, with a couple changes, per request.
********************************************************************************/
...
function Alter_Original_Count($CID){
	...
	while(strtoupper($Barcode) != "X" && strtoupper($Barcode) != ""){
		...
		$sql = "SELECT COUNT(*) THE_COUNT FROM CARGO_ACTIVITY WHERE PALLET_ID = '".$Barcode."' AND CUSTOMER_ID = '".$Cust."' AND ARRIVAL_NUM = '".$Arrival."' AND SERVICE_CODE NOT IN ('1', '2', '8', '6', '12', '16')";
		$ora_success = ora_parse($short_term_data_cursor, $sql);
		database_check($ora_success, "Unable to get\nPallet Info\n(AOC6a)");
		$ora_success = ora_exec($short_term_data_cursor, $sql);
		database_check($ora_success, "Unable to get\nPallet Info\n(AOC6b)");
...
//					mail("lstewart@port.state.de.us,awalter@port.state.de.us\r\n", $mailSubject, $body, $mailHeaders);
					...
					$sql = "INSERT INTO SCANNER_EMAIL_LOG (FROM_SCANNER_TYPE, DATE_SENT, EMAIL_TO_LIST, EMAIL_BODY, EMAIL_SUBJECT) VALUES ('Argen', SYSDATE, '".$inv_mail_list."', '".$body."', '".$mailSubject."')";
					$ora_success = ora_parse($modify_cursor, $sql);
...
//			Receive_Walmart($Arrival, $Barcode, $Comm);
			...
			if(!$proceed){
				...
				$sql = "SELECT DATE_RECEIVED, TO_CHAR(DATE_RECEIVED, 'MM/DD/YYYY') TEXT_DATE, QTY_DAMAGED, WAREHOUSE_LOCATION, VARIETY, CARGO_SIZE, CHEP FROM CARGO_TRACKING WHERE PALLET_ID = '".$Barcode."' AND ARRIVAL_NUM = '".$Arrival."' AND COMMODITY_CODE = '".$Comm."' AND RECEIVER_ID = '".$Cust."'";
				$ora_success = ora_parse($short_term_data_cursor, $sql);
				database_check($ora_success, "Unable to get\nPallet Info\n(US4a)");
				$ora_success = ora_exec($short_term_data_cursor, $sql);
				database_check($ora_success, "Unable to get\nPallet Info\n(US4b)");
...
/*					} elseif($short_term_data_row['TEXT_DATE'] != date('m/d/Y')) {
				...
				do{
					...
					echo $asterisk_array[1]."2)Loc: ".$loc."\n";
					echo "***************\n";
					echo $asterisk_array[2]."3)Comm: ".$commname."\n";
					echo $asterisk_array[3]."4)QTY Rcvd: ".$qty_rec."\n";
					echo $asterisk_array[4]."5)QTY Dmg: ".$qty_dmg."\n";
					echo $asterisk_array[5]."6)Faulty?: ".$fault."\n";
					...
					if($walmart_split_count > 0 && ($Choice != "" && $Choice != "X" && $Choice != "5")){
						$Choice = "bad";
					}
...
function Load_Truck($CID){
	...
	while($continue_function){ // in case they finish order and want to move to next one 
		...
		while($order_num == ""){
			...
			// is this a Port-expedited order?
			$sql = "SELECT COUNT(*) THE_COUNT FROM ARGFRUIT_EXPED WHERE CUSTOMER_ID = '".$cust."'";
			$ora_success = ora_parse($short_term_data_cursor, $sql);
			database_check($ora_success, "Cannot Retrieve\nCustomer\n(LT21a)");
			$ora_success = ora_exec($short_term_data_cursor, $sql);
			database_check($ora_success, "Cannot Retrieve\nCustomer\n(LT21b)");
			...
			if($short_term_data_row['THE_COUNT'] >= 1){
				...
				ora_fetch_into($short_term_data_cursor, $short_term_data_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC);
				if($short_term_data_row['THE_COUNT'] >= 1){
					// REIDRECT OUT OF FUNCTION TO FINISH
					Load_Truck_PortExp($CID, $order_num, $cust);
					return;
				} else {
					$sql = "SELECT COUNT(*) THE_COUNT 
							FROM ARGENFRUIT_ORDER_HEADER_NB
							WHERE ORDER_NUM = '".$order_num."'";
					$ora_success = ora_parse($short_term_data_cursor, $sql);
					database_check($ora_success, "Cannot Retrieve\nCustomer\n(LT23a)");
					$ora_success = ora_exec($short_term_data_cursor, $sql);
...
/*			if($continue_pallet && ($cust != $pallet_owner)){
			...
			// get this new "port pallet" concept
			if($continue_pallet){
				$sub_choice = "default";
...
/*				if($cust != $pallet_owner){
				...
				$sql = "UPDATE CARGO_TRACKING SET QTY_IN_HOUSE = QTY_IN_HOUSE - ".$qty_to_ship.", CARGO_STATUS = '".$wing."' WHERE PALLET_ID = '".$Barcode."' AND RECEIVER_ID = '".$cust."' AND ARRIVAL_NUM = '".$Arrival."'";
				$ora_success = ora_parse($modify_cursor, $sql);
				database_check($ora_success, "Unable to \nUpdate Pallet\n(LT14a)");
				$ora_success = ora_exec($modify_cursor, $sql);
				database_check($ora_success, "Unable to \nUpdate Pallet\n(LT14b)");

				// new argen fruit "port pallet" thingy
				$sql = "UPDATE CARGO_TRACKING_ADDITIONAL_DATA ";
				if($port_pallet == "Y"){
					$sql .= "SET PORT_PALLET = 'Y' ";
				} else {
					$sql .= "SET PORT_PALLET = 'N' ";
				}
				$sql .= "WHERE PALLET_ID = '".$Barcode."' AND RECEIVER_ID = '".$cust."' AND ARRIVAL_NUM = '".$Arrival."'";
				$ora_success = ora_parse($modify_cursor, $sql);
				database_check($ora_success, "Unable to \nUpdate Pallet\n(LT14a-a)");
				$ora_success = ora_exec($modify_cursor, $sql);
				database_check($ora_success, "Unable to \nUpdate Pallet\n(LT14a-b)");
...
//			mail("lstewart@port.state.de.us,awalter@port.state.de.us\r\n", $mailSubject, $email_list_of_changed_owner_pallets, $mailHeaders);
			...
			$sql = "INSERT INTO SCANNER_EMAIL_LOG (FROM_SCANNER_TYPE, DATE_SENT, EMAIL_TO_LIST, EMAIL_BODY, EMAIL_SUBJECT) VALUES ('Argen', SYSDATE, '".$inv_and_fina_mail_info_list."', '".$email_list_of_changed_owner_pallets."', '".$mailSubject."')";
			$ora_success = ora_parse($modify_cursor, $sql);
			database_check($ora_success, "Unable to\nInsert Email\n(LT17a)");
			$ora_success = ora_exec($modify_cursor, $sql);
			database_check($ora_success, "Unable to\nInsert Email\n(LT17b)");
...
function Load_Truck_PortExp($CID, $order_num, $cust){
	global $rf_conn;
	global $scanner_type;
	...
	if($cust == "1626"){
		$detail_table = "ARGENFRUIT_ORDER_DETAIL";
	} else {
		$detail_table = "ARGENFRUIT_ORDER_DETAIL_NB";
	}
...
//	$total_cases = 0;
	...
	// get cases on order, including partials
	$sql = "SELECT NVL(SUM(QTY_CHANGE), 0) THE_SUM, COUNT(DISTINCT PALLET_ID) THE_PLTS
			FROM CARGO_ACTIVITY 
			WHERE CUSTOMER_ID = '".$cust."' 
				AND ORDER_NUM = '".$order_num."' 
				AND SERVICE_CODE IN ('6') 
	...
	while(ora_fetch_into($short_term_data_cursor, $short_term_data_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		if($short_term_data_row['THE_SUM'] > 0 || $short_term_data_row['THE_SUM'] < 0){
			$total_cases = (0 + $short_term_data_row['THE_SUM']);
			$total_pallets = (0 + $short_term_data_row['THE_PLTS']);
		}
	}
...
//			fscanf(STDIN, "%s\n", $junk);
		...
		// is this pallet on HOLD?
		if($continue_pallet){
			$sql = "SELECT NVL(HOLD_STATUS, 'N') THE_HOLD FROM CARGO_TRACKING_ADDITIONAL_DATA WHERE RECEIVER_ID = '".$cust."' AND PALLET_ID = '".$Barcode."' AND ARRIVAL_NUM = '".$Arrival."'";
			$ora_success = ora_parse($short_term_data_cursor, $sql);
			database_check($ora_success, "Cannot Retrieve\nPallet HOLD Info\n(LTP9a)");
			$ora_success = ora_exec($short_term_data_cursor, $sql);
			database_check($ora_success, "Cannot Retrieve\nPallet HOLD Info\n(LTP9b)");
		...
		if($continue_pallet){
			// get other pallet data
			$sql = "SELECT NVL(TO_CHAR(DATE_RECEIVED, 'MM/DD/YYYY'), 'NONE') THE_DATE, CUSTOMER_NAME, COMMODITY_NAME, 
						NVL(VESSEL_NAME, CT.ARRIVAL_NUM) THE_VES, WAREHOUSE_LOCATION, QTY_RECEIVED, QTY_DAMAGED, CARGO_SIZE, VARIETY, QTY_IN_HOUSE,
						BATCH_ID, CARGO_SIZE, BOL
					FROM CARGO_TRACKING CT, CUSTOMER_PROFILE CUSP, COMMODITY_PROFILE COMP, VESSEL_PROFILE VP 
					WHERE PALLET_ID = '".$Barcode."' 
						AND CT.RECEIVER_ID = '".$cust."' 
						AND CT.ARRIVAL_NUM = '".$Arrival."' 
						AND CUSP.CUSTOMER_ID = CT.RECEIVER_ID 
						AND COMP.COMMODITY_CODE = CT.COMMODITY_CODE 
			...
			$qty_to_ship = $short_term_data_row['QTY_IN_HOUSE'];
			$date_rec = $short_term_data_row['THE_DATE'];
			$variety = trim($short_term_data_row['VARIETY']);
			$voucher = trim($short_term_data_row['BATCH_ID']);
			$impcode = trim($short_term_data_row['BOL']);
			$cargo_size = trim($short_term_data_row['CARGO_SIZE']);
			$custname = substr($short_term_data_row['CUSTOMER_NAME'], 0, 12);
			$commname = substr($short_term_data_row['COMMODITY_NAME'], 0, 12);
			$vesname = substr($short_term_data_row['THE_VES'], 0, 12);
			$loc = substr($short_term_data_row['WAREHOUSE_LOCATION'], 0, 15);
		...
		// is this pallet type even on the order?
		if($continue_pallet){
			$sql = "SELECT COUNT(*) THE_COUNT
					FROM ".$detail_table."
					WHERE ORDER_NUM = '".$order_num."'
						AND VARIETY = '".$variety."'
						AND VOUCHER_NUM = '".$voucher."'
						AND (IMPORT_CODE IS NULL OR IMPORT_CODE = '".$impcode."')
						AND (IMPORT_SIZE IS NULL OR IMPORT_SIZE = '".$cargo_size."')";
//			echo $sql."\n";
//			fscanf(STDIN, "%s\n", $junk);
			$ora_success = ora_parse($short_term_data_cursor, $sql);
			database_check($ora_success, "Cannot Retrieve\nPallet Info\n(LTP11a)");
			...
			database_check($ora_success, "Cannot Retrieve\nPallet Info\n(LTP11b)");
			ora_fetch_into($short_term_data_cursor, $short_term_data_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC);
			$orderlines = $short_term_data_row['THE_COUNT'];
			if($orderlines < 1){
				fresh_screen("ARGEN FRUIT\nTruck Out\nEnter X to exit.\n\n".$Barcode."\n**PALLET DOES NOT\nMATCH ORDER\nDETAIL\nCONTACT SUPERVISOR**", "bad");
				fscanf(STDIN, "%s\n", $junk);
				$continue_pallet = false;
		...
		// is this pallet going to cause the order to go over?
		if($continue_pallet){
			$sql = "SELECT SUM(CARTONS) THE_CTNS
					FROM ".$detail_table."
					WHERE ORDER_NUM = '".$order_num."'
						AND VARIETY = '".$variety."'
						AND VOUCHER_NUM = '".$voucher."'
...
//			echo $sql."\n";
			...
			database_check($ora_success, "Cannot Retrieve\nPallet Info\n(LTP12b)");
			ora_fetch_into($short_term_data_cursor, $short_term_data_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC);
			$requested = $short_term_data_row['THE_CTNS'];
			...
			$sql = "SELECT SUM(QTY_CHANGE) THE_CTNS
					...
					WHERE CT.ARRIVAL_NUM = CA.ARRIVAL_NUM
						...
						AND CT.VARIETY = '".$variety."'
						AND ('".$cargo_size."' IS NULL OR CT.CARGO_SIZE = '".$cargo_size."')
						AND ('".$impcode."' IS NULL OR CT.BOL = '".$impcode."')";
//			echo $sql."\n";
//			fscanf(STDIN, "%s\n", $junk);
			$ora_success = ora_parse($short_term_data_cursor, $sql);
			database_check($ora_success, "Cannot Retrieve\nPallet Info\n(LTP13a)");
			...
			database_check($ora_success, "Cannot Retrieve\nPallet Info\n(LTP13b)");
			ora_fetch_into($short_term_data_cursor, $short_term_data_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC);
			$already_scanned = $short_term_data_row['THE_CTNS'];

			if(($already_scanned + $qty_to_ship) > $requested){
				fresh_screen("ARGEN FRUIT\nTruck Out\nEnter X to exit.\n\n".$Barcode."\n**CASES REQUESTED\nEXCEEDS CASES\nON ORDER\nCONTACT SUPERVISOR**", "bad");
				fscanf(STDIN, "%s\n", $junk);
				$continue_pallet = false;
			}
...
if($DoleSuperCHOICE == 1){
	...
	do {
		...
		fresh_screen(" Argen Fruit (Checker)");
		echo " 1: Super(", $SID, ")\n";
		echo " 2: Chkr(", $CID, ")\n";
		echo " 3: Unload Cargo\n";
