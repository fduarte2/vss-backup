...
?>
	...
	<tr>
		...
		<td width="10%" align="right"><font size="2" face="Verdana">Total Carton Count:</font></td>
		<td width="40%" align="left"><font size="2" face="Verdana"><? echo $display_order_info['total_carton']; ?></font></td>
	</tr>
	<tr>
		...
		<td width="10%" align="right"><font size="2" face="Verdana">Total Pallet Count:</font></td>
		<td width="40%" align="left"><font size="2" face="Verdana"><? echo $display_order_info['total_pallet']; ?></font></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td width="40%" align="left"><font size="2" face="Verdana"><? echo $display_order_info['total_boxdmg']; ?></font></td>
	</tr>
	<tr>
		<td width="10%" align="right"><font size="2" face="Verdana">BoL Comments:</font></td>
		<td width="40%" align="left"><input type="text" name="bol_comment" size="15" maxlength="15" value="<? echo $display_order_info['bol_comment']; ?>"<? echo $new_disabled; ?>></td>
		<td width="10%" align="right"><font size="2" face="Verdana">Total Price:</font></td>
		<td width="40%" align="left"><font size="2" face="Verdana"><? echo "$".number_format($display_order_info['total_price'], 2); ?></font></td>
	</tr>
	<tr>
		<td width="10%" align="right"><font size="2" face="Verdana">Temp Recorder#:</font></td>
		<td width="40%" align="left"><input type="text" name="temp_recorder" size="15" maxlength="15" value="<? echo $display_order_info['temp_recorder']; ?>"<? echo $new_disabled; ?>></td>
...
function get_display_values(&$display_order_info, &$display_orderdet_info, $submitted_values_top, $submitted_values_bottom, $submit, $ordernum, $cust, $success, $conn){
	...
	if($submit == "Save" && !$success){
		...
		$sql = "SELECT VESSELID, COMMODITYCODE, ORDERSTATUSID, CUSTOMERID, DIRECTORDER, CONSIGNEEID, TESTATUS, CUSTOMERPO, SNMGNUM, LOADTYPE, COMMENTS,
						TRANSPORTERID, TRANSPORTCHARGES, DRIVERNAME, TRAILERNUM, TRUCKTAG, SEALNUM, COMMENTSCANCEL,
		...
		$display_order_info['bol_comment'] = "??";
		$display_order_info['temp_recorder'] = "??";
	...
	$display_order_info['total_price'] = ociresult($stid, "TOTALPRICE");
	...
	for($i = 0; $i < 10; $i++){
		if($submitted_values_bottom[$i]['size'] != ""){
			...
			if($submitted_values_bottom[$i]['price'] == "" || !is_numeric($submitted_values_bottom[$i]['price']) || $submitted_values_bottom[$i]['price'] <= 0){
				$error .= "Price (on line ".($i + 1).") is required, must be more than zero, and must be Numeric.<br>";
			}
