...
//		printf(ora_errorcode($conn));
	...
	$commtype = $HTTP_POST_VARS['commtype'];
	$cust = $HTTP_POST_VARS['cust'];
	$submit = $HTTP_POST_VARS['submit'];
	$del_file = $HTTP_POST_VARS['del_file'];

	if($submit == "Upload File" && $vessel != "" && $cust != "" && $commtype != ""){
...
//		echo "imp-filename: ".substr($impfile_sourcenote, -35)."<br>";
	...
	//			$useable_data[($i - 1)]["cust"] = trim($data->sheets[0]['cells'][$i][1]);
				...
				$useable_data[($i - 1)]["desc"] = trim($data->sheets[0]['cells'][$i][9]);
				$useable_data[($i - 1)]["pkg"] = trim($data->sheets[0]['cells'][$i][10]);
				$useable_data[($i - 1)]["bol"] = trim($data->sheets[0]['cells'][$i][11]);
				$useable_data[($i - 1)]["wt"] = trim($data->sheets[0]['cells'][$i][12]);
	...
	//			$useable_data[($i - 1)]["cust"] = trim($data->sheets[0]['cells'][$i][1]);
				$useable_data[($i - 1)]["BC"] = trim($data->sheets[0]['cells'][$i][1]);
				if(strlen($useable_data[($i - 1)]["BC"]) == "30" && substr($useable_data[($i - 1)]["BC"], 0, 5) == "61112"){
					$barcode_mor_check = true;
				} else {
					$barcode_mor_check = false;
				}
				$useable_data[($i - 1)]["comm"] = trim($data->sheets[0]['cells'][$i][2]);
				$useable_data[($i - 1)]["variety"] = trim($data->sheets[0]['cells'][$i][3]);
				$useable_data[($i - 1)]["label"] = trim($data->sheets[0]['cells'][$i][4]);
				...
				$useable_data[($i - 1)]["qty"] = trim($data->sheets[0]['cells'][$i][7]);
				$useable_data[($i - 1)]["whs"] = trim($data->sheets[0]['cells'][$i][8]);
				$useable_data[($i - 1)]["pkg"] = trim($data->sheets[0]['cells'][$i][9]);
				$useable_data[($i - 1)]["desc"] = trim($data->sheets[0]['cells'][$i][10]);
				$useable_data[($i - 1)]["bol"] = trim($data->sheets[0]['cells'][$i][11]);
				$useable_data[($i - 1)]["wt"] = trim($data->sheets[0]['cells'][$i][12]);
...
//		echo "VAlidating file<br>";
		if($commtype == "CLEM"){
			$upload_valid = validate_upload_clem($useable_data, $vessel, $cust, $commtype, $barcode_mor_check, $rfconn); // this function also resolves in-barcode aspects
		} else {
			$upload_valid = validate_upload_chilean($useable_data, $vessel, $cust, $commtype, $rfconn);
		}
...
/*			$sql = "UPDAET WIP_UPLOADS
			...
			for($i = 2; $i <= $data->sheets[0]['numRows']; $i++){
				if($commtype == "CLEM" && $useable_data[($i - 1)]["qty"] > 0){
					$mark = "AT POW";
				} elseif($commtype == "CLEM" && $useable_data[($i - 1)]["qty"] == 0){
					$mark = "SHIPPED";
				} else {
					$mark = "";
				}
				if($commtype == "CLEM"){
					$batch = $useable_data[($i - 1)]["qty"];
					$exporter = $useable_data[($i - 1)]["pkg"];
				} else {
					$batch = $useable_data[($i - 1)]["pkg"];
					$exporter = "";
				}
...
//				echo $sql."<br>";
		...
		while(ocifetch($stid)){
			...
			if(ociresult($short_term_data, "THE_COUNT") >= 1){		
				...
				if(ociresult($stid, "BOL") != ""){
					$sql .= ", BOL = '".ociresult($stid, "BOL")."'";
				}
				if(ociresult($stid, "WEIGHT") != ""){
					$sql .= ", WEIGHT = '".ociresult($stid, "WEIGHT")."'";
				}
				if(ociresult($stid, "BATCH_ID") != ""){
					$sql .= ", BATCH_ID = '".ociresult($stid, "BATCH_ID")."'";
				}
				if(ociresult($stid, "EXPORTER_CODE") != ""){
					$sql .= ", EXPORTER_CODE = '".ociresult($stid, "EXPORTER_CODE")."'";
				}
				if(ociresult($stid, "MARK") != ""){
					$sql .= ", MARK = '".ociresult($stid, "MARK")."'";
				}
				...
				$sql = "UPDATE CARGO_ACTIVITY
						SET QTY_CHANGE = '".ociresult($stid, "QTY")."'
						WHERE PALLET_ID = '".ociresult($stid, "PALLET_ID")."'
...
/*			$sql = "UPDAET WIP_UPLOADS
			...
			for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
				...
				// was removed then re-added on 12/16/2015 per Sally email
...
//   $sql = "SELECT LR_NUM, VESSEL_NAME FROM VESSEL_PROFILE WHERE SHIP_PREFIX = 'CHILEAN' AND TO_CHAR(LR_NUM) IN (SELECT ARRIVAL_NUM FROM CARGO_TRACKING WHERE QTY_IN_HOUSE > 0 AND DATE_RECEIVED IS NULL AND RECEIVER_ID = '".$user_cust_num."' ORDER BY LR_NUM DESC";
	$sql = "SELECT LR_NUM, VESSEL_NAME FROM VESSEL_PROFILE WHERE (SHIP_PREFIX IN ('ARG FRUIT', 'CLEMENTINES', 'CHILEAN') OR LR_NUM = '9999999') ORDER BY LR_NUM DESC";
	$stid = ociparse($rfconn, $sql);
	ociexecute($stid);
	while(ocifetch($stid)){
?>
...
?>
					</select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Commodity Type:  <select name="commtype">
							<option value=""<? if($commtype == ""){?> selected <?}?>>Select a Commodity Type</option>
							<option value="CHILEAN"<? if($commtype == "CHILEAN"){?> selected <?}?>>CHILEAN</option>
							<option value="CLEM"<? if($commtype == "CLEM"){?> selected <?}?>>CLEMENTINE</option>
							<option value="ARG FRUIT"<? if($commtype == "ARG FRUIT"){?> selected <?}?>>ARGENTINE FRUIT</option>
							<option value="PERUVIAN"<? if($commtype == "PERUVIAN"){?> selected <?}?>>PERUVIAN</option>
					</select>
		</td>
	</tr>
	<tr>
...
<form name="save_data" action="add_or_update.php" method="post">
	...
	<tr>
		<td width="5%">&nbsp;</td>
		<td width="40%"><select name="Add" multiple=true size=10 style="width:330px" readonly>
<?
...
?>
		...
		<td width="10%">&nbsp;</td>
		<td width="40%"><select name="Update" multiple=true size=10 style="width:330px" readonly>
<?
		$sql = "SELECT PALLET_ID, RECEIVER_ID
				FROM ADD_OR_UPDATE_HOLD_TABLE 
...
<?
	$sql = "SELECT DISTINCT ARRIVAL_NUM, SOURCE_NOTE
			...
			WHERE (PALLET_ID, RECEIVER_ID, ARRIVAL_NUM) NOT IN
				(SELECT PALLET_ID, CUSTOMER_ID, ARRIVAL_NUM
				FROM CARGO_ACTIVITY
				WHERE DATE_OF_ACTIVITY >= SYSDATE - 365)
...
function validate_upload_chilean($useable_data, $vessel, $cust, $commtype, $rfconn){
...
// 2 - 10)  grower (CARGO_TRACKING.CARGO_DESCRIPTION)
		if(strlen($useable_data[$i]["desc"]) > 60){
			$return .= "Line: ".$badline." - Cargo Description (Grower) cannot be longer than 60 characters.<br>";
		}

// 2 - 11)  pkg (CARGO_TRACKING.BATCH_ID for Chilean)
...
// 3 - 2)  Commodity code
		...
		if($commtype == "CLEM"){
			$sql .= " AND COMMODITY_TYPE = 'CLEMENTINES'";
		} else {
			$sql .= " AND COMMODITY_TYPE = '".$commtype."'";
		}
//		echo $sql."<br>";
		$stid = ociparse($rfconn, $sql);
		ociexecute($stid);
		ocifetch($stid);
		if(ociresult($stid, "THE_COUNT") <= 0){
			$return .= "Line: ".$badline." - Commodity Not in COMMODITY_PROFILE for type ".$commtype.".<br>";
		}
/*
// 3 - 3)  Customer#
		$sql = "SELECT COUNT(*) THE_COUNT
...
function validate_upload_clem(&$useable_data, $vessel, $cust, $commtype, $barcode_mor_check, $rfconn){
	$return = "";
// step 1, make sure no fields have an invalid character (" or , or ' or \)
...
// 1)  Precheck
		// does this pallet already exist? 
		$sql = "SELECT COUNT(*) THE_COUNT
				FROM CARGO_TRACKING
		...
		if($exists == false && $barcode_mor_check == true){
			// if this is a new pallet (and the barcode is suitably "moroccan"), try and reconcile mising parts of the input from the barcode
			if($useable_data[$i]["size"] == ""){
				$useable_data[$i]["size"] = substr($useable_data[$i]["BC"], 27, 3);
			}
			if($useable_data[$i]["qty"] == ""){
				$useable_data[$i]["qty"] = substr($useable_data[$i]["BC"], 24, 3);
			}
			if($useable_data[$i]["pkg"] == ""){
				$useable_data[$i]["pkg"] = substr($useable_data[$i]["BC"], 13, 4);
			}
		}
...
// 2 - 5)  size
		if(!is_numeric($useable_data[$i]["size"])){
			$return .= "Line: ".$badline." - Clementine Size must be a number.<br>";
		}
		if(strlen($useable_data[$i]["size"]) != 3){
			$return .= "Line: ".$badline." - Clementine Size must be 3 digits.<br>";
		}
		if($useable_data[$i]["size"] == "" && $exists == false){
			$return .= "Line: ".$badline." - For New Pallets, Clementine Size must be entered.<br>";
		}
...
// 2 - 7)  qty
		...
		if($useable_data[$i]["qty"] == "" && $exists == false){
			$return .= "Line: ".$badline." - For New Pallets, Clementine QTY must be entered.<br>";
		}
...
// 2 - 9)  grower (CARGO_TRACKING.CARGO_DESCRIPTION)
		if($useable_data[$i]["desc"] != ""){
			$return .= "Line: ".$badline." - For Clementines, the Package field must be blank.<br>";
		}

// 2 - 10)  pkg 
		if(strlen($useable_data[$i]["pkg"]) > 4){
			$return .= "Line: ".$badline." - PKG-type cannot be longer than 4 characters.<br>";
		}
		if($useable_data[$i]["pkg"] == "" && $exists == false){
			$return .= "Line: ".$badline." - For New Pallets, Clementine PKG-House must be entered.<br>";
		}

// 2 - 11)  weight
		if(!$exists && $useable_data[$i]["wt"] != ""){
			$return .= "Line: ".$badline." - Clementine New Pallets get their weight from the Barcode.  Please clear the Weight field.<br>";
		}

		if(!is_numeric($useable_data[$i]["wt"]) && $useable_data[$i]["wt"] != ""){
			$return .= "Line: ".$badline." - Clementine Weight must be longer numeric.<br>";
		}

		// compile the description
		if($useable_data[$i]["desc"] == ""){
			$useable_data[$i]["desc"] = $useable_data[$i]["pkg"]."-".$useable_data[$i]["size"]."-".$useable_data[$i]["qty"];
		}
...
// 3 - 2)  Commodity code
		...
		if($commtype == "CLEM"){
			...
			$sql .= " AND COMMODITY_TYPE = 'CHILEAN'";
		}
//		echo $sql."<br>";
		$stid = ociparse($rfconn, $sql);
		ociexecute($stid);
