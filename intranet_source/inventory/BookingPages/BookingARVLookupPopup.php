...
<table border="1" width="100%" cellpadding="4" cellspacing="0">
<?
	$sql = "SELECT CT.ARRIVAL_NUM, CT.PALLET_ID, BAD.BOOKING_NUM, NVL(TO_CHAR(DATE_RECEIVED, 'MM/DD/YYYY'), '---NONE---') THE_REC, BPGC.SSCC_GRADE_CODE,
				BAD.ORDER_NUM, ROUND(BAD.WIDTH * UC1.CONVERSION_FACTOR, 1) THE_WIDTH, ROUND(BAD.DIAMETER * UC2.CONVERSION_FACTOR, 1) THE_DIA,
				NVL(TO_CHAR(DATE_OF_ACTIVITY, 'MM/DD/YYYY'), '---NONE---') THE_SHIP_TIME,
				ROUND(WEIGHT * UC3.CONVERSION_FACTOR) WEIGHT_LB, NVL(CA.ORDER_NUM, '---NONE---') OB_ORDER
...
?>
	<tr>
		...
		<td><font size="2" face="Verdana"><? echo $row['THE_DIA']."cm/".round($row['THE_DIA'] / 2.54, 1)."\""; ?></font></td>
		<td><font size="2" face="Verdana"><? echo $row['THE_WIDTH']."cm/".round($row['THE_WIDTH'] / 2.54, 1)."\""; ?></font></td>
		<td><font size="2" face="Verdana"><? echo $row['THE_REC']; ?></font></td>
		<td><font size="2" face="Verdana"><? echo $row['WEIGHT_LB']; ?></font></td>
		<td><font size="2" face="Verdana"><? echo $row['THE_SHIP_TIME']; ?></font></td>
		<td><font size="2" face="Verdana"><? echo $row['OB_ORDER']; ?></font></td>
<?
		} while(ora_fetch_into($cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC));
	}
?>
