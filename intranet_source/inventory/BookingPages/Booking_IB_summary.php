...
<form name="the_form" action="Booking_IB_summary.php" method="post">
	<tr>
...
*/
		...
		$sql = "SELECT TO_CHAR(DATE_OF_ACTIVITY, 'MM/DD/YYYY') ACT_DATE, CT.ARRIVAL_NUM,
					COUNT(DISTINCT CT.PALLET_ID) THE_ROLLS, BAD.ORDER_NUM, BAD.BOOKING_NUM,
					SUM(ROUND(WEIGHT * UC3.CONVERSION_FACTOR)) WEIGHT_LB, SUM(ROUND(WEIGHT * UC4.CONVERSION_FACTOR)) WEIGHT_KG
				FROM CARGO_TRACKING CT, BOOKING_ADDITIONAL_DATA BAD, CARGO_ACTIVITY CA, 
					UNIT_CONVERSION_FROM_BNI UC3, UNIT_CONVERSION_FROM_BNI UC4
				WHERE CT.ARRIVAL_NUM = BAD.ARRIVAL_NUM And CT.PALLET_ID = BAD.PALLET_ID And CT.RECEIVER_ID = BAD.RECEIVER_ID
		...
		$sql .= "GROUP BY TO_CHAR(DATE_OF_ACTIVITY, 'MM/DD/YYYY'), CT.ARRIVAL_NUM, BAD.ORDER_NUM, BAD.BOOKING_NUM
				ORDER BY TO_DATE(TO_CHAR(DATE_OF_ACTIVITY, 'MM/DD/YYYY'), 'MM/DD/YYYY'), CT.ARRIVAL_NUM, BAD.ORDER_NUM, BAD.BOOKING_NUM"; 
