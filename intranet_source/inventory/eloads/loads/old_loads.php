...
<table border="0" width="100%" cellpadding="4" cellspacing="0">
   <tr>
      ...
      <td>
	 <p align="left">
	    <font size="5" face="Verdana" color="#0066CC">Completed Load Requests
	    </font>
...
<?
  ...
  $stmt = "select to_char(req_date, 'MM/DD/YYYY HH24:MI:SS') req_date, to_char(create_date, 'MM/DD/YYYY HH24:MI:SS') create_date, to_char(sent_date, 'MM/DD/YYYY HH24:MI:SS') sent_date, load_num, status from eloads_picklist where status in ('" . fileRecv . "') and req_date > to_date('$start_date', 'MM/DD/YYYY') and req_date < to_date('$end_date', 'MM/DD/YYYY') order by req_date desc";
  ...
  while(ora_fetch_into($cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
    if($found_something == 0){
        $found_something = 1;
        ?>
        <br /><br /><b>Completed Loads (Click for details): </b><br />
        <a name="table">
        <table bgcolor="#f0f0f0" bordercolor="black" width="100%" border="2" cellpadding="4" cellspacing="1">
        <th>Re-Start</th><th>Load Number</th><th>Request Date</th><th>Creation Time<th>Sent Date</th><th>Status</th><th>Re-Print</th>
        <?
     }
     ...
     switch($status){
       case fileRecv:
         ...
         $def_string = "The Picklist has been received from Oppy.  Transaction Complete";
         break;
       case fileUnknown:
     ...
     printf("<tr bgcolor=\"$status_color\"><td align=\"center\"><a href=\"gen_load.php?load_num=$load_num\"><img src=\"images/re-send.gif\" border=\"0\" alt=\"Re-Send\"></a></td><td $on_click><font color=\"black\">$load_num</td><td $on_click>$request_date</td><td $on_click>$creation_date</td><td $on_click>$sent_date</td><td $on_click>$status_text</td></font><td align=\"center\"><a href=\"print_picklist.php?load_num=$load_num\"><img src=\"images/print-icon.gif\" border=\"0\" alt=\"Print\"></a></td></tr>\n"); 
  }
