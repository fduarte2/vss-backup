<?
   ...
   $stmt = "select ct.cargo_description, ct.warehouse_location, ct.arrival_num, ct.qty_received, ct.qty_damaged, ct.qty_in_house, ct.free_time_end, ct.billing_storage_date, to_char(date_received, 'MM/DD/YYYY HH24:MI:SS') date_received, cp.commodity_name, cust_p.customer_name from cargo_tracking ct, commodity_profile cp, customer_profile cust_p where pallet_id = '$pallet_id' and cp.commodity_code = ct.commodity_code and ct.receiver_id = cust_p.customer_id";
...
rowid";
   ...
   // Cargo Activity
   $stmt = "select ca.order_num, ca.qty_change, to_char(date_of_activity, 'MM/DD/YYYY HH24:MI:SS') date_of_activity, cp.customer_name, sc.service_name, per.login_id from cargo_activity ca, customer_profile cp, service_category sc, per_owner.personnel per where pallet_id = '$pallet_id' and ca.customer_id = cp.customer_id and ca.service_code = sc.service_code and ca.activity_id = per.employee_id order by activity_num";
   $ora_success = ora_parse($cursor, $stmt);
   $ora_success = ora_exec($cursor);
