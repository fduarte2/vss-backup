...
<?
				$sql = "SELECT TO_CHAR(DATE_OF_ACTIVITY, 'MM/DD/YYYY HH24:MI') THE_ACT, ORDER_NUM, CT.PALLET_ID, CARGO_DESCRIPTION, WEIGHT FROM CARGO_TRACKING CT, CARGO_ACTIVITY CA WHERE CT.PALLET_ID = CA.PALLET_ID AND CT.RECEIVER_ID = '312' AND CA.SERVICE_CODE ".$service_code_sql." AND (ACTIVITY_DESCRIPTION IS NULL OR ACTIVITY_DESCRIPTION LIKE 'DMG%') AND CUSTOMER_ID = '312' AND TO_CHAR(DATE_OF_ACTIVITY, 'MM/DD/YYYY') = '".$date_row['THE_DATE']."' AND ORDER_NUM = '".$order_row['ORDER_NUM']."' ORDER BY PALLET_ID";
