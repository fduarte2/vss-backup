  ...
  if($conn < 1){
   ...
   $tmp = array('b'=>'Helvetica-Bold.afm', 'i'=>'Helvetica-Oblique.afm', 'bi'=>'Helvetica-BoldOblique.afm', 'ib'=>'Helvetica-BoldOblique.afm');
	...
	if(!ora_fetch_into($Short_Term_Cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		...
		$sql = "SELECT * FROM WDI_OFFSITE_LOAD WHERE ORDER_NUM = '".$order."'";
		$ora_success = ora_parse($Short_Term_Cursor, $sql);
		$ora_success = ora_exec($Short_Term_Cursor, $sql);
		ora_fetch_into($Short_Term_Cursor, $short_term_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC);
		$send_to = $short_term_row['OFFSITE_DESTINATION'];
...
//		$pdf->ezSetDy(-3);
		...
		$pdf->ezSetDy(37);
		if($send_to != "LUCCA"){
			$pdf->ezText("Ship To:  Gloucester Terminals, LLC", 10, $right);
			$pdf->ezText("Foot of King and Essex Streets", 10, $right);
			$pdf->ezText("Gloucester City, NJ 08030", 10, $right);
		} else {
		...
		$sql = "SELECT CA.PALLET_ID, NVL(WIM.ITEM_DESCRIPTION, 'UNKNOWN') THE_DESC, DSP.CARGO_SIZE, 
					QTY_CHANGE, EXPORTER_CODE, MARK, NVL(TO_CHAR(WM_ITEM_NUM), 'UNKNOWN') MASTER_NUM,
					DSP.BATCH_ID GROWERNUM, WIM.GROWER GROWERNAME, DSP.COMMODITY_CODE
				FROM
				CARGO_ACTIVITY CA,
		...
		while(ora_fetch_into($cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
			$print_methyl = print_footer($print_methyl, $row['COMMODITY_CODE'], $conn);
...
//										'grower'=>$row['SUB_CUSTID'],
	...
	if($email == true){
		...
		$mailSubject = str_replace("_1_", date('m/d/Y'), $mailSubject);
...
function print_footer($print_flag, $commodity, $conn){
	if($print_flag == true){
		return true;
	}
	...
	$sql = "SELECT NVL(COMMODITY_TYPE, 'NONE') THE_COMM from COMMODITY_PROFILE WHERE COMMODITY_CODE = '".$commodity."'";
	$ora_success = ora_parse($Short_Term_Cursor, $sql);
	$ora_success = ora_exec($Short_Term_Cursor, $sql);
	ora_fetch_into($Short_Term_Cursor, $short_term_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC);
	if($short_term_row['THE_COMM'] == "CHILEAN"){
		return true;
	} else {
		return false;
	}
