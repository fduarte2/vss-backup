...
*************************************************************/
  ...
  if($user != "martym" && $user != "ltreut" && $user != "ltreut1"){
    printf("This page is only authorized to be used by Martin McLaughlin / Lisa Treut");
    include("pow_footer.php");
    exit;
  }
  ...
  $cursor = ora_open($conn);
	...
	if($submit == "Remove Vessel" && $vessel != ""){
		...
		$sql = "DELETE FROM VOYAGE_CARGO WHERE LR_NUM = '".$vessel."'";
		ora_parse($cursor, $sql);
		ora_exec($cursor);
	}
