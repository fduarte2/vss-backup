...
<?
		...
		$sql = "SELECT PALLET_ID, CUSTOMER_ID, BOL, CARGO_DESCRIPTION MARK, COMMODITY_NAME, NVL(TO_CHAR(DATE_RECEIVED, 'MM/DD/YYYY HH24:MI:SS'), 'NR') THE_REC, QTY_RECEIVED, CUSTOMER_NAME FROM CARGO_TRACKING CT, COMMODITY_PROFILE CP, CUSTOMER_PROFILE CUSTP WHERE ARRIVAL_NUM = '".$vessel."'";
		if($cust != "ALL"){
		...
		$sql .= " AND CT.COMMODITY_CODE = CP.COMMODITY_CODE AND CT.RECEIVER_ID = CUSTP.CUSTOMER_ID AND CT.COMMODITY_CODE LIKE '503%' ORDER BY CUSTOMER_ID, BOL, MARK, PALLET_ID";
		...
		while(ora_fetch_into($cursor_first, $first_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
			if($total_pallets == 0){
				// initialize BOL for subtotal loop.
...
<tr>
	...
	<td colspan="2"><font size="2" face="Verdana">Plt: <? echo $sub_total_pallets; ?></font></td>
...
<?
			...
			$sql = "SELECT COUNT(*) THE_COUNT FROM CARGO_ACTIVITY WHERE ARRIVAL_NUM = '".$vessel."' AND PALLET_ID = '".$first_row['PALLET_ID']."' AND SERVICE_CODE NOT IN ('1', '2', '8', '9', '12', '16') AND (ACTIVITY_DESCRIPTION != 'VOID' OR ACTIVITY_DESCRIPTION IS NULL)";
...
?>
	<tr>
		...
		<td <? if ($first_row['THE_REC'] == "NR") {?> bgcolor="#33CC00" <?} else {?> bgcolor="#FFFFFF" <?}?>><font size="2" face="Verdana"><? echo $first_row['THE_REC']; ?></font></td>
		<td rowspan="<? echo $second_row['THE_COUNT']; ?>"><font size="2" face="Verdana"><? echo $first_row['QTY_RECEIVED']; ?></font></td>
<?
				...
				$sql = "SELECT ACTIVITY_NUM, QTY_CHANGE, TO_CHAR(DATE_OF_ACTIVITY, 'MM/DD/YYYY HH24:MI:SS') THE_ACT, UPPER(SERVICE_NAME) THE_SERV, NVL(ORDER_NUM, 'N/A') THE_ORD, CUSTOMER_ID FROM CARGO_ACTIVITY CA, SERVICE_CATEGORY SC WHERE ARRIVAL_NUM = '".$vessel."' AND PALLET_ID = '".$first_row['PALLET_ID']."' AND CA.SERVICE_CODE NOT IN ('1', '2', '8', '9', '12', '16') AND (ACTIVITY_DESCRIPTION != 'VOID' OR ACTIVITY_DESCRIPTION IS NULL) AND CA.SERVICE_CODE = SC.SERVICE_CODE ORDER BY ACTIVITY_NUM";
...
<?
				...
				$sql = "SELECT MAX(DATE_OF_ACTIVITY), SERVICE_CODE FROM CARGO_ACTIVITY WHERE PALLET_ID = '".$first_row['PALLET_ID']."' AND CUSTOMER_ID = '".$first_row['CUSTOMER_ID']."' AND ARRIVAL_NUM = '".$vessel."' AND SERVICE_CODE IN ('6', '7', '11', '13') AND (ACTIVITY_DESCRIPTION != 'VOID' OR ACTIVITY_DESCRIPTION IS NULL) GROUP BY SERVICE_CODE ORDER BY MAX(DATE_OF_ACTIVITY) DESC";
				ora_parse($Short_Term_Cursor, $sql);
				ora_exec($Short_Term_Cursor);
