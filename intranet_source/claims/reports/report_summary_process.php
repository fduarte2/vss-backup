...
if ($start_date != "") {
  if (strpos($stmt, "where") === false) {
    ...
    $stmt = $stmt . " and claim_date >= '$start_date'";
  } 
}
...
if ($end_date != "") {
  if (strpos($stmt, "where") === false) {
    ...
    $stmt = $stmt . " and claim_date <= '$end_date'";
  } 
}
...
$stmt .= " group by customer_invoice_num, exporter, bni_bl, voyage, rf_pallet_id, product_name 
...
$stmt = "select customer_name from customer_profile where customer_id = '$customer_id'";
...
if ($pg_rows > 0) {
  ...
  for ($i=0; $i<$pg_rows; $i++) {
    ...
    $invoice = $row['customer_invoice_num'];
    $exporter = strtoupper($row['exporter']);
    $bl = $row['bni_bl'];
    $voy = $row['voyage'];
    $pId = $row['rf_pallet_id'];
...
pg_close($pg_conn);
   ...
   $pdf->ezSetDy(-5);
   $pdf->ezText("<b>Season: 2004</b>", 10, $left);
   ...
   $pdf->ezSetDy(-15);
   $pdf->ezText("<b>2004 Claim Submission                     $tot_submission</b>", 10, $left);
   $pdf->ezSetDy(-5);
