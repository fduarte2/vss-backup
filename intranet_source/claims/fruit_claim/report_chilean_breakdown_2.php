...
while(list($cust_id, $cust_name)=each($cust)){
  $sql = "select a.submission, b.denial, b.shipper, b.pow, c.acceptance
          ...
          from claim_header_rf h, claim_body_rf b
          where h.claim_id = b.claim_id and customer_id = $cust_id and h.season='$season' and b.status = 'C'
  ...
  while (ora_fetch($cursor)){
	...
	$accepted = $submission - $denial;
	$tot_accepted += $accepted;

        if ($accepted > 0){
                $dis_accepted = '$'.number_format($accepted, 2, '.',',');
        }else{
                $dis_accepted = "-";
        }
	if ($submission > 0){
   ...
   array_push($data, array('cust'=>'<b>Total</b>',
                           'sub'=>'<b>$'.number_format($tot_accepted, 2, '.',',').'</b>',
...
ora_logoff($conn);
   ...
   $arrHeading = array( 'cust'=>'<b>Customer</b>',
			'sub'=>'<b>Claim accepted</b>',
		//	'denial'=>'<b>Denial</b>',
			'shipper'=>'<b>PSW account</b>', 
			'pow'=>'<b>POW account</b>',
		//	'accp'=>'<b>POW account</b>',
			'sixty_eight'=>'<b>68%</b>');
   ...
   $pdf->ezSetDy(-20);
   $pdf->ezText("<b>$season Chilean Claim Report</b>", 14, $center);
   $pdf->ezSetDy(-5);
   ...
   $pdf->ezTable($heading, $arrHeading, '', array('showHeadings'=>0, 'shaded'=>0, 'showLines'=>2, 'fontSize'=>9, 'width'=>550,'cols'=>$arrCol));
   $pdf->selectFont('/usr/lib/php/Pdf/fonts/Helvetica.afm');
   $pdf->ezTable($data, $arrHeading, '', array('showHeadings'=>0, 'shaded'=>0, 'showLines'=>2,'fontSize'=>8,'width'=>550,'cols'=>$arrCol));
