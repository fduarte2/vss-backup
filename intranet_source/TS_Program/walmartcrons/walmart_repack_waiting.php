...
//			AND DATE_RECEIVED IS NOT NULL
	$sql = "SELECT NVL(VESSEL_NAME, 'TRUCK') THE_VES, CT.PALLET_ID, MAX(ORDER_NUM) THE_ORD
			FROM CARGO_TRACKING CT, VESSEL_PROFILE VP, CARGO_ACTIVITY CA, WDI_VESSEL_RELEASE WVR, CARGO_TRACKING_ADDITIONAL_DATA CTAD
			...
			GROUP BY NVL(VESSEL_NAME, 'TRUCK'), CT.PALLET_ID
			ORDER BY THE_VES, THE_ORD, CT.PALLET_ID";
	...
	if(!ora_fetch_into($cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		...
		do {
			$body_string .= $row['PALLET_ID']." (".$row['THE_VES']." - ".$row['THE_ORD'].")\r\n";
		} while(ora_fetch_into($cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC));

		$sql = "SELECT * FROM EMAIL_DISTRIBUTION
				WHERE EMAILID = 'AQCRPWDI'";
