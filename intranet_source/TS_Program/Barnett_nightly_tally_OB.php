...
//	$mailheaders .= "Bcc: " . "awalter@port.state.de.us,lstewart@port.state.de.us,sadu@port.state.de.us,hdadmin@port.state.de.us,martym@port.state.de.us,ltreut@port.state.de.us\r\n";
	...
	$sql = "SELECT DISTINCT ORDER_NUM FROM CARGO_ACTIVITY 
			WHERE TO_CHAR(DATE_OF_ACTIVITY, 'MM/DD/YYYY') = '".$date."' 
			AND SERVICE_CODE = '6'
			AND ACTIVITY_DESCRIPTION IS NULL
			AND PALLET_ID IN
				(SELECT PALLET_ID FROM CARGO_TRACKING WHERE REMARK = 'BOOKINGSYSTEM')
			ORDER BY ORDER_NUM";
	...
	if(!ora_fetch_into($select_cursor1, $select_row1, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		mail($mailTO, $mailsubject, "No Outbound Orders Today", $mailHeaders);
	} else {
		do {
			$total_rolls = 0;
			...
			$mailsubject = "Barnett Outbound Tally ".$date." on Container: ".$container_id."\r\n";
			$xls_file = "<TABLE border=0 CELLSPACING=1>";
	
			$xls_file .= "<tr><td>&nbsp;</td><td>&nbsp;</td><td colspan=8>PORT OF WILMINGTON TALLY - COMMERCIAL PAPER (Outbound)</td><td>&nbsp;</td><td>&nbsp;</td></tr>";
			...
			$sql = "SELECT TO_CHAR(MIN(DATE_OF_ACTIVITY), 'MM/DD/YYYY HH:MI AM') START_TIME, 
					TO_CHAR(MAX(DATE_OF_ACTIVITY), 'MM/DD/YYYY HH:MI AM') END_TIME
					FROM CARGO_ACTIVITY 
					WHERE ORDER_NUM = '".$select_row1['ORDER_NUM']."'
					AND SERVICE_CODE = '6'
					AND ACTIVITY_DESCRIPTION IS NULL";
			ora_parse($Short_Term_Cursor, $sql);
			ora_exec($Short_Term_Cursor);
			...
			$xls_file .= "<tr><td>&nbsp;</td><td colspan=6>ORDER ".$select_row1['ORDER_NUM']."</td><td colspan=4>END TIME: ".$EndTime."</td><td>&nbsp;</td><td>&nbsp;</td></tr>";
			$xls_file .= "<tr><td>&nbsp;</td><td colspan=6>CONTAINER: ".$container_id."</td></tr>";
			...
			$sql = "SELECT CT.PALLET_ID, Round(BAD.WIDTH * UC1.CONVERSION_FACTOR, 1) THE_WIDTH, NVL(PERS.LOGIN_ID, 'N/A') THE_CHECKER,
					...
					FROM BOOKING_ADDITIONAL_DATA BAD, CARGO_TRACKING CT, CARGO_ACTIVITY CA, BOOKING_PAPER_GRADE_CODE BPGC, 
						UNIT_CONVERSION_FROM_BNI UC1, 
						UNIT_CONVERSION_FROM_BNI UC2, UNIT_CONVERSION_FROM_BNI UC3, UNIT_CONVERSION_FROM_BNI UC4,
						PER_OWNER.PERSONNEL PERS
					WHERE CA.ORDER_NUM = '".$select_row1['ORDER_NUM']."' 
					...
					ORDER BY NVL(BOOKING_NUM, '--NONE--'), CT.PALLET_ID, BAD.ORDER_NUM";
...
//			$xls_file .= "<tr><td colspan=12>&nbsp;</td></tr>";
			...
			$Content.="--MIME_BOUNDRY\r\n";
			$Content.="Content-Type: application/pdf; name=\"BarnettTallyOut".date('mdy').$select_row1['ORDER_NUM'].".xls\"\r\n";
