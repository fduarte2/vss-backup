...
//		printf(ora_errorcode($conn));
	...
	$mailSubject = "NEW BEDFORD MA CBP AG Seal Compliance Report";
...
//				AND GATEPASS_PDF_DATE <= (SYSDATE - 30/1440)
	$sql = "SELECT DISTINCT
			...
			WHERE
				...
				AND voyage.port_of_destination = 'NEW BEDFORD MA'";
	$border_data = ociparse($rfconn, $sql);
	ociexecute($border_data);
	if(!ocifetch($border_data)){
		// nothing to do, notify, end script
		mail("", $mailSubject, "For your information only: No trucks were loaded prior to the time this automatic email was generated.", $mailheaders);
		exit;
	} else { do {
		...
		$mailTO = str_replace("-", "", ociresult($short_term_data, "FAX_NUMBER"))."@rcfax.com\r\n";
...
//		echo $sql."\n";
		...
		if(!ocifetch($arv_data)){
			...
			$pdf->ezSetY(555);
			$pdf->ezText("<b>NEW BEDFORD MA</b>", 12, $left);
			$pdf->ezSetY(555);
