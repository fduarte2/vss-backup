<?
   ...
   $arrHeading = array('comm'=>'<b>Program</b>', 'service'=>'<b>Service</b>', 'aHours'=>'<b>Hours</b>', 'aPlt'=>'<b>Plts</b>', 'tons'=>'<b>Tons</b>','budget'=>'<b>Budget</b>','aProd'=>'Productivity(T/H)', 'result'=>'<b>Variance</b>');
 ...
 //                  'plt'=>array('width'=>45, 'justification'=>'center'),
                //   'lab'=>array('width'=>45, 'justification'=>'center'),
                   'aHours'=>array('width'=>50, 'justification'=>'center'),
                   'aPlt'=>array('width'=>50, 'justification'=>'center'),
                   'tons'=>array('width'=>50, 'justification'=>'center'),
		   'aProd'=>array('width'=>100, 'justification'=>'center'),
   ...
   while (ora_fetch($cursor_bni2)){
	...
	if ($vessels <>""){
        	$SQL[0]="select a.*, b.budget from (
                	select c.itg, 'BACKHAUL' as service_type, '', sum(hours), sum(qty),sum(tonnage),''
                	from $productivity p, commodity_itg c, itg i
                	where p.commodity = c.commodity_code and c.itg = '$itg' and
                	service_type = 'BACKHAUL' and p.vessel in ($vessels)  and c.itg <>'PacSea' and 
			c.itg = i.itg(+) and (i.itg_group <> 2 or c.itg = 'Chilean')
                	group by c.itg) a, itg_budget b
                	where a.service_type = b.type(+) and a.itg = b.itg(+)
                 	order by a.itg";
	}
        $SQL[1]="select a.*, b.budget from (
                select  c.itg, service_type, '', sum(hours), sum(qty),sum(tonnage), s.id
                from $productivity p, service_type s, commodity_itg c, itg i
                where s.type = p.service_type and p.commodity = c.commodity_code and c.itg = '$itg' and
                s.id in (1,3,4,5,6) and c.itg = i.itg(+) and i.itg_group <> 2 and 
        ...
        $SQL[2]="select c.itg, 'Other','', sum(hours), sum(qty), sum(tonnage),'',''
                from $productivity p, service_type s, commodity_itg c, itg i
                where s.type = p.service_type and p.commodity = c.commodity_code and c.itg = '$itg' and
		s.id in (7,8,9) and c.itg = i.itg(+) and i.itg_group <> 2 and
                date_time >= to_date('$sDate','mm/dd/yyyy') and date_time <= to_date('$eDate','mm/dd/yyyy')
	...
	for($i = $start; $i<3; $i++){
			...
			$lr_num = ora_getcolumn($cursor_bni, 2);
...
*/
			...
			if ($prods == "0.00"){
                                $result = "";
			}else if (is_numeric($budget) && is_numeric($prods)){
...
//   $pdf->ezTable($heading1, $arrHeading1, '', array('showHeadings'=>0, 'shaded'=>0, 'showLines'=>2, 'width'=>510,'cols'=>$arrCol1));
   ...
   if ($eTot_aHours > 0){
        $pdf->ezSetDy(-15);
        $pdf->ezText("<b><i>*$dis_pro hours</i></b>", 10, $left);
   }
...
*/
   $pdf->ezSetDy(-15);
   $pdf->ezText("<b><i>* Service Other (e.g. maintenance, non-revenue, stand by, etc) does not have productivity measures.</i></b>", 10, $left);
...
//        $mailTo1 .= ",ithomas@port.state.de.us";
        ...
        $mailheaders .= "Cc: wstans@port.state.de.us,jjaffe@port.state.de.us,lstewart@port.state.de.us,hdadmin@port.state.de.us,";
