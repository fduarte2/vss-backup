...
//   $tmp = array('b'=>'Helvetica-Bold.afm', 'i'=>'Helvetica-Oblique.afm', 'bi'=>'Helvetica-BoldOblique.afm', 'ib'=>'Helvetica-BoldOblique.afm','font'=>'Symbol.afm');
   ...
   $pdf->ezSetDy(-10);
   $pdf->ezText("<b><i>Week of $sDate </i></b>", 18, $center);
   $pdf->ezSetDy(-5);
   ...
   $sql = "select distinct service_type, id from $productivity p, service_type s
	   ...
	   order by id";
   $sql = "select distinct itg from $productivity p, commodity_itg 
	   where p.commodity = c.commodity_code and itg not in ('Dole', 'Dole Container Fruit', 'Dole Fresh', 'Vehicles')
   ...
   while (ora_fetch($cursor_bni2)){
	$itg = ora_getcolumn($cursor_bni2, 0);	
   	...
   	$sql = "select a.*, b.budget from (
		...
		s.id in (1,2,4,5,6) and
		date_time >= to_date('$sDate','mm/dd/yyyy') and
		date_time <= to_date('$eDate','mm/dd/yyyy') 
   	...
   	while(ora_fetch($cursor_bni)){
		...
		}else if ($tons > 0 && $aHours ==0){
			if ($no_hours ==""){
                                $no_hours = $itg."-".$service;
                        }else if ($dis_itg <> ""){
                                $no_hours .= "; ".$dis_itg."-".$service;
                        }else{
                                $no_hours .= ", ".$service;
                        }
		}else if ($tons ==0 && $aHours > 0){
   ...
   /*
        if ($count > 1){
        	array_push($data, array('service'=>'<b><i>Sub Total</i></b>',
                                	'comm'=>'',
                                	'num'=>'',
