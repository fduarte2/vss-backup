  ...
  if($conn < 1){
   ...
   $tmp = array('b'=>'Helvetica-Bold.afm', 'i'=>'Helvetica-Oblique.afm', 'bi'=>'Helvetica-BoldOblique.afm', 'ib'=>'Helvetica-BoldOblique.afm');
	...
	if(!ora_fetch_into($cursor_cust, $cust_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		$pdf->ezText("<b>No dock returns found for given date.</b>", 14, $center);
	} else {
		do{
			if($cust_pallets > 0){
				$pdf->ezNewPage();
			}
			$pdf->ezText("<b>Daily Dock Return Summary Report</b>", 14, $left);
			$pdf->ezText("<b>Date of Dock Return: ".$the_date."</b>", 14, $left);
...
//					echo $sql;
			...
			$pdf->ezSetDy(8);
			$pdf->ezText("<b>____________________________________________________________________________________________</b>", 10, $left);
			$pdf->ezText("<b>Customer Total for ".substr($cust_row['CUSTOMER_NAME'], 0, 20)."                                 Plts = ".$cust_pallets."      Cases = ".$cust_cases."</b>", 12, $left);
