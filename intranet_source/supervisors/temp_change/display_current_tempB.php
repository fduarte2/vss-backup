		...
		$quality = $status[$whse][$box];
			...
			// Leave Color as Light Green and get out 
				...
				if ($quality == "Bad") {
					...
					if ( ($temp - $set_point) * ($temp - $set_point) > $prodTemp[$prod][HIGH_WARNING] * 
$prodTemp[$prod][HIGH_WARNING] ) {
			     			$bgcolor = "bgcolor=red";
		        		}
					else 
						if (($temp - $set_point) * ($temp - $set_point) > $prodTemp[$prod][HIGH_ALERT]) * 
$prodTemp[$prod][HIGH_ALERT]) {
