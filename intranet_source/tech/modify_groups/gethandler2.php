...
$sql = "select * from CUSTOMER_GROUP_MEMBER WHERE GROUP_ID = " . $_REQUEST['group_id'];

//$sql = "select GROUP_ID, CUSTOMER_ID from CUSTOMER_GROUP_MEMBER WHERE GROUP_ID = 1";
...
$getHandler->DefineField("CUSTOMER_ID");
...
{
	$getHandler->CreateNewRecord($results[$i]['GROUP_ID'] . $results[$i]['CUSTOMER_ID']);
	$getHandler->DefineRecordFieldValue("GROUP_ID", $results[$i]['GROUP_ID']);	// set ID
        $getHandler->DefineRecordFieldValue("CUSTOMER_ID", $results[$i]['CUSTOMER_ID']);      // customer_ID
