...
<head>
<title>phpScheduleIt Setup Program</title>
...
function doPrintForm() {
	global $conf;
?>
<h3 align="center">Please log into your database</h3>
<form name="login" id="login" method="post" action="<?=$_SERVER['PHP_SELF']?>">
  <table width="60%" border="0" cellspacing="3" cellpadding="0" align="center" style="border: solid 1px #333333; background-color: #fafafa;">
    <tr>
      <td>Enter database root username:</td>
      <td><input type="text" name="user" class="textbox" /></td>
    </tr>
    <tr>
      <td>Enter database root password:</td>
      <td><input type="password" name="password" class="textbox" /></td>
    </tr>
    <tr>
      <td><input type="submit" name="login" value="Login to database" class="button" /></td>
    </tr>
  </table>
  <br />
  <table width="80%" align="center" cellpadding="3" cellspacing="0" border="0" style="font-family: Verdana, Arial; font-size: 12px; background-color: #ffffff; border: solid 1px #DDDDDD">
    <tr>
      <td>
	  <ul>
	  <li>Root user is <b>not</b> required. Any database user who has permission to create tables is acceptable.</li>
	  <li>This will set up all the necessary databases and tables for phpScheduleIt.</li>
	  <li>It also populates any required tables.</li>
	  <? if ($conf['db']['drop_old']) echo '<li>Warning: THIS WILL ERASE ALL DATA IN PREVIOUS phpScheduleIt DATABASES!</li>';?>
	  </ul></td>
    </tr>
  </table>
</form>
...
function checkConfig() {
	...
	switch ($conf['db']['dbType']) {	// Check database type
		...
		case 'fbsql' :;
			break;
		default :
			echo 'Not a valid database type in the config.php file.';
			return false;
			break;
	}

	if (empty($conf['db']['dbUser'])) {		// Check database user
		echo 'Database user is not set in the config.php file.';
		return false;
	}

	if (empty($conf['db']['dbPass'])) {		// Check database password
		echo 'Database user password is not set in the config.php file.';
		return false;
	}

	if (empty($conf['db']['dbName'])) {		// Check database name
		echo 'Database name not set in the config.php file.';
		return false;
	}
...
function doLogin() {
    ...
    if (DB::isError($db)) {
		...
		echo '<h4 align="center">Successfully connected as ' . $_SESSION['user'] . "</h4>\n"
			. "<form name=\"create\" id=\"create\" method=\"post\" action=\"{$_SERVER['PHP_SELF']}\">\n"
			. "<input type=\"submit\" name=\"create\" value=\"Create tables &gt;\" class=\"button\" />\n"
			. "</form>\n";
	}
}
...
function doCreate() {
	...
	$sqls = array (
					...
					array ('create table reservations (
							...
							modified integer,
							parentid char(16),
							is_blackout integer(1) not null default 0,
							summary text
...
function doFinish() {
	?>
	Congratulations! You have finished setting up phpScheduleIt and are ready to begin
	using it.<br /><br />
