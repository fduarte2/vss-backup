...
<p style="font-weight: bold; text-align: right;">View stats for schedule:</p>
</td>
<td align="left">
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <tr>
    <td class="tableTitle" colspan="2">At A Glance</td>
  </tr>
  <tr class="cellColor<?=($color++%2)?>">
    <td width="175">Total Users:</td>
  ...
  <tr class="cellColor<?=($color++%2)?>">
    <td>Max Reservation:</td>
    <td><?=CmnFns::minutes_to_hours($stats->longest)?>
    </td>
  </tr>
  <tr class="cellColor<?=($color++%2)?>">
    <td>Min Reservation:</td>
    <td><?=CmnFns::minutes_to_hours($stats->shortest)?>
    </td>
  </tr>
  <tr class="cellColor<?=($color++%2)?>">
    <td>Avg Reservation:</td>
    <td><?=@CmnFns::minutes_to_hours($stats->get_total_time()/$stats->get_num_res())?>
    </td>
  </tr>
  <tr><td height="1" bgcolor="#666666" colspan="2"></td></tr>
  <tr class="cellColor<?=($color++%2)?>">
    <td>Most Active Resource:</td>
    <td><?=$stats->active_resource['name'] . ' : ' . $stats->active_resource['num'] . ' reservations'?>
    </td>
  </tr>
  <tr class="cellColor<?=($color++%2)?>">
    <td>Most Active User:</td>
    <td><?=$stats->active_user['name'] . ' : ' . $stats->active_user['num'] . ' reservations'?>
    </td>
  </tr>
</table>
<?
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <tr>
    <td class="tableTitle" colspan="2">System Stats</td>
  </tr>
  <tr class="cellColor<?=($color++%2)?>">
    <td width="175">phpScheduleIt version:</td>
    <td><?=$conf['app']['version']?>
    </td>
  </tr>
  <tr class="cellColor<?=($color++%2)?>">
  ...
  <tr><td height="1" bgcolor="#666666" colspan="2"></td></tr>
  <tr class="cellColor<?=($color++%2)?>">
    <td>phpScheduleIt root directory:</td>
    <td><?=$conf['app']['weburi']?>
    </td>
  </tr>
  <tr class="cellColor<?=($color++%2)?>">
    <td>Using permissions:</td>
    <td><?=($conf['app']['use_perms']) ? 'Yes' : 'No'?>
    </td>
  </tr>
  <tr class="cellColor<?=($color++%2)?>">
    <td>Using logging:</td>
    <td><?=($conf['app']['use_log']) ? 'Yes' : 'No'?>
    </td>
  </tr>
  <tr class="cellColor<?=($color++%2)?>">
    <td>Log file:</td>
...
function print_stats(&$stats, $index = NULL) {
	...
	echo '<tr><td style="text-align: right;" colspan="3">Reservations 0</td>'
		...
		. '</td></tr>';
	echo '</table>';
	echo '<p style="text-align: center;"><a href="#top">Return to top</a></p>';
}
?>