...
<p style="font-weight: bold; text-align: right;">View schedule:</p>
</td>
<td align="left">
...
<table align="center" cellpadding="5" cellspacing="10">
  <tr style="font-size: 10px; font-weight: bold; text-align: center; vertical-align: center;">
    <td style="width: 75px; height: 38px; background-color:#<?=$conf['ui']['my_res'][0]['color']?>; border: 2px #000000 solid;">My Reservations</td>
	<td style="width: 75px; height: 38px; background-color:#<?=$conf['ui']['my_past_res'][0]['color']?>; border: 2px #000000 solid;">My Past Reservations</td>
	<td style="width: 75px; height: 38px; background-color:#<?=$conf['ui']['other_res'][0]['color']?>; border: 2px #000000 solid;">Other Reservations</td>	
	<td style="width: 75px; height: 38px; background-color:#<?=$conf['ui']['other_past_res'][0]['color']?>; border: 2px #000000 solid;">Other Past Reservations</td>
  	<td style="width: 75px; height: 38px; background-color:#<?=$conf['ui']['blackout'][0]['color']?>; border: 2px #000000 solid; color: #CCCCCC;">Blacked Out Time</td>
  </tr>
</table>
<?
}
...
function print_name_cell($ts, $id, $name, $shown, $is_blackout, $scheduleid) {
	...
	if ($is_blackout) {
		$link->doLink("javascript: blackout('r', '$id','$ts', '', '$scheduleid');", $name, '', '', "Set blackout times for $name on " . date('M d, Y', $ts));
	}
	else {
		...
		// Else do not
		if ($shown)	
			$link->doLink("javascript: reserve('r','$id','$ts','', '$scheduleid');", $name, '', '', "Reserve $name on " . date('M d, Y', $ts));
		else
...
?>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="5" align="center">
     <tr>
	  <td align="center"><h5><? $link->doLink($_SERVER['PHP_SELF'] . '?date=' . date('m-d-Y',mktime(0,0,0,$m, $d - 7, $y)) . "&amp;$scheduleid", '&laquo; Prev Week', '', '', 'Jump 1 week back') ?></h5></td>
      <? if ($printAllCols) { ?>
	  <td align="center"><h5><? $link->doLink($_SERVER['PHP_SELF'] . '?date=' . date('m-d-Y',mktime(0,0,0,$m, $d - $viewDays, $y)) . "&amp;$scheduleid", "&#8249; Prev $viewDays days", '', '', "Previous $viewDays days") ?></h5></td>
      <? } ?>
	  <td align="center"><h5><? $link->doLink($_SERVER['PHP_SELF'] . "?$scheduleid", 'This Week', '', '', 'Jump to this week') ?></h5></td>
      <? if ($printAllCols) { ?>
	  <td align="center"><h5><? $link->doLink($_SERVER['PHP_SELF'] . '?date=' . date('m-d-Y',mktime(0,0,0,$m, $d + $viewDays, $y)) . "&amp;$scheduleid", "Next $viewDays days &#8250;", '', '', "Next $viewDays days") ?></h5></td>
      <? } ?>
	  <td align="center"><h5><? $link->doLink($_SERVER['PHP_SELF'] . '?date=' . date('m-d-Y',mktime(0,0,0,$m, $d + 7, $y)) . "&amp;$scheduleid", 'Next Week &raquo;', '', '', 'Jump 1 week ahead') ?></h5></td>
	 </tr>
     <tr>
      <td align="center" colspan="<? echo ($printAllCols) ? '5' : '3'?>">
       <form name="jumpWeek" method="post" action="<?= $_SERVER['PHP_SELF'] . '?' . $scheduleid ?>" onsubmit="return checkDate();">
         <input type="text" name="jumpMonth" value="mm" class="textbox" size="3" maxlength="2" onclick="this.value = '';" />
         <input type="text" name="jumpDay" value="dd" class="textbox" size="3" maxlength="2" onclick="this.value = '';" />
         <input type="text" name="jumpYear" value="yyyy" class="textbox" size="5" maxlength="4" onclick="this.value = '';" />
         <input name="jumpForm" type="submit" value="Jump To Date" class="button" />
       </form>
      </td>
     </tr>
    </table>	
	<h5 align="center"><? $link->doLink("javascript: window.open('popCalendar.php','calendar','width=260,height=250,scrollbars=no,location=no,menubar=no,toolbar=no,resizable=yes'); void(0);", 'View Monthly Calendar', '', '', 'Open up a navigational calendar')?></h5>	
<?
}
?>