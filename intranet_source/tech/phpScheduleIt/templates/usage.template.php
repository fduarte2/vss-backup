    ...
    <form name="searchForm" method="post" action="<?=$_SERVER['PHP_SELF']?>">
      <table width="100%" border="0" cellspacing="0" cellpadding="1" align="center">
		...
		<table width="100%" border="0" cellspacing="1" cellpadding="0">
          <tr> 
            <td colspan="2" class="tableTitle">&#8250; Select Search Criteria</td>
          </tr>
		  <tr class="cellColor"> 
            <td width="15%" class="formNames">Schedules:</td>
            <td> 
              <select name="scheduleid[]" size="4" multiple="multiple" class="textbox">
                <option selected="selected" value="all">All Schedules</option>
                <?
                // Write out all users
              ...
              <br />Hold CTRL to select more than one user.
            </td>
          </tr>
          <tr class="cellColor"> 
            <td width="15%" class="formNames">Users:</td>
            <td> 
          ...
          <tr class="cellColor"> 
            <td class="formNames">Resources:</td>
            <td> 
              <select name="machid[]" size="4" multiple="multiple" class="textbox">
			    <option selected="selected" value="all">All Resources</option>
                <?
              ...
              <br />Hold CTRL to select more than one resource.
            </td>
          </tr>
          <tr class="cellColor"> 
            <td class="formNames">Starting Date:</td>
            <td> 
          ...
          <tr class="cellColor"> 
            <td class="formNames">Ending Date:</td>
            <td> 
          ...
          <tr class="cellColor"> 
            <td class="formNames">Starting Time:</td>
            <td> 
          ...
          <tr class="cellColor"> 
            <td class="formNames">Ending Time:</td>
            <td> 
		  ...
		  <tr class="cellColor">
		    <td class="formNames">Output Type:</td>
			<td>
...
function printUserInfo($fname, $lname, $memberid, $type) {  
    ...
    switch ($type) {
		case 'html' :
    		echo '<h4 align="center">' . $link->getLink("javascript: viewUser('" . $memberid . "');", $fname . ' ' . $lname, '', '', 'View user information');
			break;
		case 'text' :
     ...
     <tr><td class="tableBorder">  
      <table width="100%" border="0" cellspacing="1" cellpadding="0">    
	   ...
	   <tr class="rowHeaders" align="center">
        <td width="5%">#</td>
        <td width="11%">Date</td>  
        <td width="22%">Created</td>  
        <td width="22%">Modified</td>       
...
<?
			...
			echo "$fname $lname - $name ($schedule)\n"
				. "#\tDate\t\tCreated\t\t\t\tModified\t\t\tStart Time\tEnd Time\tTotal Time\n";
	}
...
function printTableFooter($hours, $type, $percent) {
    ...
    <tr class="cellColor">
	<td colspan="8">
	<span style="font-weight: bold; margin-right: 15px;">Total hours: <?= $hours ?></span>
	<?=$percent . '% of total resource time'?></td>
    </tr>
   </table>
  </td>
 </tr>
...
<?
			...
			echo $GLOBALS['sglStr']
				. "Total hours: $hours\n"
				. "Percent of resource time: $percent%\n"
				. $GLOBALS['dblStr'] . "\n";
	}
	// No implementation for xml or csv
}
	...
	switch ($type) {
			case 'html' :
				...
				echo '<tr class="cellColor" align="center" style="font-weight: normal;">'
					...
					. '<td>' . $startTime . "</td>\n"
					. '<td>' . $endTime . "</td>\n"
					. '<td>' . $link->getLink("javascript: reserve('m','','','" . $resid. "');", 'Edit', '', '', 'Edit this reservation') . "</td>\n"
					. '<td>' . $totTime . "</td>\n</tr>\n";
				break;
        	case 'text' :
...
function print_change_output($form, $obj_name = '') {
	...
	echo '<span style="font-size: 11px;">View these results as: </span><select name="outputtype" onchange="javascript: document.jump_output.submit();" class="textbox">'
		. '<option value="html"' . (($form['outputtype'] == 'html') ? ' selected="selected"' : '') . '>HTML</option>'
		. '<option value="text"' . (($form['outputtype'] == 'text') ? ' selected="selected"' : '') . '>Text</option>'
		. '<option value="xml"' . (($form['outputtype'] == 'xml') ? ' selected="selected"' : '') . '>XML</option>'
		. '<option value="csv"' . (($form['outputtype'] == 'csv') ? ' selected="selected"' : '') . '>CSV</option>'
		. '</select>';
	echo '</form></div>';
