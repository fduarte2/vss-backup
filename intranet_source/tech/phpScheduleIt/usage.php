...
if (!Auth::isAdmin()) {
	CmnFns::do_error_box('This is only accessable to the administrator');
}

$t = new Template(isset($_POST['search']) ? 'Search Results' : 'Search Resource Usage');
...
function search($type) {
    ...
    echo "<h3 align=\"center\">Search Results: $recs reservations found</h3>\n"
    	. "<h5 align=\"center\">" . $link->getLink($_SERVER['PHP_SELF'], 'Try a different search') . "</h5>\n";
	...
	if ($type != 'html') {		// Plain-text view
		echo '<pre>';
		echo 'Search Run On: ' . date('M jS, Y - h:i:s a') . "\r\n\r\n";
	}	
    ...
    for ($i = 0; $i < count($res); $i++) {
        ...
        $modified = !empty($rs['modified']) ? CmnFns::formatDateTime($rs['modified']) : 'N/A';
