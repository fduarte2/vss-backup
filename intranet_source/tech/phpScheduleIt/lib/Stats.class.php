...
class Stats {
	...
	*/
	function print_schedule_error() {
		CmnFns::do_error_box('That schedule is not available.<br/><a href="javascript: history.back();">Back</a>', '', false);
	}
	...
	* 			can be: MONTH, DAY_0F_WEEK, DAY_OF_MONTH, USER, RESOURCE, PI, START_TIME, END_TIME
	*/
	function set_stats($stat_type) {
		...
		switch ($stat_type) {
			case MONTH :
				...
				$this->graph_title = 'Reservations by month';
			break;
			case DAY_OF_WEEK :
				$this->labels =& $this->day_names;
				$this->values =& $this->dayofweek;
				$this->total = $this->numRes;
				$this->graph_title = 'Reservations by day of the week';
			break;
			case DAY_OF_MONTH :
				...
				$this->dyn_title = 'Reservations per month';
			break;
			case USER :
				...
				$this->dyn_title = 'Reservations per user';
			break;
			case RESOURCE :
				$this->labels =& $this->machids;
				$this->values =& $this->resource;
				$this->total = $this->numRes;
				$this->graph_title = 'Reservations per resource';
			break;
			case START_TIME :
				...
				$this->total = $this->numRes;
				$this->graph_title = 'Reservations per start time';
			break;
			case END_TIME :
				...
				$this->total = $this->numRes;
				$this->graph_title = 'Reservations per end time';
			break;
		}
	...
	*/
	function print_multiple_stats() {
		if (!$this->dynlabel) {			// Only print all when we are not using dynamic labels
			$this->graph_title = $this->dyn_title . ' [All Reservations]';
		...
		foreach ($this->index as $k => $v) {
			...
			$this->graph_title = $this->dyn_title . ' for ' . $v;					// Set the title
	...
	*/
	function get_percent($val) {
		...
		return sprintf('%.02f', ($val/$this->total) * 100);
	}
