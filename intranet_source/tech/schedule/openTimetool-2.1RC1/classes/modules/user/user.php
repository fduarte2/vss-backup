...
//FIXXME check for references in TABLE_PRICE, and all the other tables too
    ...
    */
    function save( $data )
    {
        ...
        if ($data['email'] && !vp_Validation::isEmail($data['email'])) {
            $applError->set('Please enter a valid email-address!');
            $ret = false;
        }

        if ($data['hoursPerDay'] && !is_numeric($data['hoursPerDay'])) {
            $applError->set('The hours per day has to be a number!');
            $ret = false;
        }
        if (!$data['hoursPerDay']) { // if no hours are set store null, so no 0 appears
        ...
        if (!$data['login'] ) {  // we always need a login to be given!!!
