...
//
//  $Log: member.php,v $
//  Revision 1.6.2.1  2003/04/10 18:04:32  wk
//  - use references for getInstance!
//
//  Revision 1.6  2003/03/06 11:06:35  wk
//  - use static tree-cache class
//
//  Revision 1.5  2003/03/04 19:08:20  wk
//  - add updateSpecial()
//
//  Revision 1.4  2003/02/18 20:13:08  wk
//  - added getManagerProjects()
//
//  Revision 1.3  2003/01/28 15:18:34  wk
//  - E_ALL stuff
//
//  Revision 1.2  2002/11/13 18:57:42  wk
//  - added method getInstance
//  - and check if a member can be removed
//
//  Revision 1.1  2002/10/28 11:16:02  wk
//  - handle members of projects
//
//
    ...
    */
    function addMember( $userIds , $projectId , $isManager=0 )
    {
        ...
        foreach( $userIds as $aUserId ) {
            $data[] = array('user_id'=>$aUserId,'projectTree_id'=>$projectId,'isManager'=>$isManager);
        }
...
*/
        ...
        if (sizeof($missingManagers = array_diff($managers,$allUserIdsInDb))) {
            ...
            foreach ($missingManagers as $aAdd) {
                $newData[] = array('user_id'=>$aAdd,'isManager'=>1,'projectTree_id'=>$projectId);
            }
        ...
        if (sizeof($missingMembers = array_diff($members,$allUserIdsInDb))) {
            ...
            foreach ($missingMembers as $aAdd) {
                $newData[] = array('user_id'=>$aAdd,'isManager'=>0,'projectTree_id'=>$projectId);
            }
            $this->addMultiple($newData);
        }
    ...
    */
    function isManager( $projectId=null , $userId=null )
    {
        ...
        if( $projectId == null && sizeof($manages) )
            return true;
        if( $projectId && $manages[$projectId] )
            return true;
