   ...
   if (!$connection){
    ...
    if ($dh = opendir($dir.$path)) {
 		...
 		if ($ipos = strrpos($file, ".")){
				...
				$file_name[$array_index] = substr($file, 0, $ipos-1);
...
<? for($i = $array_index -1 ; $i >=0 ; $i--){ ?>
  <option value="<? echo $cube_file[$i] ?>"><? echo $file_name[$i] ?>
...
<script language="javascript">
  ...
  CubeOpen("<? echo $cube_file[$array_index -1] ?>");
