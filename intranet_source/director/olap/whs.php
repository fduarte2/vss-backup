...
/*
				$file_name[$array_index] = substr($file, 0, $ipos);
                            	$cube_file[$array_index] = $path."/".$file;
				if ($file_name[$array_index] == $default){
					$cFile = $path."/".$file;
				}
*/
				$fName = substr($file, 0, $ipos);
				$cube_file = $path."/".$file;
                                if ($fName == $default){
                                        $cFile = $cube_file;
                                }

				array_push($file_name, array('fName'=>$fName, 'cube_file'=>$cube_file));
    ...
    $sorted_file = array_sort($file_name, "fName");
...
<center><a href="CubeDocumentation.doc">Click here for Documentation</a><br /></center>
...
<!--
<option value="<? echo $cube_file[$array_index-1] ?>"  ><? echo $file_name[$array_index-1] ?>
-->
<? for($i = 0; $i < $array_index; $i++){ 
	if ($sorted_file[$i][fName] == $default){
		$strSelect = "selected";
	}else{
		$strSelect = "";
	}
...
?>
  ...
  <option value="<? echo $sorted_file[$i][cube_file] ?>" <? echo $strSelect ?> ><? echo $sorted_file[$i][fName] ?>
