...
//    printf("</body></html>");
   ...
   $arrHeading2 = array('sup'=>'<b>Supervisor</b>', 'pReg'=>'<b>Reg</b>', 'pOT'=>'<b>OT</b>', 'pDT'=>'<b>DT</b>', 'pOther'=>'<b>Other</b>','lST'=>'<b>ST</b>','lOT'=>'<b>OT</b>', 'lDT'=>'<b>DT</b>','lMH'=>'<b>MH</b>','lDF'=>'<b>DF</b>');
...
//	$sTime += 24*60*60;
	...
	$pre_user = "";
   	...
   	for ($i = 0; $i < count($user); $i++){
		array_push($data, array('sup'=>$user[$i], 'pReg'=>$pReg[$i], 'pOT'=>$pOT[$i], 'pDT'=>$pDT[$i], 'pOther'=>$pOther[$i], 'lST'=>$lST[$i], 'lOT'=>$lOT[$i], 'lDT'=>$lDT[$i], 'lMH'=>$lMH[$i],'lDF'=>$lDF[$i]));
		$tot_pReg += $pReg[$i];
    		$tot_pOT += $pOT[$i];
		$tot_pDT += $pDT[$i];
        	$tot_pOther += $pOther[$i];
 ...
 //  }
   array_push($data, array('sup'=>'<b>Total</b>', 'pReg'=>$tot_pReg, 'pOT'=>$tot_pOT, 'pDT'=>$tot_pDT, 'pOther'=>$tot_pOther, 'lST'=>$tot_lST, 'lOT'=>$tot_lOT, 'lDT'=>$tot_lDT, 'lMH'=>$tot_lMH, 'lDF'=>$tot_lDF));
...
<? if ($j < count($data)-1 && $data[$j][lOT] > 0) { ?>
                <td align=right><a href="labor_ticket.php?sDate=<?echo $sDate?>&eDate=<?echo $eDate?>&sup=<?echo $data[$j][sup]?>&type=OT" target="blank"><?echo htmlText($data[$j][lOT])?></a></td>
<? }else{ ?>
                <td align=right><?echo htmlText($data[$j][lOT])?></td>
<? } ?>
<? if ($j < count($data)-1 && $data[$j][lDT] > 0) { ?>
                <td align=right><a href="labor_ticket.php?sDate=<?echo $sDate?>&eDate=<?echo $eDate?>&sup=<?echo $data[$j][sup]?>&type=DT" target="blank"><?echo htmlText($data[$j][lDT])?></a></td>
<? }else{ ?>
                <td align=right><?echo htmlText($data[$j][lDT])?></td>
<? } ?>
<? if ($j < count($data)-1 && $data[$j][lMH] > 0) { ?>
                <td align=right><a href="labor_ticket.php?sDate=<?echo $sDate?>&eDate=<?echo $eDate?>&sup=<?echo $data[$j][sup]?>&type=MH" target="blank"><?echo htmlText($data[$j][lMH])?></a></td>
<? }else{ ?>
                <td align=right><?echo htmlText($data[$j][lMH])?></td>
<? } ?>
<? if ($j < count($data)-1 && $data[$j][lDF] > 0) { ?>
                <td align=right><a href="labor_ticket.php?sDate=<?echo $sDate?>&eDate=<?echo $eDate?>&sup=<?echo $data[$j][sup]?>&type=DF" target="blank"><?echo htmlText($data[$j][lDF])?></a></td>
<? }else{ ?>
