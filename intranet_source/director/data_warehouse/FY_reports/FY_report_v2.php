...
function viewreport(){
  if (document.get_data.arv.value == "Vessel"){
	document.get_data.action="FY_vessel_v2.php";
  }else{
        document.get_data.action="FY_OTR.php";
  }
...
<table border="0" width="100%" cellpadding="4" cellspacing="0">
   <tr>
      ...
      <td>
	 <p align="left">
	    <font size="5" face="Verdana" color="#0066CC">Scanned Cargo Report
</font>
	    <hr>
	 </p>
      </td>
...
?>
		...
		<td align="center"><font size="2" face="Verdana">Commodity Type:  <select name="comm">
<?
...
?>
												<select></font></td>
		<td align="right"><font size="2" face="Verdana">Arrival Via:  <select name="arv">
						<option value="Vessel"<? if($arv == "Vessel"){ ?> selected <? } ?>>Vessel</option>
						<option value="OTR"<? if($arv == "OTR"){ ?> selected <? } ?>>OTR</option>
			</select></td>
	</tr>
	<tr>
		<td align="left"><font size="2" face="Verdana">Customer:</td>  
		<td colspan="2"><select name="cust">
...
?>
	...
	<tr>
		...
		<td colspan="2"><select name="vessel">
															<option value="all">All</option>
<?
		$sql = "SELECT DISTINCT LR_NUM, VESSEL_NAME FROM VESSEL_PROFILE WHERE LR_NUM <= 20000 ORDER BY LR_NUM DESC";
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
		while(ora_fetch_into($Short_Term_Cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
?>
...
?>
	...
	<tr>
		<td colspan="3" align="center"><input type="submit" name="submit" value="Generate Report" onclick="javascript:viewreport()"><hr></td>
	</tr>
</form>
</table>
<?
