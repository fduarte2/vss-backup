...
//	$submit = $HTTP_POST_VARS['submit'];
	...
	$filename = "ITRFFY".date('mdYhi')."OTR.xls";
...
<?
	...
	if(!$fp){
		...
		AND CT.ARRIVAL_NUM != '4321' AND 
		CT.DATE_RECEIVED IS NOT NULL 
...
$sql2 = "SELECT COUNT(*) THE_COUNT
				FROM ".$cargo_tracking." CT, ".$cargo_activity." CA
				WHERE 
				CT.RECEIVER_ID ".$sql_cust." 
				AND CT.ARRIVAL_NUM != '4321' 
				AND CT.DATE_RECEIVED IS NOT NULL 
				AND CT.DATE_RECEIVED <= ".$day_number." + TO_DATE('07/01/".($FY - 1)."', 'MM/DD/YYYY') 
				AND CA.DATE_OF_ACTIVITY >= ".$day_number." + TO_DATE('07/01/".($FY - 1)."', 'MM/DD/YYYY') 
				AND CT.RECEIVING_TYPE = 'T' 
				AND CT.RECEIVER_ID = CA.CUSTOMER_ID 
				AND CT.ARRIVAL_NUM = CA.ARRIVAL_NUM 
				AND CT.PALLET_ID = CA.PALLET_ID 
				...
				AND CA.SERVICE_CODE IN ('6', '11')";
		//echo $sql2;
		ora_parse($select_cursor, $sql2);
		ora_exec($select_cursor);
		while(ora_fetch_into($select_cursor, $select_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
			$todays_count = $select_row['THE_COUNT']; 
			fwrite($fp, $day_number."\t");
			fwrite($fp, $todays_count."\n");
		}
