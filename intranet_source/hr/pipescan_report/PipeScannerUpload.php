...
//  $conn = ora_logon("SAG_OWNER@BNITEST", "BNITEST238");
  ...
  $Short_Term_Cursor = ora_open($conn);
	...
	if($submit != "" && $HTTP_POST_FILES['detail_file']['name'] != "" && $HTTP_POST_FILES['exception_file']['name'] != ""){
...
//		print_r($temp);
		...
		$exception_filename = $start_year.$start_month.$start_day.$start_hour."to".$end_year.$end_month.$end_day.$end_hour."exception.";
		$xcpfilename = basename($HTTP_POST_FILES['exception_file']['name']);
//		echo $filename."<br>";
		$temp = split("\.", $xcpfilename);
//		print_r($temp);
		$exception_filename .= $temp[1];
		...
		$target_path_exception = "./".$exception_filename;
//		echo $target_path."<br>";

		if(move_uploaded_file($HTTP_POST_FILES['detail_file']['tmp_name'], $target_path_detail) && move_uploaded_file($HTTP_POST_FILES['exception_file']['tmp_name'], $target_path_exception)){
			$sql = "INSERT INTO PIPESCAN_LOG (START_SCAN, END_SCAN, UPLOAD_TIME, UPLOADED_BY, COMMENTS, SKIP_REASONS, DETAIL_FILE_NAME, EXCEPTION_FILE_NAME) VALUES(TO_DATE('".$start_time."', 'MM/DD/YYYY HH24'), TO_DATE('".$end_time."', 'MM/DD/YYYY HH24'), SYSDATE, '".$user."', '".$comments."', '".$skip_reasons."', '".$detail_filename."', '".$exception_filename."')";
			ora_parse($cursor, $sql);
			ora_exec($cursor);
			system("/bin/chmod a+r $target_path_detail");
			system("/bin/chmod a+r $target_path_exception");
		} else {
...
<table border="0" width="100%" cellpadding="4" cellspacing="0">
   <tr>
      ...
      <td>
	 <p align="left">
	    <font size="5" face="Verdana" color="#0066CC">Pipe-Scanner Upload / Report
</font>
	    <hr>
	 </p>
      </td>
...
<table border="0" width="100%" cellpadding="4" cellspacing="0">
<form name="get_data" action="get_report.php" method="post">
	<tr>
	...
	<tr>
		...
		<td width="4%">&nbsp;</td>
<?
	$sql = "SELECT TO_CHAR(START_SCAN, 'MM/DD/YYYY HH24:MI') THE_START, TO_CHAR(END_SCAN, 'MM/DD/YYYY HH24:MI') THE_END, UPLOAD_TIME, DETAIL_FILE_NAME, EXCEPTION_FILE_NAME FROM PIPESCAN_LOG ORDER BY UPLOAD_TIME DESC";
...
<?
		...
		$filename_exc = $row['EXCEPTION_FILE_NAME'];
?>
		<td width="48%" align="center"><? echo $row['THE_START']." to ".$row['THE_END']; ?><br><a href="<? echo $filename_det; ?>">Detail</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="<? echo $filename_exc; ?>">Exceptions</a></td>
<?
	}
?>
	</tr>
...
<table border="0" width="100%" cellpadding="4" cellspacing="0">
<form enctype="multipart/form-data" name="put_data" action="PipeScannerUpload.php" method="post">
	<tr>
<?
	$sql = "SELECT TO_CHAR(START_SCAN, 'MM/DD/YYYY HH24:MI') THE_START, TO_CHAR(END_SCAN, 'MM/DD/YYYY HH24:MI') THE_END, UPLOAD_TIME FROM PIPESCAN_LOG ORDER BY UPLOAD_TIME DESC";
	ora_parse($Short_Term_Cursor, $sql);
	ora_exec($Short_Term_Cursor);
	if(!ora_fetch_into($Short_Term_Cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
?>
...
?>
	...
	<tr>
		<td width="10%">&nbsp;</td>
		<td colspan="2">
				<font size="2" face="verdana">Month:<input type="text" name="start_month" value="MM" maxlength="2" size="4">&nbsp;&nbsp;
												Day:<input type="text" name="start_day" value="DD" maxlength="2" size="4">&nbsp;&nbsp;
												Year:<input type="text" name="start_year" value="YYYY" maxlength="4" size="4">&nbsp;&nbsp;
												Hour:<input type="text" name="start_hour" value="HH" maxlength="2" size="4">(24h)
				</font></td>
	</tr>
	<tr>
	...
	<tr>
		<td width="10%">&nbsp;</td>
		<td colspan="2">
				<font size="2" face="verdana">Month:<input type="text" name="end_month" value="MM" maxlength="2" size="4">&nbsp;&nbsp;
												Day:<input type="text" name="end_day" value="DD" maxlength="2" size="4">&nbsp;&nbsp;
												Year:<input type="text" name="end_year" value="YYYY" maxlength="4" size="4">&nbsp;&nbsp;
												Hour:<input type="text" name="end_hour" value="HH" maxlength="2" size="4">(24h)
				</font></td>
	</tr>
	<tr>
	...
	<tr>
		<td colspan="2" width="25%"><font size="2" face="Verdana">Exception File:</font></td>
		<td colspan="1">&nbsp;</td>
	</tr>
	<tr>
		<td width="10%">&nbsp;</td>
		<td colspan="2"><input type="file" name="exception_file"></td>
	</tr>
	<tr>
