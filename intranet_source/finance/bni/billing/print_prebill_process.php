<?
// File: print_prebill_process.php
//
// Lynn F. Wang  (12/05/03 - 02/17/04)
...
// get connection prameters
include("connect.php");

// Get form data
$billing_type = $HTTP_POST_VARS["selected_type"];
// HD 8668 - LABOR BILLS are being split off to their own logic (date independant)
//if ($billing_type == 'Labor') {
//	include("print_prebill_labor.php");
//	exit;
//}
...
if ( $billing_type == "Advance Billing" )
{
	...
	if($lr_num == ""){
		...
		echo "<font color=\"#FF0000\">If choosing Advance Billing, you must enter a value into the LR# box.<br><a href=\"print_prebill.php\">Click Here</a> To return to the previous page.</font>";
...
elseif ($billing_type == "Dockage/Lines")
{
	...
	if($lr_num == ""){
		...
		$area_type = "FINA";
		include("pow_header.php");
		echo "<font color=\"#FF0000\">If choosing Dockage/Lines, you must enter a value into the LR# box.<br><a href=\"print_prebill.php\">Click Here</a> To return to the previous page.</font>";
		include("pow_footer.php");
		exit;
	}
...
elseif ($billing_type == "Wharfage")
{
	...
	if($lr_num == ""){
		...
		$area_type = "FINA";
		include("pow_header.php");
		echo "<font color=\"#FF0000\">If choosing Wharfage, you must enter a value into the LR# box.<br><a href=\"print_prebill.php\">Click Here</a> To return to the previous page.</font>";
		include("pow_footer.php");
		exit;
	}
}
...
// added Adam Walter, April 2007, to accomodate for a change to "miscellaneous"
if($billing_type == "Miscellaneous"){
	$misc_date = $HTTP_POST_VARS["misc_date"];
	$misc_cust = $HTTP_POST_VARS["misc_cust"];
//	echo $misc_date." -- ".$misc_cust;
//	exit;
} else {
	$misc_date = "";
	$misc_cust = "";
...
// retrieve all prebill records for the given billing type
$stmt = getQueryByType($billing_type, $lr_num, $misc_date, $misc_cust);
ora_parse($cursor1, $stmt);
ora_exec($cursor1);
ora_fetch_into($cursor1, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC);
//echo $stmt."<br>";
//exit;
$rows = ora_numrows($cursor1);

if ($rows <= 0) {
  header("Location: print_prebill.php?type=$billing_type");
  exit;
}
...
$total_page = $pdf->ezStartPageNumbers(65, 750, 10, '', '{PAGENUM}', 1);
$bill_page = $pdf->ezStartPageNumbers(575, 695, 10, '', '', 1);

$sub_total = 0;       // total of a section of a preinvoice, like one labor ticket or billing type
$total = 0;           // preinvoice total
$grand_total = 0;     // grand total of all preinvoice
...
// get the 1st preinvoice # for the processed month
$prebill_num = getPrebillNumber();
//echo "PBN: ".$prebill_num;
//exit;
...
do {
  ...
  // to determin whether a new preinvoice need to be generated
  // 1. LABOR need to generate new prebill for each (customer, vessel, service_date)
  ...
  // 4. other types need new prebill for each (customer, vessel)
  if ($billing_type == 'Labor') {
//    if (($cust_id == $curr_cust_id) && ($lr_num == $curr_lr_num) && ($curr_service_date == $service_date)) {
  ...
  // 3. First entry of a new preinvoice, new labor ticket, or new billing type alwasy shows the date.
  ...
  // to determin whether a new section of a preinvoice need to be generated; there will be a subtotal after
  // each section of a preinvoice
  // 1. LABOR has a new section for each labor ticket #
  // 2. Advance Vessel Billing has one section for each billing type and date, including Advance Truck Loading, 
...
//	echo "date: ".$date."  cur_date: ".$curr_date."\n";
      $new_section = true;
  ...
  // we need to decide on the value of $new_prebill, $new_section before we call makeDescription
  $amount = "$" . number_format($row['SERVICE_AMOUNT'], 2, '.', ',');
  $desc = makeDescription($cursor2, $row, $bill_type);
  ...
  $date1 = $date;
  if (!$first_entry && !$new_prebill && !$new_section) {
    if ($date == $curr_date) {
      $date = "";
    } else {
      $curr_date = $date;
    ...
    if ($billing_type == 'Labor') {
      $amount = "\n\n" . $amount;
	      ...
	      ($billing_type == 'Advance Billing' && ($first_entry || $new_prebill || $show_section_header))) {
  ...
  if (!$new_prebill) {
	...
	// JAN 2014 - AND for dole paper truckunloading miscbills
    if (!$new_section) {
      $sub_total += $row['SERVICE_AMOUNT'];
    } else {
      $record = array();
      array_push($record, array('first'=>'', 'second'=>'', 'third'=>'____________'));
	  if(($billing_type == 'Miscellaneous') && ($curr_subcust_id != "") && ($curr_dole_comm_newinv == "1272" || $curr_dole_comm_newinv == "1299")){
		  array_push($record, array('first'=>'', 'second'=>"<b>Subtotal for Mill:      ".GetCustomerName($curr_subcust_id, $ora_conn)."</b>", 'third'=>"<b>".number_format($sub_total, 2, '.', ',')."</b>"));
	  } else {
    ...
    // next, need to start a new pre-invoice for the fetched row
    $prebill_num++;
    ...
    if ($billing_type == 'Labor' || $billing_type == 'Advance Billing' || $billing_type == 'Dockage/Lines' || 
      ...
      $record = array();
      array_push($record, array('first'=>'', 'second'=>'', 'third'=>'____________'));
//	  echo "dolecomm: ".$curr_dole_comm_newinv."      for current row billingum:  ".$row['BILLING_NUM']."        and curr_subcust: ".$curr_subcust_id."<br>";
	  if(($billing_type == 'Miscellaneous') && ($curr_subcust_id != "") && ($curr_dole_comm_newinv == "1272" || $curr_dole_comm_newinv == "1299")){
		  array_push($record, array('first'=>'', 'second'=>"<b>Subtotal for Mill:      ".GetCustomerName($curr_subcust_id, $ora_conn)."</b>", 'third'=>"<b>".number_format($sub_total, 2, '.', ',')."</b>"));
	  } else {
		array_push($record, array('first'=>'', 'second'=>'Subtotal:', 
    ...
    $record = array();
    array_push($record, array('first'=>'', 'second'=>'', 'third'=>''));
    array_push($record, array('first'=>'', 'second'=>'Preinvoice Total:',
	...
	// prepairation for a new preinvoice
	...
	// update current prebill information
  ...
  // write the header
  if ($first_entry || $new_prebill) {
    // no longer the 1st entry
    $first_entry = false;
    // get customer info
    $vessel_name = getVesselName($cursor2, $curr_lr_num);
    $customer_info = getCustomerInfo($ora_conn, $cursor2, $curr_cust_id);

    $general_info = array();
    array_push($general_info, array('first'=>'', 'second'=>'', 'third'=>'Prebill No:', 'fourth'=>$prebill_num));
    array_push($general_info, array('first'=>'', 'second'=>'', 'third'=>'Run Date:', 'fourth'=>$today));

    // add more space
    for ($i=0; $i<2; $i++) {
      array_push($general_info, array('first'=>'', 'second'=>'', 'third'=>'', 'fourth'=>''));
    }
    array_push($general_info, array('first'=>$customer_info . "\n\n\n" . $vessel_name, 'second'=>'',
				    'third'=>'', 'fourth'=>''));
...
} while (ora_fetch_into($cursor1, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC));

// write sub-total line of the last section of the last prebill for Labor, Advance Billing and Dockage/Lines
...
// write the total line of the last prebill
$record = array();
array_push($record, array('first'=>'', 'second'=>'', 'third'=>''));
if ($billing_type == "S-Wharfage"){
   ...
   array_push($record, array('first'=>'', 'second'=>'Preinvoice Total:  '.$total_qty.$qtySpace.$service_rate.'     ',
                             'third'=>'$' . number_format($total, 2, '.', ',')));
}else{
   array_push($record, array('first'=>'', 'second'=>'Preinvoice Total:', 
			     'third'=>'$' . number_format($total, 2, '.', ',')));
...
//array_push($record, array('first'=>'', 'second'=>'Preinvoice Total:', 'third'=>'$' . number_format($total, 2, '.', ',')));
...
$pdf->ezText("GRAND TOTAL OF " . strtoupper($billing_type) . " PREINVOICES FOR RUN DATE $today :", 
...
//			  echo "<br>";
			  ...
			  if($newpagecounter == 30){
				  ...
				  $display_array = array();
				  array_push($display_array, array('plt'=>'<b>PALLET ID</b>', 'desc'=>'<b>DESCRIPTION</b>', 'QTY'=>'<b>QTY</b>', 'ves'=>'<b>ARRIVAL NUM</b>', 'order'=>'<b>ORDER #</b>'));
...
function writeTheRecord() {
  ...
  if ($before_page_num != $after_page_num) {
    ...
    // if it is not a total line, it should have date anyway
    if (count($record) == 1) { 
...
// function edited, Adam Walter, April 2007.  Function now has 4 arguments, although final 2 only used in 1 case
function getQueryByType($billing_type, $vsl_num, $misc_date, $misc_cust) {
  global $billing;
  
  switch ($billing_type) {
  case "Advance Billing":

    if (strtoupper($vsl_num) == "ALL")
    {
  ...
  case "Holmen Storage":
    $stmt = "select * from billing where billing_type = 'H_STORAGE' and service_status = 'PREINVOICE'
             order by lr_num, arrival_num, lot_num, service_start, service_stop, billing_num";
    break;
  ...
  case "Miscellaneous":
	...
	if($misc_date != ""){
		$stmt .= "and service_date = to_date('".$misc_date."', 'MM/DD/YYYY') ";
	}
	if($misc_cust != ""){
		$stmt .= "and customer_id = '".$misc_cust."' ";
	}
...
//	$stmt .= "order by customer_id, ves_sort, service_date, service_code, billing_num";
//	$stmt .= "order by customer_id, lr_num, service_date, service_code, billing_num";
//	echo $stmt;
//	exit;
    break;
...
function makeDescription ($cursor, $row, $bill_type) {
  global $first_entry, $new_prebill, $new_section, $labor_ticket, $lr_num, $show_section_header, $short_term_cursor;
  ...
  case "TERMSER":
    ...
    if ($first_entry || $new_prebill || ($new_section && $show_section_header)) {
      $desc = $charge . ":\n\n" . $desc;
    }
  ...
  case "STORAGE":
    ...
    if ($with_surcharge) {
      $desc = "Billing #: " . $row['BILLING_NUM'] . ";  " . $row['SERVICE_DESCRIPTION'] . 
	 "\nBOL: " . $row['CARGO_BOL'] . ";  Mark: " . $row['CARGO_MARK'] . ";  " . $row['SERVICE_QTY'] 
	 . " " . $row['SERVICE_UNIT'];
    } else {
	...
	if ($factor != -1) {
      ...
      $desc = "Billing #: " . $row['BILLING_NUM'] . ";  BOL: " . $row['CARGO_BOL'] . ";  Mark: " . 
	 $row['CARGO_MARK'] . ";  " . $service_qty . " " . $service_unit . " @ " . $rate_info . " " . $row['MEMO_NUM'];
    }
  ...
  case "DOCKAGE":
    ...
    $desc = "Billing #: " . $row['BILLING_NUM'] . ";  " . $service_name . " - " . $row['SERVICE_QTY2'] . 
       " Day(s);  NRT: " . $nrt . ";  LOA: " . $loa . " FT;  Rate: " . $rate_info;
	...
	if($row['SERVICE_UNIT'] == "FEE"){ 
		...
		$desc = "Billing #: ".$row['BILLING_NUM']."; ".$row['SERVICE_DESCRIPTION'].":  ".$short_term_row['THE_CHARGE'];
	}
  ...
  case "EQUIPMENT":
    ...
    $service_name = getServiceNameNoCode($cursor, $row['SERVICE_CODE']);
    $desc = "Billing #: " . $row['BILLING_NUM'] . ";  " . $desc_service .":". $service_name . ":  " . 
       $row['START_TIME'] . " - " . $row['STOP_TIME'] . "  @ $" . $row['SERVICE_RATE'] . " / HR";
    break;
  ...
  case "H_ADMIN":
    // get billing numbers
    $stmt = "select billing_num from billing where billing_type = '$bill_type' and service_status = 'PREINVOICE' 
             and lr_num = " . $row['LR_NUM'] . " and arrival_num = " . $row['ARRIVAL_NUM'] . " order by billing_num";
    ora_parse($cursor, $stmt);
    ora_exec($cursor);
    
    $billing_nums = "";
    while (ora_fetch_into($cursor, $row2, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)) {
      if ($billing_nums == "") {
	$billing_nums = $row2['BILLING_NUM'];
      } else {
	$billing_nums .= ", " . $row2['BILLING_NUM'];
      }
    }
    ...
    $desc = "Billing #: $billing_nums;  $desc";
    break;
  ...
  case "LABOR":
    ...
    if ($row['START_TIME'] == "12:00 AM" && $row['STOP_TIME'] == "12:00 AM") {
      ...
      $desc = "Billing #: " . $row['BILLING_NUM'] . ";  " . $row['SERVICE_QTY'] . " " . $employee . ":  " . 
	 $hours . " HRS @ $" . $row['SERVICE_RATE'] . " / HR";
    } else {
      $desc = "Billing #: " . $row['BILLING_NUM'] . ";  " . $row['SERVICE_QTY'] . " " . $employee . ":  " . 
	 $row['START_TIME'] . " - " . $row['STOP_TIME'] . " @ $" . $row['SERVICE_RATE'] . " / HR";
    }
  ...
  case "FREIGHT":
  case "H_STORAGE":
    $desc = "Billing #: " . $row['BILLING_NUM'] . ";  " . trim($row['SERVICE_DESCRIPTION']);
    break;
  ...
  case "TRKLOADING":
    ...
    if ($qty2_unit != "") {
	 ...
	 $row['ACTIVITY_NUM_WO'];
      ...
      $desc = "Billing #: " . $row['BILLING_NUM'] . ";  W/O #: " . $row['DELIVERY_NUM'] . ";  BOL: " . 
	 $row['CARGO_BOL'] . ";  Mark: " . $row['CARGO_MARK'] . ";  " . $qty1 . " " . $row['QTY1_UNIT'] . " = " . 
	 $qty2 . " " . $row['QTY2_UNIT'] . " = " . $service_qty . " " . $row['SERVICE_UNIT'] . " @ " . $rate_info;
    } else {
      $desc = "Billing #: " . $row['BILLING_NUM'] . ";  W/O #: " . $row['DELIVERY_NUM'] . ";  BOL: " . 
	 $row['CARGO_BOL'] . ";  Mark: " . $row['CARGO_MARK'] . ";  " . $qty1 . " " . $row['QTY1_UNIT'] . " = " . 
	 $service_qty . " " . $row['SERVICE_UNIT'] . " @ " . $rate_info;
    }
  ...
  case "TRKUNLDNG":
    ...
    $qty2_unit = trim($row['QTY2_UNIT']);
    if ($qty2_unit != "") {
      $desc = "Billing #: " . $row['BILLING_NUM'] . ";  BOL: " . $row['CARGO_BOL'] . ";  Mark: " . $row['CARGO_MARK'] . 
	 ";  " . $row['QTY_EXPECTED'] . " " . $row['QTY1_UNIT'] . ";  " . $row['QTY2_EXPECTED'] . " " . 
	 $row['QTY2_UNIT'] . ";  " .  $service_qty . " " . $service_unit . " @ $" . $row['SERVICE_RATE'] . 
	 " / " . $rate_unit;
    } else {
      $desc = "Billing #: " . $row['BILLING_NUM'] . ";  BOL: " . $row['CARGO_BOL'] . ";  Mark: " . $row['CARGO_MARK'] . 
	 ";  " . $row['QTY_EXPECTED'] . " " . $row['QTY1_UNIT'] . ";  " . $service_qty . " " . $service_unit . " @ $" . 
  ...
  case "WHARFAGE":
		...
		$desc = "Billing #: " . $row['BILLING_NUM'] . ";  " . $service_name . ":  " . $commodity . ";  " . 
		   $qty_unit . " @ " . $rate_info;
	...
	if($row['SERVICE_DESCRIPTION'] == "SECURITY"){ 
		...
		$desc = "Billing #: ".$row['BILLING_NUM']."; SECURITY CHARGE:  ".$commodity.";  ".$service_qty." ".$short_term_row['UNIT']." @ $".$short_term_row['RATE_CHARGE']."/".$short_term_row['UNIT'];
	}
  ...
  case "S-WHARFAGE":
    ...
    $desc = "Billing #: ". $row['BILLING_NUM']." \r\n".$row['SERVICE_DESCRIPTION'].$descSpace .$service_qty .$qtySpace .$service_rate ;
    break;
  ...
  case "FUMINSP":
	  ...
	  $desc = "Billing #: ". $row['BILLING_NUM']." ".$row['DESC_TEXT']." ".$container_text." ".$row['TYPE_TEXT']." ".$row['SERVICE_QTY']." ".
