 ...
 * 
 * Created by Lynn F. Wang on 08/20/03
...
if (!$bni_conn) {
  printf("Cannot log on to the BNI Server: " . ora_errorcode($bni_conn) . ". Please report to TS!");
  exit;
}
...
if ($billing_type == 'LABOR' && $whole_labor_ticket == "on") {
  $stmt = "update billing set service_status = 'PREINVOICE'
...
// if it is FREIGHT (Holmen Freight) bill, mark the shipping order as preinvoiced
if ($billing_type == 'FREIGHT') {
  ...
  if ($row['RF_ORDER_NUM'] != "") {
    $stmt = "update order_detail set freight_charge_status = 'PREINVOICED'
...
// if it is LABOR bill with a labor ticket #, then update billing status of this labor ticket
if ($billing_type == 'LABOR') {
  // updates for Labor bill
  ...
  if ($labor_ticket_num != "") {
    ...
    // mark the labor ticket as billed (Y) since we un-deleted this billing record
    // make Oracle database connection
    $lcs_conn = ora_logon("LABOR@$lcs", "LABOR");
    if (!$lcs_conn) {
      printf("Cannot logging on to the LCS Oracle Server: " . ora_errorcode($lcs_conn));
    ...
    // update labor ticket
    $stmt = "update labor_ticket_header set bill_status = 'Y'
...
$stmt = "insert into billing_change (username, action, change_reason, billing_num, billing_type, 
            customer_id, lr_num)
         values ('$user', 'Un-Delete Prebill', '$reason', $billing_num, '$billing_type', 
            $customer_id, $lr_num)";
