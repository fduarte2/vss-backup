<?
 ...
 $i_cust = $HTTP_GET_VARS[cust];
 $i_gl = $HTTP_GET_VARS[gl];
 $i_service = $HTTP_GET_VARS[service];
 $i_comm = $HTTP_GET_VARS[comm];
...
<?
        ...
        if ($gl ==""){
                $sql = "select gl_code from service_category where service_code = $i_service";
                ora_parse($cursor, $sql);
                ora_exec($cursor);
                if (ora_fetch($cursor)){
                        $i_gl = ora_getcolumn($cursor, 0);
                }
        }
        $sql = "select id from gl_allocation_header
                 where customer_id = $i_cust and gl_code = '$i_gl' and service_code = '$i_service' and commodity_code = '$i_comm'";
        ora_parse($cursor, $sql);
        ora_exec($cursor);
        if (ora_fetch($cursor)){
