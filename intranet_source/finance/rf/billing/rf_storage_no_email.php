...
/*
  ...
  $mailheaders .= "Cc: " . "lwang@port.state.de.us\r\n";
  $mailTO = $argv[1];
  $body = "";
*/
  // Connect to RF
...
*/
  ...
  while(ora_fetch_into($cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
    ...
    if ($rows <= 0) {
      ...
      $stmt = "select * from $cargo_activity where pallet_id = '$pallet_id' and arrival_num = '$arrival_num' and customer_id = '$receiver_id'";
      $ora_success = ora_parse($cursor2, $activity_stmt);
  ...
  $stmt = "select * from $cargo_tracking where billing_storage_date between to_date('$start_date 00:00:00', 'MM/DD/YYYY HH24:MI:SS') and to_date('$end_date 23:59:59', 'MM/DD/YYYY HH24:MI:SS') and BILL = '1' and qty_in_storage > 0";
  $ora_success = ora_parse($cursor, $stmt);
  $ora_success = ora_exec($cursor);
...
function getCommodity($receiver_id, $from_shipping_line, $to_shipping_line){
  // Default to 0000
  $return = "0000";
