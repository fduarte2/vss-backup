...
// starting here, information will only conditionally display on page if date is already chosen.
	if($Start_Date != ""){
		...
		$sql = "SELECT CM.LR_NUM VESSEL, SUM(CARGO_WEIGHT) WEIGHT, SUM(QTY_EXPECTED) PALLETS FROM CARGO_MANIFEST CM, VOYAGE V WHERE CM.LR_NUM = V.LR_NUM AND CM.COMMODITY_CODE IN ('5100', '5101', '5901', '5900') AND V.DATE_FINISHED > to_date('$Start_Date', 'MM/DD/YYYY') GROUP BY CM.LR_NUM ORDER BY CM.LR_NUM";
...
<?
			if($total_pallets + $current_pallets > 30000){
				...
				$over30Kratio = $over30000by / $current_pallets;
			}
			if($total_pallets + $current_pallets > 100000){
				$over100000by = ($total_pallets + $current_pallets) - 100000;
				$over100Kratio = $over100000by / $current_pallets;
			}
...
?>
	<tr>
		...
		<td align="left"><b>&nbsp&nbsp<? echo round((($current_lbs * $over30Kratio) / 2000), 2); ?></b></td>
	</tr>
<?
			$over30Kprinted = TRUE;
			}
			if($over100000by != 0 && $over100Kprinted == FALSE){				
?>
	<tr>
		<td colspan="2" align="right"><font size="2" face="Verdana"><b>100,000 Pallets passed.  <? echo $over100000by; ?> pallets over the 100,000 mark.</b></font></td>
		<td align="right"><font size="2" face="Verdana"><b>Ton overage from previous ship:</b></font></td>
		<td align="left"><b>&nbsp&nbsp<? echo round((($current_lbs * $over100Kratio) / 2000), 2); ?></b></td>
	</tr>
<?
			$over100Kprinted = TRUE;
			}
		}
?>
	<tr>
