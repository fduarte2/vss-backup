...
* XXXXXX this is a special file, derived from A2.php,
*  Specifically designed for a single run, allowing
*  All of WHS A in the report, but excluding
*  Customer 358.  It is so specific, I'm not even
*  Linking to it on the intranet.
****************************************************/
...
<table border="0" width="100%" cellpadding="4" cellspacing="0">
<form name="the_form" action="A-Special.php" method="post">
	<tr>
		<td width="1%">&nbsp;</td>
...
<?
	...
	$sql = "SELECT RBD.PALLET_ID PALLET_ID, 
				...
				FROM RF_BILLING_DETAIL RBD, (SELECT DISTINCT PALLET_ID, ARRIVAL_NUM, RECEIVER_ID
											FROM CARGO_TRACKING_AUDIT
											WHERE WAREHOUSE_LOCATION LIKE 'A%'
											OR WAREHOUSE_LOCATION LIKE 'WING A%'
											AND AUDIT_TIME > TO_DATE('".$dateStart."', 'MM/DD/YYYY') 
											AND AUDIT_TIME < TO_DATE('".$dateEnd."', 'MM/DD/YYYY')
											AND RECEIVER_ID != '358') CTA,
				RF_BILLING RB
...
?>
	<tr>
		<td align="center"><font size="3" face="Verdana">No warehouse A2 Billing Records for the given date range.  <a href="A-Special.php">Click Here</a> to return to the previous page.</font></td>
	</tr></table>
<?
	exit;
	}
...
?>
	...
	<tr>
		<td align="center"><font size="2" face="Verdana"><BR><BR><a href="A-Special.php">Return to the previous page.</a></font></td>
	</tr>
</table>
<?
	fclose($handle);
