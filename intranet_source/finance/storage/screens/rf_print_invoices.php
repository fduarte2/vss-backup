	...
	if($start_num == "" || $end_num == "" || !is_numeric($start_num) || !is_numeric($end_num)){
		...
		while($current_invoice <= $end_num){
			...
			if(ora_fetch_into($Short_Term_Cursor, $short_term_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
				$customer = $short_term_row['CUSTOMER_ID'];
				$bill_num = $short_term_row['BILLING_NUM'];
				$unit = $short_term_row['SERVICE_UNIT'];
				$unit2 = $short_term_row['SERVICE_UNIT2'];
				$inv_date = $short_term_row['THE_INV_DATE'];
				$start_date = $short_term_row['THE_START_DATE'];
				$stop_date = $short_term_row['THE_STOP_DATE'];
				$vessel = $short_term_row['ARRIVAL_NUM'];
				$qty = $short_term_row['SERVICE_QTY'];
				$qty2 = $short_term_row['SERVICE_QTY2'];
				$rate = $short_term_row['SERVICE_RATE'];
				$inv_total = $short_term_row['SERVICE_AMOUNT'];
				$grand_total += $inv_total;

				if($vessel === "0"){
//					echo "billnum: ".$bill_num."    vessel:".$vessel."<br>";
					PrintByPoolInstead($conn, $pdf, $invoice_type, $inv_date, $customer, $bill_num, $start_date, $stop_date, $qty, $rate, $current_invoice, $end_num);
				} elseif($short_term_row['COMMODITY_CODE'] == "1299"){
					PrintDolePaperInstead($conn, $pdf, $invoice_type, $inv_date, $customer, $vessel, $bill_num, $start_date, $stop_date, $qty, $rate, $current_invoice, $end_num);
				} else {
...
function PrintByPoolInstead($conn, &$pdf, $invoice_type, $inv_date, $customer, $bill_num, $start_date, $stop_date, $qty, $rate, $current_invoice, $end_num){
	...
	while(ora_fetch_into($Short_Term_Cursor, $short_term_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		...
		if($print_detail_lines == 0){
			if($first_pass == false){
				...
				$first_pass = false;
				if($short_term_row['COMMODITY_CODE'] == '5298'){
...
//			$pdf->ezText("  ".$qty." PALLETS @ $".$rate."/PLT", 10, array('aleft'=>150));
		...
		$output_array[0] = array("pool"=>"Pool: ".$short_term_row['DOLE_POOL'],
								"desc"=>$short_term_row['THE_PLTS']." Pallets @ $".$short_term_row['SERVICE_RATE']."/plt           Arrival# ".$short_term_row['ARRIVAL_NUM'],
								"totaldollar"=>"$".number_format($short_term_row['THE_AMT'], 2));
		...
		$pdf->ezSetDy(-1);
		$print_detail_lines++;
		$invoice_total += $short_term_row['THE_AMT'];
...
//							"desc"=>$short_term_row['THE_PLTS']." pallets @ $".$short_term_row['SERVICE_RATE']."/plt",
//							"totaldollar"=>"$".$short_term_row['THE_AMT']));
...
function PrintDolePaperInstead($conn, &$pdf, $invoice_type, $inv_date, $customer, $vessel, $bill_num, $start_date, $stop_date, $qty, $rate, $current_invoice, $end_num){
...
//	$BC_unit = array();
	...
	ora_parse($subheaders, $sql);
	ora_exec($subheaders);
	while(ora_fetch_into($subheaders, $subheaders_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
		$BC_array_size = 0;
		$sub_total = 0;
		...
		$sql = "SELECT PALLET_ID, VARIETY_DESCRIPTION, SERVICE_QTY, SERVICE_QTY2, SERVICE_RATE, SERVICE_AMOUNT, SERVICE_UNIT, SERVICE_UNIT2
				FROM RF_BILLING_DETAIL
				WHERE SUM_BILL_NUM = '".$bill_num."'
					AND DOLEPAPER_ID = '".$subheaders_row['DOLEPAPER_ID']."'
					AND DOLEPAPER_BOL = '".$subheaders_row['DOLEPAPER_BOL']."'
					AND DOLEPAPER_ORDER = '".$subheaders_row['DOLEPAPER_ORDER']."'
					AND SERVICE_STATUS != 'DELETED'
				ORDER BY PALLET_ID";
		ora_parse($Short_Term_Cursor, $sql);
		ora_exec($Short_Term_Cursor);
...
//			$BC_unit[$BC_array_size] = $short_term_row['SERVICE_UNIT'];
		...
		$dole_cust_sql = "SELECT SUBSTR(CUSTOMER_NAME, 0, 23) THE_CUST
							FROM CUSTOMER_PROFILE
							WHERE CUSTOMER_ID = '".$subheaders_row['DOLE_ORIG_CUST']."'";
		ora_parse($Short_Term_Cursor, $dole_cust_sql);
		ora_exec($Short_Term_Cursor);
		ora_fetch_into($Short_Term_Cursor, $short_term_row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC);
		
		$barcodes_column[$column_counter] = "Cust#: ".$short_term_row['THE_CUST'];
		$BC_qty_column[$column_counter] = "";
		$BC_rate_column[$column_counter] = "";
		$BC_amt_column[$column_counter] = "";
		$BC_subtot_column[$column_counter] = "";
		$column_counter++;
		$barcodes_column[$column_counter] = "Booking#: ".$subheaders_row['DOLEPAPER_ID'];
		$BC_qty_column[$column_counter] = "P.O.: ".$subheaders_row['DOLEPAPER_ORDER'];
		$BC_rate_column[$column_counter] = "BoL: ".$subheaders_row['DOLEPAPER_BOL'];
		$BC_amt_column[$column_counter] = "";
		$BC_subtot_column[$column_counter] = "";
		$column_counter++;
