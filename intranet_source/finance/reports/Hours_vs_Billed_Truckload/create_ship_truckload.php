...
// NECESSARY FUNCTIONS
function compile_truckload_hours($S, $E, $C, $type, $pay_lunch, $pay_dinner) {
...
function compile_ST_hours($S, $E, $C, $type, $pay_lunch, $pay_dinner) {
	$running_REG_total = 0;
	$running_OT_total = 0;
	$running_DT_total = 0;
	$time_loop = 0;
...
// we get a single digit number
  ...
  if (!$handle){
	...
	fwrite($handle, "Hours:\n");
	fwrite($handle, "\t\tTruckloading\t\t\tStandby\t\t\t\t\t\tTotal\n");
	fwrite($handle, "Date\t\tST\tOT\tDT\tST\tOT\tDT\t\n");
...
//  $sql = "SELECT DISTINCT to_char(HIRE_DATE, 'MM/DD/YYYY/hh24:mi') THE_DATE FROM HOURLY_DETAIL WHERE VESSEL_ID = '".$vessel_number."' AND SERVICE_CODE IN ('6110', '6111', '6114', '6119', '6710', '6711', '6714', '6719', '6130', '6131', '6134', '6139') ORDER BY THE_DATE";
  $sql = "SELECT DISTINCT to_char(HIRE_DATE, 'MM/DD/YYYY/hh24:mi') THE_DATE FROM SF_HOURLY_DETAIL WHERE VESSEL_ID = '".$vessel_number."' AND (SERVICE_CODE LIKE '622%' OR SERVICE_CODE LIKE '672%') ORDER BY THE_DATE";
...
// use a function to sum the results
		if(substr($looping_date_row['SERVICE_CODE'], 0, 3) == '622'){
			$BH_REG_current += compile_truckload_hours($looping_date_row['STARTING'], $looping_date_row['ENDING'], $current_date, 'ST', $looping_date_row['PAY_LUNCH'], $looping_date_row['PAY_DINNER']);
			$BH_OT_current += compile_truckload_hours($looping_date_row['STARTING'], $looping_date_row['ENDING'], $current_date, 'OT', $looping_date_row['PAY_LUNCH'], $looping_date_row['PAY_DINNER']);
			$BH_DT_current += compile_truckload_hours($looping_date_row['STARTING'], $looping_date_row['ENDING'], $current_date, 'DT', $looping_date_row['PAY_LUNCH'], $looping_date_row['PAY_DINNER']);
		}
		if(substr($looping_date_row['SERVICE_CODE'], 0, 3) == '672'){
			$ST_REG_current += compile_ST_hours($looping_date_row['STARTING'], $looping_date_row['ENDING'], $current_date, 'ST', $looping_date_row['PAY_LUNCH'], $looping_date_row['PAY_DINNER']);
			$ST_OT_current += compile_ST_hours($looping_date_row['STARTING'], $looping_date_row['ENDING'], $current_date, 'OT', $looping_date_row['PAY_LUNCH'], $looping_date_row['PAY_DINNER']);
			$ST_DT_current += compile_ST_hours($looping_date_row['STARTING'], $looping_date_row['ENDING'], $current_date, 'DT', $looping_date_row['PAY_LUNCH'], $looping_date_row['PAY_DINNER']);
		}
	}

	$total_for_date = ($BH_REG_current + $BH_OT_current + $BH_DT_current + $ST_REG_current + $ST_OT_current + $ST_DT_current);
	fwrite($handle, $nice_date."\t\t".number_format($BH_REG_current, 2)."\t".number_format($BH_OT_current, 2)."\t".number_format($BH_DT_current, 2)."\t".number_format($ST_REG_current, 2)."\t".number_format($ST_OT_current, 2)."\t".number_format($ST_DT_current, 2)."\t\t\t\t".number_format($total_for_date, 2)."\n");
    ...
    $ST_DT_total += $ST_DT_current;
  	...
  	$overall_total_total = $BH_REG_total + $BH_OT_total + $BH_DT_total + $ST_REG_total + $ST_OT_total + $ST_DT_total;
...
// I am guaranteed that values will be 1/4 hour increments, so 2 decimal palces is good.
  ...
  fwrite($handle, "\nTotals:\t\t".$BH_REG_total."\t".$BH_OT_total."\t".$BH_DT_total."\t".$ST_REG_total."\t".$ST_OT_total."\t".$ST_DT_total."\t\t\t\t".$overall_total_total."\n\n\n");
...
// get the advanced billed portion
  $sql = "SELECT CUSTOMER_ID, COMMODITY_CODE, SERVICE_CODE, SUM(SERVICE_AMOUNT) TOTAL_AMOUNT FROM BILLING WHERE LR_NUM = '".$vessel_number."' AND (BILLING_TYPE = 'ADTRKLOAD') AND SERVICE_STATUS = 'INVOICED' AND (SERVICE_CODE LIKE '622%' OR SERVICE_CODE LIKE '672%') GROUP BY CUSTOMER_ID, COMMODITY_CODE, SERVICE_CODE";
  ...
  $sql = "SELECT SERVICE_DATE, LABOR_RATE_TYPE, SUM(SERVICE_AMOUNT) SERVICE_TOTAL FROM BILLING WHERE LR_NUM = '".$vessel_number."' AND BILLING_TYPE IN ('TRKLOADING', 'MISC') AND SERVICE_STATUS = 'INVOICED' AND (SERVICE_CODE LIKE '622%' OR SERVICE_CODE LIKE '672%') GROUP BY SERVICE_DATE, LABOR_RATE_TYPE ORDER BY SERVICE_DATE";
...
// closing ledgers
  fwrite($handle, "Description of LCS Truckload Hours (Service Codes 6220 - 6229):\n");
  ...
  fwrite($handle, "Description of LCS Truckload-standby (Service Codes 6720 - 6729) Hours:\n");
  ...
  fwrite($handle, "Advanced: Total billed amounts under GL_CODE type \"3074\"\n");
  fwrite($handle, "       COMBINED WITH amounts under BILLING_TYPE of \"ADTRKLOAD\"\n");
  fwrite($handle, "       And that have a service code in any of the above lists\n");
  fwrite($handle, "DT / ST / OT / MH / DF:  Total billed amounts under billing type \"Trkloading\"\n");
