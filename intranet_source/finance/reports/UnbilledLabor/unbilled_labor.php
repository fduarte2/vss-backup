...
<?
	if($submit != ""){
		...
		$output = "<table border=\"1\" width=\"100%\" cellpadding=\"4\" cellspacing=\"0\">
						<tr>
							<td width=\"40%\" colspan=\"3\" align=\"center\">Start Date:".$start_date."</td>
							<td>&nbsp</td>
							<td width=\"40%\" colspan=\"3\" align=\"center\">End Date:".$end_date."</td>
						</tr>";
//					AND (LTH.BILL_STATUS IS NULL OR LTH.BILL_STATUS != 'Y')
		$sql = "SELECT LTH.TICKET_NUM, LU.USER_NAME, CU.CUSTOMER_NAME THE_CUST, LTH.VESSEL_ID, LTH.COMMODITY_CODE, COMP.COMMODITY_NAME, LTH.SERVICE_GROUP, SG.SERVICE_GROUP_NAME,
					TO_CHAR(LTH.SERVICE_DATE, 'MM/DD/YYYY') THE_DATE
				FROM LABOR_TICKET_HEADER LTH, LCS_USER LU, CUSTOMER CU, SAG_OWNER.COMMODITY_PROFILE COMP, SERVICE_GROUP SG
				WHERE LTH.USER_ID = LU.USER_ID
					...
					AND (LTH.BILL_STATUS = 'N' OR LTH.BILL_STATUS IS NULL)
					AND LTH.CUSTOMER_ID = CU.CUSTOMER_ID
					AND LTH.COMMODITY_CODE = COMP.COMMODITY_CODE
					AND LTH.SERVICE_GROUP = SG.SERVICE_GROUP_ID
				ORDER BY LTH.TICKET_NUM"; 
		ora_parse($cursor, $sql);
		ora_exec($cursor);
		if(!ora_fetch_into($cursor, $row, ORA_FETCHINTO_NULLS|ORA_FETCHINTO_ASSOC)){
			$output .= "<tr><td colspan=\"6\" align=\"center\">No Unbillied Tickets for date range.</td></tr>";
		} else {
			$output .= "<tr>
			...
			do {
				$output .= "<tr>
								...
								<td><font face=\"Verdana\" size=\"2\"><b>".$row['COMMODITY_NAME']."</td>
								<td><font face=\"Verdana\" size=\"2\"><b>".$row['SERVICE_GROUP']."-".$row['SERVICE_GROUP_NAME']."</td>
							</tr>";
