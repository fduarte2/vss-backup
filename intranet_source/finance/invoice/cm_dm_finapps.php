...
//  $line_context="BNI";
 ...
 // Grab all sent items
  $sql = "select memo_num as invoice_num, customer_id, invoice_date, asset_code, service_code, gl_code, commodity_code, service_description, tosolomon, service_amount, lr_num, service_status 
			from billing 
			where tosolomon is null and service_status in ('CREDITMEMO', 'DEBITMEMO') order by invoice_num";
...
", "", $description_array[$i]);
	...
	$service_status_array[$i] = ora_getcolumn($cursor, 11);
...
//   echo "h= ".$h." bnicust = ".$bni_cust_id." i= ".$i."<br>";
   ...
   if(!ora_fetch($prod_cursor)){
		...
		$address_id = ora_getcolumn($prod_cursor, 0);
   		...
   		$sql = "select sum(service_amount) totalamt from billing where memo_num = '" . $invoice_array[$h] . "' and service_status in ('CREDITMEMO', 'DEBITMEMO')";
//		echo $sql;
   		$statement = ora_parse($cursor, $sql);
   		ora_exec($cursor);
   		$total = ora_getcolumn($cursor, 0);

		if($service_status_array[$h] == "CREDITMEMO"){
			$batch_source="BNI Credit Memo";
			$pay_term_id = "null";
			$trx_type = "Credit Memo - DSPC";
			$line_context="BNI Credit Memo";
		} else {
			$batch_source="BNI Debit Memo";
			$pay_term_id = 4;
			$trx_type = "Debit Memo - DSPC";
			$line_context="BNI Debit Memo";
		}
		//inset total
...
//						echo "c ".$sql."<br>";
		...
		// Update BNI to reflect import
             	$sql_update = "update billing set tosolomon = 'L' where memo_num = '" .  $invoice_array[$h] . "'";
