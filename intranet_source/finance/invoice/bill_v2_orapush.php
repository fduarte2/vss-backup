<?
/*
*		Adam Walter, Jul 2014
*
*		Moving of the "v2 bills system" to ora_finapps
*
*****************************************************************/
...
// $prod_conn = ora_logon("APPS@PROD_BACKUP", "APPS");
 ...
 $prod_cursor = ora_open($prod_conn);
    ...
    $solomon_array= array();
    $lr_num= array();
    $distributions= array(); // this is a number, that is the count of distinct values in all of the other arrays for the given invoice.
    $asset_array= array();
    $service_array= array();
    $gl_array= array();
 ...
 // Grab all sent items
  $sql = "select bh.invoice_num, bh.customer_id, bh.invoice_date, bd.asset_code, nvl(bd.service_code_det, bh.service_code), bd.gl_code, bh.commodity_code, 
			bh.billing_type, bh.tosolomon, bd.service_amount, '0' 
			from bill_header bh, bill_detail bd
			where tosolomon is null 
				and service_status in ('INVOICED')
...
//   		$sql = "select sum(service_amount) totalamt from billing where invoice_num = '" . $invoice_array[$h] . "' and service_status = 'INVOICED'";
   		$sql = "select sum(BD.service_amount) totalamt 
				from bill_header BH, bill_detail BD 
				where BH.invoice_num = '" . $invoice_array[$h] . "' and BH.service_status = 'INVOICED' and BH.billing_num = BD.billing_num";
   		$statement = ora_parse($cursor, $sql);
   		ora_exec($cursor);
   		$total = round(ora_getcolumn($cursor, 0), 2);
		//inset total
...
//		echo $insert_interface."<br>";
		...
		//insert distributions.  each "ARRAY" is an array for the current invoice, which was compiled as a comma-separated array earlier.
...
//							echo $insert_distributions."<br>";
		...
		// Update BNI to reflect import
             	$sql_update = "update bill_header set tosolomon = 'L' where invoice_num = '" .  $invoice_array[$h] . "'";
