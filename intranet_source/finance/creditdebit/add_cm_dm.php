...
<?
	if($system == "BNI"){
		...
		$sql = "SELECT MAX(BD.ASSET_CODE) ASSET_CODE, MAX(BD.GL_CODE) GL_CODE, SUM(SERVICE_AMOUNT) SERVICE_AMOUNT,
					BH.SERVICE_CODE, BH.ARRIVAL_NUM, BH.CUSTOMER_ID, BH.COMMODITY_CODE, BH.BILLING_TYPE, BH.BILLING_NUM THE_BIL
				FROM BILL_HEADER BH, BILL_DETAIL BD
				WHERE INVOICE_NUM = '".$inv_num."'
					AND BH.BILLING_NUM = BD.BILLING_NUM
				GROUP BY BH.SERVICE_CODE, BH.ARRIVAL_NUM, BH.CUSTOMER_ID, BH.COMMODITY_CODE, BH.BILLING_TYPE, BH.BILLING_NUM";
	}
//	echo $sql."<br>";
