...
$sql = "insert into $table (hire_date, employee, start_time, end_time, earning_type_id, commodity, customer_id, service,
	vessel_id, equipment_id, location_id, special_code, duration, supervisor, sf_row_number, employee_type_id,
	pay_lunch, pay_dinner)
	select h.hire_date, h.employee_id, h.start_time, h.end_time, h.earning_type_id, h.commodity_code, 
...
$sql = "insert into $table (hire_date, employee, start_time, end_time, earning_type_id, commodity, customer_id, service,
        vessel_id, equipment_id, location_id, special_code, duration, supervisor, sf_row_number, employee_type_id,
        pay_lunch, pay_dinner)
	select h.hire_date, h.employee_id, h.start_time, h.hire_date + 1, h.earning_type_id, h.commodity_code, 
	h.customer_id, h.service_code, h.vessel_id, h.equipment_id, h.location_id, h.special_code, 
...
$sql = "insert into $table (hire_date, employee, start_time, end_time, earning_type_id, commodity, customer_id, service,
        vessel_id, equipment_id, location_id, special_code, duration, supervisor, sf_row_number, employee_type_id,
        pay_lunch, pay_dinner)
	select h.hire_date + 1 , h.employee_id, h.hire_date + 1 , h.end_time + 1, h.earning_type_id, h.commodity_code, 
	h.customer_id, h.service_code, h.vessel_id, h.equipment_id, h.location_id, h.special_code, 
...
$sql = "UPDATE $table SET SERVICE = (SELECT SERVICE_NAME 
	...
	WHERE TO_CHAR(SERVICE_CODE) = SERVICE)";
$statement = ora_parse($cursor, $sql);
if (ora_exec($cursor)){
        ora_commit($conn_lcs);
}else{
