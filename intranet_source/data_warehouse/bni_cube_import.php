...
$host = "dspc-s17";
  ...
  if ($handle = fopen ($basefile, "r")){
	while (!feof ($handle)) {
          	$fLine = fgets($handle);
		list($run_date, $sys, $trucked_in, $comm, $cust, $vess, $mark, $id, $location, $unit, $received, $storage_end, $qty) = split(",",$fLine); 
		if ($comm ==""){
			$comm = $pre_comm;
		}else{
			$pre_comm = $comm;
		...
		$sql = "insert into cargo_detail (run_date, location, commodity_code, qty, qty_unit, cargo_id, cargo_customer, cargo_system, date_received, lr_num, storage_end, commodity_name, vessel_name, customer_name, cargo_mark, trucked_in) values ('$run_date', '$location', '$comm_code', $qty, '$unit', '$id', '$cust_id', '$sys', '$received','$lr_num',$storage_end,'$comm','$vess','$cust','$mark','$trucked_in')";
