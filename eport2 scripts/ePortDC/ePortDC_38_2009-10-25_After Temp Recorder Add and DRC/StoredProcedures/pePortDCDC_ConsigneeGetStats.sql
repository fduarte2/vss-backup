if exists (select * from sysobjects where id = object_id(N'pePortDCDC_ConsigneeGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_ConsigneeGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_ConsigneeGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_Consignee] DC_Consignee_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
