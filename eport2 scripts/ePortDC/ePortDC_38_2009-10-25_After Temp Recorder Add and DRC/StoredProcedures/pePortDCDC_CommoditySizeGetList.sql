if exists (select * from sysobjects where id = object_id(N'pePortDCDC_CommoditySizeGetList') and sysstat & 0xf = 4) drop procedure pePortDCDC_CommoditySizeGetList 
GO

-- Returns a query resultset from table [dbo].[DC_CommoditySize]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortDCDC_CommoditySizeGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[DC_CommoditySize] DC_CommoditySize_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY DC_CommoditySize_.[SizeId] asc '
        ...
        SELECT Identity(int,1,1) AS IS_ROWNUM_COL,
         ...
         FROM [dbo].[DC_CommoditySize], #IS_SECONDARY_TEMP_T_GETLIST
             WHERE 1=2 
        ...
        SET @l_query_cols = 
            N'DC_CommoditySize_.[SizeId],
            DC_CommoditySize_.[Descr],
            DC_CommoditySize_.[Price],
            DC_CommoditySize_.[SizeHigh],
            DC_CommoditySize_.[SizeLow],
            DC_CommoditySize_.[WeightKG],
            CAST(BINARY_CHECKSUM(DC_CommoditySize_.[SizeId],DC_CommoditySize_.[Descr],DC_CommoditySize_.[Price],DC_CommoditySize_.[SizeHigh],DC_CommoditySize_.[SizeLow],DC_CommoditySize_.[WeightKG]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [SizeId] from #IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS, ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'WHERE DC_CommoditySize_.[SizeId] = IS_ALIAS.[SizeId] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(DC_CommoditySize_.[SizeId],DC_CommoditySize_.[Descr],DC_CommoditySize_.[Price],DC_CommoditySize_.[SizeHigh],DC_CommoditySize_.[SizeLow],DC_CommoditySize_.[WeightKG]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[DC_CommoditySize] DC_CommoditySize_ ' + 
            'WHERE 1=2;'
