if exists (select * from sysobjects where id = object_id(N'pePortDCDC_UserGetList') and sysstat & 0xf = 4) drop procedure pePortDCDC_UserGetList 
GO

-- Returns a query resultset from table [dbo].[DC_User]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortDCDC_UserGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[DC_User] DC_User_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY DC_User_.[UserId] asc '
        ...
        SELECT Identity(int,1,1) AS IS_ROWNUM_COL,
         ...
         FROM [dbo].[DC_User], #IS_SECONDARY_TEMP_T_GETLIST
             WHERE 1=2 
        ...
        SET @l_query_cols = 
            ...
            DC_User_.[Password],
            DC_User_.[Phone],
            DC_User_.[PhoneMobile],
            DC_User_.[Printer],
            DC_User_.[Role],
            CAST(BINARY_CHECKSUM(DC_User_.[UserId],DC_User_.[Email],DC_User_.[Name],DC_User_.[Password],DC_User_.[Phone],DC_User_.[PhoneMobile],DC_User_.[Printer],DC_User_.[Role]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [UserId] from #IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS, ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'WHERE DC_User_.[UserId] = IS_ALIAS.[UserId] ' 
