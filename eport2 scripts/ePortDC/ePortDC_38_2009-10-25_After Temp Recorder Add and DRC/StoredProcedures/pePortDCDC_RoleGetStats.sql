if exists (select * from sysobjects where id = object_id(N'pePortDCDC_RoleGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_RoleGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_RoleGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_Role] DC_Role_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
