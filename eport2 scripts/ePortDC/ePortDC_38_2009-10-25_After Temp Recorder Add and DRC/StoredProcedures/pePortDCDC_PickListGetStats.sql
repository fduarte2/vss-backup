if exists (select * from sysobjects where id = object_id(N'pePortDCDC_PickListGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_PickListGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_PickListGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_PickList] DC_PickList_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
