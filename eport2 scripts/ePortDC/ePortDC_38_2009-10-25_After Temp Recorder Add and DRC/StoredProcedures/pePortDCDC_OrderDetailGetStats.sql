if exists (select * from sysobjects where id = object_id(N'pePortDCDC_OrderDetailGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_OrderDetailGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_OrderDetailGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_OrderDetail] DC_OrderDetail_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
