if exists (select * from sysobjects where id = object_id(N'pePortDCDC_TransporterDrillDown') and sysstat & 0xf = 4) drop procedure pePortDCDC_TransporterDrillDown 
GO

-- This stored procedure will return query result based on 
-- the passed in select, search and ORDER BY clauses.
CREATE PROCEDURE pePortDCDC_TransporterDrillDown
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_createtemp_from = 
            ' FROM [dbo].[DC_Transporter] AS [DC_Transporter_], #IS_SECONDARY_TEMP_T_GETLIST'
