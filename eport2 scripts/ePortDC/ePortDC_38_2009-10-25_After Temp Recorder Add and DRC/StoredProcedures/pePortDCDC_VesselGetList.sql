if exists (select * from sysobjects where id = object_id(N'pePortDCDC_VesselGetList') and sysstat & 0xf = 4) drop procedure pePortDCDC_VesselGetList 
GO

-- Returns a query resultset from table [dbo].[DC_Vessel]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortDCDC_VesselGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[DC_Vessel] DC_Vessel_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY DC_Vessel_.[VesselId] asc '
        ...
        SELECT Identity(int,1,1) AS IS_ROWNUM_COL,
         ...
         FROM [dbo].[DC_Vessel], #IS_SECONDARY_TEMP_T_GETLIST
             WHERE 1=2 
        ...
        SET @l_query_cols = 
            N'DC_Vessel_.[VesselId],
            DC_Vessel_.[ArrivalDate],
            DC_Vessel_.[FixedFreight],
            DC_Vessel_.[VesselName],
            CAST(BINARY_CHECKSUM(DC_Vessel_.[VesselId],DC_Vessel_.[ArrivalDate],DC_Vessel_.[FixedFreight],DC_Vessel_.[VesselName]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [VesselId] from #IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS, ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'WHERE DC_Vessel_.[VesselId] = IS_ALIAS.[VesselId] ' 
