if exists (select * from sysobjects where id = object_id(N'pePortDCDC_PackHouseGetList') and sysstat & 0xf = 4) drop procedure pePortDCDC_PackHouseGetList 
GO

-- Returns a query resultset from table [dbo].[DC_PackHouse]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortDCDC_PackHouseGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[DC_PackHouse] DC_PackHouse_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY DC_PackHouse_.[PackHouseId] asc '
        ...
        SELECT Identity(int,1,1) AS IS_ROWNUM_COL,
         ...
         FROM [dbo].[DC_PackHouse], #IS_SECONDARY_TEMP_T_GETLIST
             WHERE 1=2 
        ...
        SET @l_query_cols = 
            N'DC_PackHouse_.[PackHouseId],
            DC_PackHouse_.[GroupName],
            DC_PackHouse_.[PackHouseName],
            CAST(BINARY_CHECKSUM(DC_PackHouse_.[PackHouseId],DC_PackHouse_.[GroupName],DC_PackHouse_.[PackHouseName]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [PackHouseId] from #IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS, ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'WHERE DC_PackHouse_.[PackHouseId] = IS_ALIAS.[PackHouseId] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(DC_PackHouse_.[PackHouseId],DC_PackHouse_.[GroupName],DC_PackHouse_.[PackHouseName]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[DC_PackHouse] DC_PackHouse_ ' + 
            'WHERE 1=2;'
