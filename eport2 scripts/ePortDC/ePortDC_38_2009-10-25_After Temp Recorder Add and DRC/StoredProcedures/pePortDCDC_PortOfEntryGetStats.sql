if exists (select * from sysobjects where id = object_id(N'pePortDCDC_PortOfEntryGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_PortOfEntryGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_PortOfEntryGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_PortOfEntry] DC_PortOfEntry_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
