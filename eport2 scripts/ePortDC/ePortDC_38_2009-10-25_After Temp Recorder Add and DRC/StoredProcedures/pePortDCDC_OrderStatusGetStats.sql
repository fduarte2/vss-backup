if exists (select * from sysobjects where id = object_id(N'pePortDCDC_OrderStatusGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_OrderStatusGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_OrderStatusGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_OrderStatus] DC_OrderStatus_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
