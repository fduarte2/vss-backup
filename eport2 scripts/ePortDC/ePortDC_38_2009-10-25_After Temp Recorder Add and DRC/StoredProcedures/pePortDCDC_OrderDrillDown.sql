if exists (select * from sysobjects where id = object_id(N'pePortDCDC_OrderDrillDown') and sysstat & 0xf = 4) drop procedure pePortDCDC_OrderDrillDown 
GO

-- This stored procedure will return query result based on 
-- the passed in select, search and ORDER BY clauses.
CREATE PROCEDURE pePortDCDC_OrderDrillDown
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_createtemp_from = 
            ' FROM [dbo].[DC_Order] AS [DC_Order_], #IS_SECONDARY_TEMP_T_GETLIST'
