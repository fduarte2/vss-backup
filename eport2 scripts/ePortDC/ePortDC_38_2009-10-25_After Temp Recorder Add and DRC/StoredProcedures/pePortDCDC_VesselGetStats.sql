if exists (select * from sysobjects where id = object_id(N'pePortDCDC_VesselGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_VesselGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_VesselGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_Vessel] DC_Vessel_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
