if exists (select * from sysobjects where id = object_id(N'pePortDCDC_OrderGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_OrderGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_OrderGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_Order] DC_Order_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
