if exists (select * from sysobjects where id = object_id(N'pePortDCDC_PickListDrillDown') and sysstat & 0xf = 4) drop procedure pePortDCDC_PickListDrillDown 
GO

-- This stored procedure will return query result based on 
-- the passed in select, search and ORDER BY clauses.
CREATE PROCEDURE pePortDCDC_PickListDrillDown
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_createtemp_from = 
            ' FROM [dbo].[DC_PickList] AS [DC_PickList_], #IS_SECONDARY_TEMP_T_GETLIST'
