if exists (select * from sysobjects where id = object_id(N'pePortDCDC_PackHouseGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_PackHouseGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_PackHouseGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_PackHouse] DC_PackHouse_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
