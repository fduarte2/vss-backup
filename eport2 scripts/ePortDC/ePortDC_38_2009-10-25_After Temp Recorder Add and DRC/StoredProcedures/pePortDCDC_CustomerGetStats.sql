if exists (select * from sysobjects where id = object_id(N'pePortDCDC_CustomerGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_CustomerGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_CustomerGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_Customer] DC_Customer_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
