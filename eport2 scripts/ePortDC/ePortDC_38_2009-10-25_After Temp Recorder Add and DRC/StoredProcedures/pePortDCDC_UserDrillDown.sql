if exists (select * from sysobjects where id = object_id(N'pePortDCDC_UserDrillDown') and sysstat & 0xf = 4) drop procedure pePortDCDC_UserDrillDown 
GO

-- This stored procedure will return query result based on 
-- the passed in select, search and ORDER BY clauses.
CREATE PROCEDURE pePortDCDC_UserDrillDown
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_createtemp_from = 
            ' FROM [dbo].[DC_User] AS [DC_User_], #IS_SECONDARY_TEMP_T_GETLIST'
