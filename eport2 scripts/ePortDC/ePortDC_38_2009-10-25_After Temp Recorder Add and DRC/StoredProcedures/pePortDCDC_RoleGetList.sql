if exists (select * from sysobjects where id = object_id(N'pePortDCDC_RoleGetList') and sysstat & 0xf = 4) drop procedure pePortDCDC_RoleGetList 
GO

-- Returns a query resultset from table [dbo].[DC_Role]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortDCDC_RoleGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[DC_Role] DC_Role_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY DC_Role_.[RoleId] asc '
        ...
        SELECT Identity(int,1,1) AS IS_ROWNUM_COL,
         ...
         FROM [dbo].[DC_Role], #IS_SECONDARY_TEMP_T_GETLIST
             WHERE 1=2 
        ...
        SET @l_query_cols = 
            N'DC_Role_.[RoleId],
            DC_Role_.[Descr],
            CAST(BINARY_CHECKSUM(DC_Role_.[RoleId],DC_Role_.[Descr]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [RoleId] from #IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS, ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'WHERE DC_Role_.[RoleId] = IS_ALIAS.[RoleId] ' 
