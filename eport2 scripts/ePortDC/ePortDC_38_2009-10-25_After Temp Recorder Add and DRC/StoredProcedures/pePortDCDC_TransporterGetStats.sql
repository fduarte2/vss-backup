if exists (select * from sysobjects where id = object_id(N'pePortDCDC_TransporterGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_TransporterGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_TransporterGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_Transporter] DC_Transporter_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
