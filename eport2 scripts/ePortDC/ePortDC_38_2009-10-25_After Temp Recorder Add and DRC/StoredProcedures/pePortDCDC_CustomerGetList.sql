if exists (select * from sysobjects where id = object_id(N'pePortDCDC_CustomerGetList') and sysstat & 0xf = 4) drop procedure pePortDCDC_CustomerGetList 
GO

-- Returns a query resultset from table [dbo].[DC_Customer]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortDCDC_CustomerGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY DC_Customer_.[CustomerId] asc '
        ...
        SELECT Identity(int,1,1) AS IS_ROWNUM_COL,
         ...
         FROM [dbo].[DC_Customer], #IS_SECONDARY_TEMP_T_GETLIST
             WHERE 1=2 
        ...
        SET @l_query_cols = 
            N'DC_Customer_.[CustomerId],
            DC_Customer_.[Address],
            DC_Customer_.[City],
            DC_Customer_.[Comments],
            DC_Customer_.[CustomerName],
            DC_Customer_.[CustomerShortName],
            DC_Customer_.[DestCode],
            DC_Customer_.[NeedPARS],
            DC_Customer_.[Origin],
            DC_Customer_.[Phone],
            DC_Customer_.[PhoneMobile],
            DC_Customer_.[PostalCode],
            DC_Customer_.[StateProvince],
            CAST(BINARY_CHECKSUM(DC_Customer_.[CustomerId],DC_Customer_.[Address],DC_Customer_.[City],DC_Customer_.[Comments],DC_Customer_.[CustomerName],DC_Customer_.[CustomerShortName],DC_Customer_.[DestCode],DC_Customer_.[NeedPARS],DC_Customer_.[Origin],DC_Customer_.[Phone],DC_Customer_.[PhoneMobile],DC_Customer_.[PostalCode],DC_Customer_.[StateProvince]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [CustomerId] from #IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS, ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'WHERE DC_Customer_.[CustomerId] = IS_ALIAS.[CustomerId] ' 
