if exists (select * from sysobjects where id = object_id(N'pePortDCDC_CommoditySizeGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_CommoditySizeGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_CommoditySizeGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_CommoditySize] DC_CommoditySize_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
