if exists (select * from sysobjects where id = object_id(N'pePortDCDC_UserGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_UserGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_UserGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_User] DC_User_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
