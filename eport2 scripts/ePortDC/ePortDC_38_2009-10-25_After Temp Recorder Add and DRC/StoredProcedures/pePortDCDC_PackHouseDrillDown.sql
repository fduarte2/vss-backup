if exists (select * from sysobjects where id = object_id(N'pePortDCDC_PackHouseDrillDown') and sysstat & 0xf = 4) drop procedure pePortDCDC_PackHouseDrillDown 
GO

-- This stored procedure will return query result based on 
-- the passed in select, search and ORDER BY clauses.
CREATE PROCEDURE pePortDCDC_PackHouseDrillDown
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_createtemp_from = 
            ' FROM [dbo].[DC_PackHouse] AS [DC_PackHouse_], #IS_SECONDARY_TEMP_T_GETLIST'
