if exists (select * from sysobjects where id = object_id(N'pePortDCDC_OrderDetailExpediterGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_OrderDetailExpediterGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_OrderDetailExpediterGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_OrderDetailExpediter] DC_OrderDetailExpediter_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
