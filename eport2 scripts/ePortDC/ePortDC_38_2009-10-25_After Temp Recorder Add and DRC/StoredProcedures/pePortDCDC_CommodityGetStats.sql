if exists (select * from sysobjects where id = object_id(N'pePortDCDC_CommodityGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_CommodityGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_CommodityGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_Commodity] DC_Commodity_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
