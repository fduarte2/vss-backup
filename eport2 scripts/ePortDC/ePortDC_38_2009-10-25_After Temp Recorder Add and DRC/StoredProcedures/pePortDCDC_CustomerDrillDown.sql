if exists (select * from sysobjects where id = object_id(N'pePortDCDC_CustomerDrillDown') and sysstat & 0xf = 4) drop procedure pePortDCDC_CustomerDrillDown 
GO

-- This stored procedure will return query result based on 
-- the passed in select, search and ORDER BY clauses.
CREATE PROCEDURE pePortDCDC_CustomerDrillDown
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_createtemp_from = 
            ' FROM [dbo].[DC_Customer] AS [DC_Customer_], #IS_SECONDARY_TEMP_T_GETLIST'
