if exists (select * from sysobjects where id = object_id(N'pePortDCDC_OrderStatusGetList') and sysstat & 0xf = 4) drop procedure pePortDCDC_OrderStatusGetList 
GO

-- Returns a query resultset from table [dbo].[DC_OrderStatus]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortDCDC_OrderStatusGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[DC_OrderStatus] DC_OrderStatus_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY DC_OrderStatus_.[OrderStatusId] asc '
        ...
        SELECT Identity(int,1,1) AS IS_ROWNUM_COL,
         ...
         FROM [dbo].[DC_OrderStatus], #IS_SECONDARY_TEMP_T_GETLIST
             WHERE 1=2 
        ...
        SET @l_temp_colsWithAlias = 
            N'DC_OrderStatus_.[OrderStatusId]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            N'DC_OrderStatus_.[OrderStatusId],
            DC_OrderStatus_.[Descr],
            CAST(BINARY_CHECKSUM(DC_OrderStatus_.[OrderStatusId],DC_OrderStatus_.[Descr]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [OrderStatusId] from #IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS, ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'WHERE DC_OrderStatus_.[OrderStatusId] = IS_ALIAS.[OrderStatusId] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(DC_OrderStatus_.[OrderStatusId],DC_OrderStatus_.[Descr]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[DC_OrderStatus] DC_OrderStatus_ ' + 
            'WHERE 1=2;'
