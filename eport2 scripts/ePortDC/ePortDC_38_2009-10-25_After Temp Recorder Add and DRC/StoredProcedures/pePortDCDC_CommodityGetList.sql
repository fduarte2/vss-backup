if exists (select * from sysobjects where id = object_id(N'pePortDCDC_CommodityGetList') and sysstat & 0xf = 4) drop procedure pePortDCDC_CommodityGetList 
GO

-- Returns a query resultset from table [dbo].[DC_Commodity]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortDCDC_CommodityGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[DC_Commodity] DC_Commodity_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY DC_Commodity_.[CommodityCode] asc '
        ...
        SELECT Identity(int,1,1) AS IS_ROWNUM_COL,
         ...
         FROM [dbo].[DC_Commodity], #IS_SECONDARY_TEMP_T_GETLIST
             WHERE 1=2 
        ...
        SET @l_query_cols = 
            N'DC_Commodity_.[CommodityCode],
            DC_Commodity_.[CommodityName],
            DC_Commodity_.[HarmonizedSystemTariff],
            CAST(BINARY_CHECKSUM(DC_Commodity_.[CommodityCode],DC_Commodity_.[CommodityName],DC_Commodity_.[HarmonizedSystemTariff]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [CommodityCode] from #IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS, ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'WHERE DC_Commodity_.[CommodityCode] = IS_ALIAS.[CommodityCode] ' 
