if exists (select * from sysobjects where id = object_id(N'pePortDCDC_CustomerPriceGetStats') and sysstat & 0xf = 4) drop procedure pePortDCDC_CustomerPriceGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortDCDC_CustomerPriceGetStats
    ...
    BEGIN
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[DC_CustomerPrice] DC_CustomerPrice_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
    END
